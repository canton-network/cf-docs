...
Use the latest stable Canton SDK (3.x series). The 2.x SDK is for historical Daml development and does not support Canton Network features like the Global Synchronizer or Canton Coin.
...
For testing workflows that span multiple validators, use LocalNet rather than the single-node sandbox. LocalNet runs separate validator and participant processes that communicate through a local synchronizer, which better simulates a real network topology.
