{/* source: daml / daml/3.4/overview/explanations/ledger-model/interoperability.rst */}
{/* Copyright (c) 2023 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}
<Note>This page is a work in progress. It may contain incomplete or incorrect information.</Note>

<a id="interoperable-ledgers"></a>

# Synchronizer-aware projection

<Note>* Disentangle abuse of ledger term: the VSL from what is coordinated over a synchronizer
* Define synchronizer-aware projections (assign and unassign events)</Note>
...
Some Participant Nodes can connect to multiple ledgers and provide their parties unified access to those ledgers via the Ledger API.
...
Interoperability may limit the visibility a Participant Node has into a party’s ledger projection, i.e., its [local ledger](local-ledger.mdx#local-ledger), when the party is hosted on multiple Participant Nodes.
...
This document explains the visibility limitations due to interoperability and their consequences for the Update Service, by [example](#interop-limitation-examples) and formally by introducing interoperable versions of [causality graphs](#interop-causality-graph) and [projections](#ledger-aware-projection).

The presentation assumes that you are familiar with the following concepts:

* The Ledger API
* The [Daml Ledger Model](index.mdx#da-ledgers)
* [Local ledgers and causality graphs](local-ledger.mdx#local-ledger)

<Note>Interoperability for Daml ledgers is under active development.
...
The described services and guarantees may change without notice as the interoperability implementation proceeds.</Note>

<a id="interop-limitation-examples"></a>

## Interoperability Examples

<a id="interoperable-topology"></a>
...
{/* https://app.lucidchart.com/documents/edit/6b818d37-cf4c-4513-9d31-d68acddf4533 */}![A sample topology with two parties, three participant nodes, and two ledgers, described in detail through the remainder of this section.](explanations/images/multiple-domains.svg)
...
<a id="interoperable-aggregation"></a>
...
For example, in the [above topology](#multiple-ledgers), when a transaction creates a contract with stakeholder Alice on Ledger 2,
...
<a id="enter-leave-event"></a>

### Enter and Leave Events

With interoperability, a transaction can use a contract whose creation was recorded on a different ledger.
In the [above topology](#multiple-ledgers), e.g., one transaction creates a contract c1 with stakeholder Alice on Ledger 1 and another archives the contract on Ledger 2.
...
<a id="cross-ledger-transaction"></a>
...
For example, suppose that the [split paint counteroffer workflow](local-ledger.mdx#split-counteroffer-ledger) from the causality examples is committed as follows:
The actions on CounterOffer and PaintAgree contracts are committed on Ledger 1.
All actions on Ious are committed on Ledger 2, assuming that some Participant Node hosts the Bank on Ledger 2.
The last transaction is a cross-ledger transaction because the archival of the CounterOffer and the creation of the PaintAgreement commits on Ledger 1 simultaneously with the transfer of Alice’s Iou to the painter on Ledger 2.
...
So Alice merely *witnesses* the archival even though she is an [informee](ledger-privacy.mdx#def-informee) of the exercise.
...
<a id="interop-causality-graph"></a>

## Multi-ledger Causality Graphs

This section generalizes [causality graphs](local-ledger.mdx#causality-graph) to the interoperability setting.
...
: A **multi-ledger causality graph** G for a set Y of Daml ledgers is a finite, transitively closed, directed acyclic graph.
  ...
  |--------------------------------|-------------------|-------------------|
  | **Create**                     | no                | yes               |
  | consuming<br/>**Exercise**     | yes               | no                |
  | non-consuming<br/>**Exercise** | yes               | yes               |
  | **Fetch**                      | yes               | yes               |
  | **NoSuchKey**                  | no                | no                |
...
The [action order](local-ledger.mdx#def-action-order) generalizes to multi-ledger causality graphs accordingly.

In the [example for Enter and Leave events](#enter-leave-event) where the painter exercises three choices on contract c with signatories Alice and the painter, the four transactions yield the following multi-ledger causality graph.
...
<a id="interoperable-causality-graph-linear"></a>
![Transactions and their colors in this graph: tx1 is yellow. tf1 is yellow and green. tx2 is green. tf2 is green and yellow. tx3 is yellow. tf3 is yellow and green. tx4 is green.](explanations/images/interoperable-causality-graph-linear.svg)
<Note>As for ordinary causality graphs, the diagrams for multi-ledger causality graphs omit transitive edges for readability.</Note>

As an example of a cross-domain transaction, consider the [split paint counteroffer workflow with the cross-domain transaction](#cross-ledger-transaction).
...
<a id="counteroffer-interoperable-causality-graph"></a>
![The multi-ledger causality graph for the split paint counteroffer workflow. tx1 and tx3 are yellow. tx2 is green. tx4 is both yellow and green.](explanations/images/counteroffer-interoperable-causality-graph.svg)

### Consistency

Definition »Ledger trace«
: A **ledger trace** is a finite list of pairs (a<sub>i</sub>, b<sub>i</sub>)
  such that b<sub>i - 1</sub> = a<sub>i</sub> for all i > 0.
  Here a<sub>i</sub> and b<sub>i</sub> identify Daml ledgers or are the special value NONE,
...
: Let G be a multi-ledger causality graph and X be a set of actions from G on a contract in c.
  ...
  <br/>
  1. If X is not empty, then X contains a **Create** or at least one **Enter** action.
...
The first three conditions mimic the conditions of [causal consistency](local-ledger.mdx#def-causal-consistency-contract) for ordinary causality graphs.
...
For example, the above [multi-ledger causality graph with transfer actions](#interoperable-causality-graph-linear) is multi-ledger consistent for c.
...
and for each edge act<sub>1</sub> -> act<sub>2</sub>, the outgoing ledger color of act<sub>1</sub> is the same as the incoming ledger color of act<sub>2</sub>.
...
<Note>There is no multi-ledger consistency requirement for contract keys yet.
So interoperability does not provide consistency guarantees beyond those that come from the contracts they reference.
In particular, contract keys need not be unique and **NoSuchKey** actions do not check that the contract key is unassigned.</Note>

The [multi-ledger causality graph for the split paint counteroffer workflow](#counteroffer-interoperable-causality-graph) is multi-ledger consistent.
...
The notions [minimally consistent](local-ledger.mdx#minimal-consistent-causality-graph) and [reduction](local-ledger.mdx#def-reduction-causality-graph) therefore generalize from ordinary causality graphs accordingly.
...
: For an X-multi-ledger consistent causality graph G, there exists a unique minimal X-multi-ledger consistent causality graph reduce<sub>X</sub>(G) with the same vertices and the edges being a subset of G.
  reduce<sub>X</sub>(G) is called the X-**reduction** of G.
...
<a id="ledger-aware-projection"></a>
...
<Note>Every action contains all its subactions.
...
If P is an informee of act’, the Participant Node will mark act’ as merely being witnessed on P‘s transaction stream, as explained below.</Note>

The [cross-domain transaction in the split paint counteroffer workflow](#counteroffer-interoperable-causality-graph), for example, has the following projections for Alice and the painter on the Iou ledger (yellow) and the painting ledger (green).
...
{/* https://www.lucidchart.com/documents/edit/f8ec5741-7a37-4cf5-92a9-bf7b3132ba8e */}![Projections for various parties to the split paint counteroffer workflow. The green ledger projections include the yellow ledger, but the yellow ledger projections do not include the green ledger.](explanations/images/projecting-transactions-paint-offer-ledger-aware.svg)
...
For example, the [multi-ledger causality graph for the split paint counteroffer workflow](#counteroffer-interoperable-causality-graph) is projected as follows:

{/* https://app.lucidchart.com/documents/edit/d788b464-d670-4029-b2c0-d537c023052f */}![More projections for various parties to the split paint counteroffer workflow, showing greater detail. Alice and the painter have green and yellow, just green, and just yellow projections; the bank has only a yellow projection.](explanations/images/counteroffer-causality-ledgeraware-projection.svg)
...
* In Alice’s projection on the green ledger, Alice witnesses the archival of her Iou.
  As explained in the [Ledger API Ordering Guarantees](#interop-ordering-guarantees) below,
...
* In the Painter’s projections, the ShowIou transaction tx3 is unordered w.r.t. to the CounterOffer acceptance in tx4
  like in the [case of ordinary causality graphs](local-ledger.mdx#counteroffer-causality-projections).
  The edge tx3 -> tx4 is removed by the reduction step during projection.

The projection of transfer actions can be illustrated with the [Multi-Ledger causality graph with transfer actions](#interoperable-causality-graph-linear).
...
{/* https://app.lucidchart.com/documents/edit/edbf9aaf-b7da-4e68-b9c9-9e631c3a87bb */}![Causality graphs showing only the green or only the yellow ledger.](explanations/images/transfer-projection.svg)

<a id="interop-ordering-guarantees"></a>
...
1. **Transfer** actions with either an incoming or an outgoing ledger annotation are output as **Enter** and **Leave** events.
   **Transfer** actions with both incoming and outgoing ledger annotations are omitted.
2. The incoming and outgoing ledger annotations are not output.
...
The [ordering guarantees](local-ledger.mdx#ordering-guarantees) for single Daml ledgers extend accordingly.
