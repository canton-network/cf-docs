...
`OwnerToKeyMapping`, which as the name says, associates a key with a unique identifier.
...
*IdentifierDelegation* that allow one to do that. A namespace delegation delegates
...
4. The participant verifies that the remote synchronizer is running a protocol version compatible with the participant’s
   ...
   will fail.
5. The participant downloads and verifies the Synchronizer ID from the Synchronizer. The Synchronizer ID
...
<Note>
If a user with the specified ID already exists, then no additional user will be created,
...
[External Key Storage with a Key Management Service (KMS)](../secure/kms/mode/external_key_storage.mdx#external-key-storage) for more details.
</Note>
