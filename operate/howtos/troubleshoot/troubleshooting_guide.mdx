{/* source: canton / canton/3.4/participant/howtos/troubleshoot/troubleshooting_guide.rst */}
...
* **Create Health Dumps**
  > Ensure that you can create [Health Dumps](../observe/monitoring.mdx#creating-dumps). You need to share these health dumps during any support request. The health dumps provide a lot of diagnostic information that we need to troubleshoot issues.
* **Turn on Debug Logging**
  ...
  > The `-v` flag will turn on debug logging of all `com.digitalasset.canton` loggers, whereas `--debug` will turn on debug logging of all libraries too. Please also see [Logging](../observe/monitoring.mdx#logging).
* **Turn on Detailed API Logging**
  > You might want to turn on [detailed API logging](../observe/monitoring.mdx#detailed-logging). This writes all incoming commands and outgoing data into the log file, allowing you to inspect the details of a command that leads to failures. Beware that if your commands contain sensitive data, this data is written to the log file.
...
* **Turn on database query cost monitoring**
  ...
  > explained in [How to Diagnose Slow Database Queries](#how-to-diagnose-slow-db-queries).
...
* **Canton Error Codes**
  > All non-internal warnings and errors are [logged consistently](../../reference/error_codes.mdx#machine-readable-information) (or at least we aspire to do).The error code information listed in the documentation should contain all information you need in order to understand and possibly resolve the issue.

## Log Files

* **Canton Trace Ids**
  > All Canton log statements contain a [trace-id](../observe/monitoring.mdx#tracing). This tracing is turned on by default and the `trace-id` is passed between the distributed processes:
  > ```default
...
* **Setup and Use LNAV**
  > Setup lnav for viewing logs as described in [viewing logs](../observe/monitoring.mdx#lnav). It will require a few minutes to get used to it, but the payoff of this investment is great and comes fast. **In particular get familiar with loading multiple files, filtering, searching and jumping to errors**.
...
* Are the nodes [connected to a sequencer](../operate/synchronizers/connectivity.mdx#synchronizer-connections)? Errors that often happen here are:
...
> * overloaded - resolve by tuning according to our [performance configuration guide](../optimize/performance.mdx#performance-configuration).
> * unable to complete the transaction processing within the given time (i.e. transactions are too big) - resolve by increasing the timeouts as described in our [performance configuration guide](../optimize/performance.mdx#performance-configuration).
...
>   > Audience Based Tokens and
>   > Scope Based Tokens consult the user management service to see whether you
>   > need to grant more rights to the user. In either case debug the token to see if it contains the expected elements
>   > using [JWT.IO](https://jwt.io/#debugger-io).
...
Canton endpoints utilize the latter. The article describing [how to use HTTP/2 PING-based keep-alives](https://grpc.io/docs/guides/keepalive/)
...
approach can be used for other Canton APIs. This is described in the dedicated [keep-alive article](../configure/apis/apis.mdx#keepalive-configuration).
...
How to obtain a performant system is [extensively documented](../optimize/performance.mdx#performance-configuration).

If you have followed that documentation, we can assume that:

> * Your database pools are sufficiently sized: check metric `db-storage.queue` and you’ve [correctly setup your database](../optimize/storage.mdx#postgres-performance-tuning).
...
> * You have [optimized your model](../optimize/performance.mdx#model-tuning) to reduce the transaction size.
...
<a id="how-to-measure-db-performance"></a>
...
<a id="how-to-diagnose-slow-db-queries"></a>
...
> * verify whether you might be affected by [contention on shared resources in a shared environment](../optimize/storage.mdx#shared-env-performance).
...
<a id="configure-a-temporary-admin-user"></a>
...
[the previous chapter](#configure-a-temporary-admin-user)
...
<Note>Declarative api requires setting the non-standard-config and admin-token-config.admin-claims flags that allow
it operate with the authority of the internal canton admin.</Note>
...
<Note>Bootstrap script requires setting the non-standard-config and admin-token-config.admin-claims flags that allow
it operate with the authority of the internal canton admin.</Note>
