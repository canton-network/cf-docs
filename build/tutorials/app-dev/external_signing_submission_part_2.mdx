...
<Danger>Like in Part 1, the transaction must be [validated](external_signing_submission.mdx#tutorial-external-signing-validate), [hashed](external_signing_submission.mdx#canton-recompute-external-transaction-hash) and [signed](external_signing_submission.mdx#canton-externally-sign-transaction).
The hash computation and signature is performed by the `execute_and_get_contract_id` function provided at the end of Part 1, as shown in the next section.</Danger>
