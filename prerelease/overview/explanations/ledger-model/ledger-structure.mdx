<!-- source: daml / daml/3.5/overview/explanations/ledger-model/ledger-structure.rst -->
...
based on the templates from the [running example](index.md#da-ledgers-running-example).
...
The [integrity section](ledger-integrity.md#da-model-integrity) goes into the details of this.
...
and forms the basis for the [privacy model](ledger-privacy.md#da-model-privacy).
...
The [privacy model](ledger-privacy.md#da-model-privacy) uses the concept of subtransactions to define the visibility rules.
...
Due to the [privacy model](ledger-privacy.md#da-model-privacy), not everyone sees all parts of a party interaction.
...
#### NOTE
The [integrity conditions](ledger-integrity.md#da-model-integrity) on a ledger require that the happens-before order respects the lifecycle of contracts.
...
We discuss the more general partial orders in the [causality section](local-ledger.md#local-ledger).
