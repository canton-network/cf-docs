{/* source: daml / daml/3.4/sdk/reference/damllf/daml-lf-translation.rst */}
...
[Built-in data types](../daml/data-types.mdx#daml-ref-built-in-types) in Daml have straightforward mappings to Daml-LF.
...
Be aware that only the Daml primitive types exported by the [Prelude](../daml/stdlib/Prelude.mdx#module-prelude-72703) module map to the Daml-LF primitive types above. That means that, if you define your own type named `Party`, it will not translate to the Daml-LF primitive `Party`.

## Tuple types

Daml tuple type constructors take types `T1, T2, …, TN` to the type `(T1, T2, …, TN)`. These are exposed in the Daml surface language through the [Prelude](../daml/stdlib/Prelude.mdx#module-prelude-72703) module.
...
This section uses the syntax for Daml [records](../daml/data-types.mdx#daml-ref-record-types) with curly braces.
...
it can be upgraded via [smart contract upgrades](../../sdlc-howtos/smart-contracts/upgrade/smart-contract-upgrades.mdx#smart-contract-upgrades):
...
by smart contract upgrades is available in [Limitations in Upgrading Variants](../../sdlc-howtos/smart-contracts/upgrade/smart-contract-upgrades.mdx#limitations-in-upgrading-variants).

## Type synonyms

[Type synonyms](../daml/data-types.mdx#daml-ref-type-synonyms) (starting with the `type` keyword) are eliminated during conversion to Daml-LF. The body of the type synonym is inlined for all occurrences of the type synonym name.
...
A [template declaration](../daml/templates.mdx#daml-ref-template-name) in Daml results in one or more data type declarations behind the scenes. These data types, detailed in this section, are not written explicitly in the Daml program but are created by the compiler.
