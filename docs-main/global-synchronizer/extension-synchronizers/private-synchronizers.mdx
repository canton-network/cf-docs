...
- **Privacy** — Transactions on a private synchronizer are only visible to the synchronizer operator and connected participants, not to the entire Global Synchronizer network
...
Canton's multi-synchronizer protocol allows a single participant to be connected to multiple synchronizers simultaneously. Contracts can live on any connected synchronizer, and the Canton protocol handles cross-synchronizer transactions transparently when contracts on different synchronizers need to interact.
