...
DEFAULT_CACHE_DIR = DEFAULT_CACHE_ROOT / "daml-script"
DEFAULT_MANIFEST = REPO_ROOT / ".internal" / "generated" / "x2mdx" / "daml-script" / "manifest.json"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs-main" / "appdev" / "reference" / "daml-script"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate Daml Script docs from local SDK artifacts, write an x2mdx manifest, and render MDX pages."
    )
    ...
    parser.add_argument(
        "--source-name",
        default="Published Daml Script docs JSON from local SDK artifacts",
...
def insert_group_after_label(items: list[Any], *, after_label: str, group: dict[str, Any]) -> None:
    after_index = find_group_index(items, after_label)
    existing_index = find_group_index(items, GROUP_LABEL)
    if existing_index is not None:
        items.pop(existing_index)
        if after_index is not None and existing_index < after_index:
            after_index -= 1
    if after_index is None:
        items.append(group)
        return
    items.insert(after_index + 1, group)
...
def update_docs_navigation(
    *,
    ...
    insert_group_after_label(target_pages, after_label=STDLIB_GROUP_LABEL, group=group)
