...
The backend is the layer between your frontend and the Canton ledger. It submits commands, reads transactions, and queries contract state. This page walks through the common patterns used in Canton backend development, using the [cn-quickstart Java backend](https://github.com/digital-asset/cn-quickstart/tree/main/quickstart/backend) as the primary reference.

## Backend Language

The [cn-quickstart](https://github.com/digital-asset/cn-quickstart) project uses a Java backend built on Spring Boot. Java has first-class code generation support (`dpm codegen-java`) and is the most mature option for Canton backends.
