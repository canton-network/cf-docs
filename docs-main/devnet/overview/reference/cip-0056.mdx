...
Registry URLs for a given instrument are currently maintained by wallets themselves. A generic discovery mechanism based on the [Canton Name Service](/devnet/overview/reference/canton-name-service) is planned, where registry URLs would be stored in CNS entry metadata and retrievable through the Scan API.
...
- [Canton Coin Tokenomics](/devnet/overview/reference/canton-coin-tokenomics) -- fee structure and UTXO dust expiry for CC specifically
