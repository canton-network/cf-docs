...
**Source:** `docs/replicated/quickstart/3.4/sdk/quickstart/configure/project-structure-overview.rst`
...
The backend uses Transcode-generated Java classes from DAR files to provide type-safe ledger interactions. Rebuild the backend with `make build` to regenerate these classes after updating Daml contracts.
