...
Canton Coin is converted to **traffic credits** which are then consumed by transactions. Traffic credits are non-transferable—once CC is converted to traffic, it can only be used to pay for transaction fees.

### How Traffic Works

```mermaid
flowchart LR
    USER[User/App] --> VAL[Validator]
    VAL --> TX[Submit Transaction]
    TX --> SIZE[Calculate Size]
    SIZE --> COST[Deduct CC]
    COST --> COMMIT[Transaction Commits]
```
...
2. **Top up** when your balance gets low
...
| **Auto-top-up** | Configure automatic top-ups to maintain traffic balance |
| **Reward management** | Handle earned rewards |

<Note>
Validators can configure automatic top-ups to ensure they always have sufficient traffic for their parties' transactions. This converts CC to traffic credits automatically when the balance falls below a threshold.
</Note>
