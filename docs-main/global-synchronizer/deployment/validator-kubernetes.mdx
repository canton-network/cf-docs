...
import CantonGlobalSynchronizerReferenceConfigurationReferenceL328 from "/snippets/canton-docs/global-synchronizer_reference_configuration-reference_L328.mdx";
import CantonGlobalSynchronizerReferenceConfigurationReferenceL142 from "/snippets/canton-docs/global-synchronizer_reference_configuration-reference_L142.mdx";
...
import ExternalSpliceMainSpliceRstCodeDocsSrcValidatorOperatorValidatorHelmBash514 from "/snippets/external/splice/main/splice-rst-code-docs-src-validator-operator-validator-helm-bash-514.mdx";
...
This page walks through deploying a Canton Network validator on Kubernetes using Helm charts. For Docker Compose, see [Docker Compose Validator Deployment](/global-synchronizer/deployment/validator-docker-compose).
...
<CantonGlobalSynchronizerReferenceConfigurationReferenceL328 />
...
<CantonGlobalSynchronizerReferenceConfigurationReferenceL142 />
...
Configure how your validator connects to the network's **scan** services by defining a `scanClient` block in your `standalone-validator-values.yaml`. The block supports three modes (`bft`, `bft-custom`, `trust-single`) — see the comments in the example below.

{/* Source: hyperledger-labs/splice apps/app/src/pack/examples/sv-helm/standalone-validator-values.yaml SCAN_CLIENT_CONFIGURATION block */}
...
#   seedUrls: ["TRUSTED_SCAN_URL"] # replace with actual scan seed url(s)
...
For your selected `scanClient` type, replace `TRUSTED_SCAN_URL` with a URL of a Scan you host or trust that is reachable by your validator (for example, the GSF scan URL). For `bft` and `bft-custom` modes, you can specify multiple seed URLs separated by commas.

You also need to configure how your validator's participant connects to **sequencers** by defining a `synchronizer` block in your `standalone-validator-values.yaml`:

{/* Source: hyperledger-labs/splice apps/app/src/pack/examples/sv-helm/standalone-validator-values.yaml SYNCHRONIZER_CONFIGURATION block */}
...
If you are redeploying the validator app as part of a synchronizer migration, update the `migration.id` value in your `standalone-validator-values.yaml` to the new migration ID announced for the migrated synchronizer:

{/* Source: hyperledger-labs/splice apps/app/src/pack/examples/sv-helm/standalone-validator-values.yaml MIGRATION block */}
...
<ExternalSpliceMainSpliceRstCodeDocsSrcValidatorOperatorValidatorHelmBash451 />
...
<ExternalSpliceMainSpliceRstCodeDocsSrcValidatorOperatorValidatorHelmBash514 />
...
By default your node is configured to automatically purchase traffic on a pay-as-you-go basis. To disable or tune the top-up loop, edit the `topup` section in your `standalone-validator-values.yaml`:

{/* Source: hyperledger-labs/splice apps/app/src/pack/examples/sv-helm/standalone-validator-values.yaml CONFIGURING_TOPUP block */}
...
{/* Source: hyperledger-labs/splice apps/app/src/pack/examples/sv-helm/validator-values.yaml SWEEP block */}
...
{/* Source: hyperledger-labs/splice apps/app/src/pack/examples/sv-helm/validator-values.yaml AUTO_ACCEPT block */}
...
{/* Source: hyperledger-labs/splice apps/app/src/pack/examples/sv-helm/validator-values.yaml PARTICIPANT_PRUNING_SCHEDULE block */}
