...
For the structure of a template, see [Overview: Template Structure](structure.md).
...
- The name is used when [creating](updates.md#daml-ref-create) a contract of this template (usually, from within a choice).
...
- `with` keyword. The parameters are in the form of a [record type](data-types.md#daml-ref-record-types).
- Passed in when [creating](updates.md#daml-ref-create) a contract from this template. These are then in scope inside the template body.
- A template parameter can’t have the same name as any [choice arguments](choices.md#daml-ref-choice-arguments) inside the template.
...
- Use when a party needs visibility on a contract, or be informed or contract events, but is not a [signatory](#daml-ref-signatories) or [controller](choices.md#daml-ref-controllers).
...
- See [Reference: Choices](choices.md) for full reference information.
...
2. Provide a sensible encoding of them over the [Ledger API](../glossary.md#ledger-api).
...
- For a full explanation, see [Reference: Contract Keys](contract-keys.md#contractkeys).
...
- The clause must start with the keywords `interface instance`, followed by
  ...
  must be implemented.
- See [Reference: Interfaces](interfaces.md) for full reference information on interfaces, or
  section [Interface Instances](interfaces.md#interface-instances) for interface instances specifically.
