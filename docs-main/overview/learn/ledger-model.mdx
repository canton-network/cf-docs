...
**When to use:** When a Party's agreement essential to the contract's existence.
...
|------|-------------|----------|---------------|--------------|
| **Signatory** | N/A | Always | If controller | Must authorize |
| **Observer** | N/A | Always | If controller | No |
| **Controller** | N/A | Choice + consequences | Yes (their choices) | Via consuming choice |
| **Actor** | Yes | If stakeholder | If controller | If signatory |

## Transaction Structure

Transactions in Canton are trees of **actions**. Each action creates, exercises, or fetches* contracts.

(*Note that fetches are not typically returned in the transaction stream. See [Here](TODO) for more detail).
...
A transaction tree records all creates and exercises that occur within a single transaction, ordered by cause and effect.
...
Atomicity on these exercises is enforced by the ledger. Partial execution is impossible.
