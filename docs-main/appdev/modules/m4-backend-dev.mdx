...
Wait for all services to come up. The onboarding containers will automatically register the app-user tenant and create an `AppInstallRequest`.

**Create a license to test with.** The quickstart doesn't create licenses automatically — they're created by the app provider through the `AppInstall` workflow. Log in to the application frontend at `http://app-provider.localhost:3000` as `app-provider` (password: check your Keycloak realm config, or use the Keycloak admin console at `http://keycloak.localhost:8082` with `admin`/`admin`). From the UI, accept the pending app install request and create a license. Alternatively, use the backend API directly:

1. Accept the app install: `POST /app-installs/{contractId}:accept`
2. Create a license: `POST /app-installs/{contractId}/licenses`

**Get an auth token.** The backend uses OAuth2 via Keycloak. The easiest way is to log in through the frontend at `http://app-provider.localhost:3000` and copy the bearer token from your browser's network inspector (look for the `Authorization` header on any `/licenses` request).

Alternatively, obtain a token from the command line using the client credentials grant with the backend service account:
...
**Get a license contract ID.** List the active licenses:

```bash
curl -s http://localhost:8080/licenses \
  -H "Authorization: Bearer $TOKEN" | jq .
```

Pick a `contractId` from the response. Now test the comment endpoints:

```bash
# Add a comment
curl -X POST "http://localhost:8080/licenses/<CONTRACT_ID>/comments?commandId=$(uuidgen)" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"body": "Renewed for another quarter"}'

# List comments
curl -s "http://localhost:8080/licenses/<CONTRACT_ID>/comments" \
  -H "Authorization: Bearer $TOKEN" | jq .
```

Replace `<CONTRACT_ID>` with the contract ID from the previous step.
