...
When sequencer pruning is enabled, you must keep historical backups for each pruning window. The time difference between two historical backups must be smaller than the `retentionPeriod`. See the [SV Backup](/global-synchronizer/production-operations/sv-backup) page for full backup requirements.
