...
2. **Projection Distribution**: Each stakeholder receives only the transaction projection they're entitled to see
3. **Independent Validation**: Each stakeholder validates their projection against Daml rules
...
sequenceDiagram
    ...
    Med->>V1: Deliver projection (encrypted)
    Med->>V2: Deliver projection (encrypted)
    V1->>V1: Validate projection
    V2->>V2: Validate projection
    V1->>Med: Confirmation
    V2->>Med: Confirmation
...
- Current 3.4-era deployments use CometBFT-based ordering
...
| **L2 Rollups** | Sequencer | Fraud/validity proofs | Limited; composability is usually constrained across L2s |
