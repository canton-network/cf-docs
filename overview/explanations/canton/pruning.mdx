{/* source: canton / canton/3.4/overview/explanations/canton/pruning.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="pruning-overview"></a>
...
Canton implementation. Please refer to the how-to guide on participant pruning,
and how-to guide on sequencer and mediator pruning for details on how to configure pruning.
...
{/* https://lucid.app/lucidchart/6e815c36-b8e8-4f77-919b-2ff1d02d169f/edit?invitationId=inv_cfe298c2-1432-46df-b1b0-270878f6e253 */}![image](explanations/canton/images/pruning-history_v_snapshot.svg)
<Note>The participant retains some of its historical data in a long-term store, called the
[Participant Query Store (PQS)](https://docs.digitalasset-staging.com/build/3.3/component-howtos/pqs/).
...
Once a node has been pruned up to time t, it will not be able to serve events that precede t.</Note>
...
* Data items pertaining to the ordering layer when using the BFT Orderer.
  ...
  get them unstuck.
  <Note>For ordering layers other than the BFT orderer, such as those based on external drivers, pruning is handled separately.</Note>
...
* Never deletes its most recently observed transaction in the deduplication window, which has a [configurable length](https://docs.digitalasset-staging.com/build/3.3/sdlc-howtos/applications/develop/command-deduplication)
...
* Never deletes data, e.g., member’s previous timestamp, unless the member acknowledges that it no longer needs it.
  ...
  Please refer to the [global synchronizer documentation](https://docs.dev.sync.global/sv_operator/sv_pruning.html#sequencer-pruning) and the
  how-to guide on sequencer pruning for details.
...
[non-repudiation section](#pruning-non-repudiation) below.
...
<a id="pruning-non-repudiation"></a>

## Non-repudiation

<Note>A fork represents a situation where two or more participant nodes in the network diverge in their view of their shared
state. Forks can happen due to misconfiguration, e.g., when changing contract stores via the repair service, bugs, or
malicious behavior.</Note>
...
<Note>Commitments enable participant nodes to verify agreement on their shared state, so that they can prune history.</Note>
...
<Note>Two participant nodes that have no shared active contracts do not exchange commitments.</Note>
...
<Note>The reconciliation interval can be adjusted on a running synchronizer. The period length change takes effect
immediately.</Note>
...
<Note>Sequencer time advancements that a participant node observers are called **ticks**, and are said to *tick* the ACS
commitment processor.</Note>
...
<Note>Each commitment contains the cryptographic hash summarizing the ACS state, as well as metadata about the period and
sender.</Note>
...
<a id="commitments-optimizations"></a>
