...
A party is a **witness** of an action if it is an informee of that action's view. Witnesses are responsible for validating the portion of the transaction they can see and sending confirmation or rejection messages to the mediator via the sequencer.
...
Consistency also requires **key consistency** for contracts with keys. Contract key semantics are being reworked in Canton 3.x — keys exist syntactically in Daml, but the uniqueness guarantees differ from 2.x where no two active contracts of the same template could share the same contract key. Contract key support is under active development for Canton 3.5.
