...
Deployments without authentication need no changes, as long as you use the `.env` file shipped with the new release, in which both audiences are already equal.

If your deployment uses authentication (`-a`) with different values for `LEDGER_API_AUTH_AUDIENCE` and `VALIDATOR_AUTH_AUDIENCE`, as the placeholders in the `.env` file of earlier releases and the [community Keycloak configuration guide](/global-synchronizer/deployment/community-keycloak-config) suggest, `start.sh` refuses to start after the upgrade and reports that the two audiences must be equal. Pick one of the following:

1. Keep your configuration and add `-G` to your `start.sh` command. This deploys exactly what earlier releases deployed, without the Wallet Gateway and the Splice Portfolio. You can enable them later by following the second option.
2. Move to a single audience:
   ...
   4. Run your `start.sh` command without `-G`. Users have to log in to the wallet and CNS UIs again, as their existing tokens carry the old audience.
...
Set up an OIDC provider that meets the [OIDC provider requirements](/global-synchronizer/deployment/validator-kubernetes#oidc-provider-requirements) described for the Kubernetes deployment, which also includes a walkthrough for [Auth0](/global-synchronizer/deployment/validator-kubernetes#configuring-an-auth0-tenant). Community-contributed guides cover [Okta and Keycloak](/global-synchronizer/deployment/oidc-providers) and a [Keycloak setup](/global-synchronizer/deployment/community-keycloak-config). The URLs to configure for callbacks are `http://wallet.localhost` and `http://ans.localhost`, and `http://walletgateway.localhost/callback` for the Wallet Gateway (unless you skip it with `-G`).
...
| VALIDATOR_AUTH_AUDIENCE | The audience for the validator backend API, e.g. `https://canton.network.global`. Must equal `LEDGER_API_AUTH_AUDIENCE` unless you skip the Wallet Gateway with `-G`. |
...
| WALLET_GATEWAY_UI_CLIENT_ID | The client id of the OAuth app for the Wallet Gateway UI. Not needed if you skip the Wallet Gateway with `-G`. |
