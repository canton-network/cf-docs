...
description: "Deploy a Canton Network validator node on Kubernetes using Helm charts"
---
...
flowchart LR
    ...
    subgraph SV1["SV Node"]
        SC1["Scan"]
        SVA1["SV App"]
        SEQ1["Sequencer"]
    end

    subgraph SV2["SV Node"]
        SC2["Scan"]
        SVA2["SV App"]
        SEQ2["Sequencer"]
    end
...
The validator supports non-authenticated deployments as well, but this is strongly discouraged for production deployments. If you wish to run without authentication, please refer to the notes in [Running without Authentication](#running-without-authentication).

### OIDC Provider Requirements

This section provides pointers for setting up an OIDC provider for use with your Validator node. Feel free to skip directly to [Configuring an Auth0 Tenant](#configuring-an-auth0-tenant) if you plan to use [Auth0](https://auth0.com) for your Validator node's authentication needs.

These docs focus on Auth0, and are being continuously tested and maintained. Other OIDC providers can be used, and are in active use by various community members, who have contributed some notes and examples in the [Okta and Keycloak community authored examples](https://docs.dev.sync.global/community/oidc-config-okta-keycloak.html).
...
An identifier that is unique to the user must be set via the `sub` field of the issued JWT. On some occasions, this identifier will be used as a user name for that user on your Validator node's Canton participant. In [Installing the Software](#installing-the-software), you will be required to configure a user identifier as the `validatorWalletUser` -make sure that whatever you configure there matches the contents of the `sub` field of JWTs issued for that user.
...
We are going to use these values, exported to environment variables named as per the `Name` column, in [Configuring Authentication on your Validator](#configuring-authentication-on-your-validator) and [Installing the Software](#installing-the-software).
...
In case you are facing trouble with setting up your (non-Auth0) OIDC provider, it can be beneficial to skim the instructions in [Configuring an Auth0 Tenant](#configuring-an-auth0-tenant) as well, to check for functionality or configuration details that your OIDC provider setup might be missing.
...
We are now going to configure your Validator node software based on the OIDC provider configuration values you exported to environment variables at the end of either [OIDC Provider Requirements](#oidc-provider-requirements) or [Configuring an Auth0 Tenant](#configuring-an-auth0-tenant). (Note that some authentication-related configuration steps are also included in [Installing the Software](#installing-the-software))
...
```bash
helm install postgres <CHART_REPO>/splice-postgres \
  -n validator --version ${CHART_VERSION} \
  -f splice-node/examples/sv-helm/postgres-values-validator-participant.yaml --wait

helm install participant <CHART_REPO>/splice-participant \
  -n validator --version ${CHART_VERSION} \
  -f splice-node/examples/sv-helm/participant-values.yaml \
  -f splice-node/examples/sv-helm/standalone-participant-values.yaml --wait

helm install validator <CHART_REPO>/splice-validator \
  -n validator --version ${CHART_VERSION} \
  -f splice-node/examples/sv-helm/validator-values.yaml \
  -f splice-node/examples/sv-helm/standalone-validator-values.yaml --wait
```
...
```bash
helm install cluster-ingress-validator <CHART_REPO>/splice-cluster-ingress-runbook \
  -n validator --version ${CHART_VERSION} \
  -f splice-node/examples/sv-helm/validator-cluster-ingress-values.yaml
```
...
By default your node will be configured to automatically purchase traffic on a pay-as-you-go basis. To disable or tune to your needs, edit the following section in the validator-values.yaml file:
...
{/* TODO: show error message for insufficient funds during traffic purchase (error-insufficient-funds), and link to the option to disable automatic top-ups and third-party traffic providers */}
...
Note that if your node is down for longer than the pruning window (48 hours in the example above), your node will most probably get corrupted, as the apps race catching up with the participant's attempts to keep pruning. It is therefore advisable to set the pruning window to a value that you are comfortable with in terms of guaranteeing uptime of your node. Setting it to 30 days is in general a reasonable choice, as the sequencers currently are also pruned after 30 days, so you will not be able to catch up with the network after a longer downtime anyway (see the [Splice documentation](https://docs.dev.sync.global) for disaster recovery guidelines).

Refer to the Canton documentation for more details on participant pruning:

- [https://docs.daml.com/ops/pruning.html](https://docs.daml.com/ops/pruning.html)
- [https://docs.daml.com/canton/usermanual/pruning.html](https://docs.daml.com/canton/usermanual/pruning.html)
