...
description: "Which APIs constitute the supported public surface, the feature status labels, and the compatibility rules that apply to them."
---

{/* DIVERGENCE: omitted the "Labs" label and its status column, the "Support" row ("Covered by standard commercial support terms"), the "Distribution and Releases" row, the "Compatibility" and "Documentation" rows, and the deprecation-window table. Also generalized "Daml's public API" to "the public API" (this docs set spans Canton, Splice, PQS, and the SDKs; Upstream docs-website source (docs/replicated/daml/3.5/overview/reference/status-definitions.rst, hash 4ca9c5c5) */}
