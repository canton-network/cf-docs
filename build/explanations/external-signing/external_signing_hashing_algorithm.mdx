{/* source: canton / canton/3.4/sdk/explanations/external-signing/external_signing_hashing_algorithm.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="external-signing-hashing-algo"></a>
...
and provide the required cryptographic guarantees. See [https://protobuf.dev/programming-guides/serialization-not-canonical/](https://protobuf.dev/programming-guides/serialization-not-canonical/) for more information on the topic.
...
Refer to the official protobuf documentation for more information about protobuf to Java type mappings: [https://protobuf.dev/programming-guides/proto3/#scalar](https://protobuf.dev/programming-guides/proto3/#scalar)
In particular:

<Note>In Java, unsigned 32-bit and 64-bit integers are represented using their signed counterparts, with the top bit simply being stored in the sign bit</Note>
...
- Addition of an `interface_id` field in [Fetch nodes](#fetch-node-encoding) for support of Daml interfaces.
- Addition of the hashing scheme version in the [final hash](#final-hash-encoding) to make the hash more robust to cross version collisions.
- Replace `ledger_effective_time` in the [metadata](#metadata-encoding) with `min_ledger_effective_time` and `max_ledger_effective_time`.
...
<Note>Not all protobuf types are described here, only the ones necessary to encode a PreparedTransaction message.</Note>
<Danger>Even default values must be included in the encoding.
For instance if an int32 field is not set in the serialized protobuf, its default value (0) should be encoded.
Similarly, an empty repeated field still results in a 0x00 byte encoding (see the repeated section below for more details)</Danger>
...
<Note>This encoding function also applies to lists generated from utility functions (e.g: `split`).</Note>
...
<Danger>`encode(node_id)` effectively finds the corresponding node in the list of nodes and encodes the node.
...
when recursively encoding sub-nodes of `Exercise` and `Rollback` nodes as seen below.</Danger>

### Node

<Note>Each node’s encoding is prefixed with additional meta-information about the node, this is made explicit in the encoding of each node.</Note>
...
<Danger>For Exercise nodes, the node seed **MUST** be defined. Therefore it is encoded as a **non** optional field, as noted via the
...
and it should be reported as a bug.</Danger>
<Note>The last encoded value of the exercise node is its `children` field. This recursively traverses the transaction tree.</Note>

<a id="fetch-node-encoding"></a>
...
<Note>Rollback nodes do not have an lf version.</Note>
