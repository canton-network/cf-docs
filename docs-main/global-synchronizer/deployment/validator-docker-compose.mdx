...
The Wallet Gateway only serves users that have been onboarded to the validator, so first [log in to the wallet UI](#logging-into-the-wallet-ui) with the user you want to use. Then open the Wallet Gateway, select the `Splice validator` network and log in with that username as the `Client ID` and `unsafe` as the `Client Secret`, replacing the prefilled Client ID. Your wallet party shows up on the `Parties` page as your primary party. You can create additional parties there, as described in [Party Management](/integrations/wallet-gateway/use/party-management).
...
<Note>
`LEDGER_API_AUTH_AUDIENCE` and `VALIDATOR_AUTH_AUDIENCE` in the `.env` file must be equal for the Splice Portfolio to work. The shipped defaults differ, so set both to the same value, e.g. `https://canton.network.global`. `start.sh` refuses to start with `-g` otherwise.
