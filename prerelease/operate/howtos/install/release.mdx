<!-- source: canton / canton/3.5/participant/howtos/install/release.rst -->
...
You can also use the Daml Enterprise Canton Docker images by following our [Docker instructions](docker.md#install-with-docker).
...
For this purpose, there are some storage [mixin configurations](../configure/general/conf_file.md#configuration-mixin) (`config/storage/`) defined.
...
#### NOTE
Please also consult the more [detailed section on storage configurations](../configure/storage/storage.md#storage-config).
...
the [Postgres performance guide](../optimize/storage.md#postgres-performance-tuning) for further information.
...
associated with the node’s identity. The Ledger API supports [JWT based authentication](../secure/apis/jwt.md#ledger-api-jwt-configuration).
On the Admin API, you can enable mTLS. Please consult the [TLS documentation section](../secure/apis/tls.md#tls-configuration) for
further information.
...
various command line options available, for example to further tune the [logging configuration](../observe/monitoring.md#logging).
...
the name used in the configuration file. Please consult the [identity management section](../operate/identity_management.md#identity-management-user-manual)
for further information.
...
2. All nodes are managed through the administration API. Whenever you use the console, almost all requests will
   ...
   the [TLS documentation section](../secure/apis/tls.md#tls-configuration).
3. Applications and users interact with the Participant Node using the gRPC Ledger API. We recommend that you secure your
   API by using TLS. You should also [authorize your clients using JWT tokens](../secure/apis/jwt.md#ledger-api-jwt-configuration).
...
* [Static configuration](../configure/general/conf_file.md#static-configuration) are items which are not supposed to change and are therefore captured in the configuration file.
...
* Dynamic configuration are items such as Daml archives (DARs), synchronizer connections, or parties. All such changes are effected
  through [console commands](../operate/console/console.md#canton-console) (or the [administration APIs](../operate/console/console.md#administration-apis)).
...
The final step is to connect the participant to the synchronizer. Refer to [the connectivity guide](../operate/synchronizers/connectivity.md#synchronizer-connections)
...
2. The administration API should be secured using client certificates as described in [TLS documentation section](../secure/apis/tls.md#tls-configuration).
