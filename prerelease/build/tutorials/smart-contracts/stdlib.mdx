...
An example of a relatively wide Tuple can be found in the test modules of the [Handle exceptions (Deprecated)](exceptions) project.
...
You’ve seen typeclasses in use all the way from [Data types](data). It’s now time to look under the hood.
...
The `Eq` typeclass allows values of a type to be compared for (in)-equality. It makes available two function: `==` and `/=`. Most data types from the standard library have an instance of `Eq`. As you already learned in [Data types](data), you can let the compiler automatically derive instances of `Eq` for you using the `deriving` keyword.
