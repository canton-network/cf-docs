...
The project both changes and adds to the `Iou` model presented in [Parties and authority](parties):
...
This project showcases how you can put the `Update` and `Script` actions you learned about in [Parties and authority](parties) to good use. For example, the `Merge` and `Split` choices each perform several actions in their consequences.
...
In [Handle exceptions (Deprecated)](exceptions), we will learn about how errors in your model can be handled in Daml.
