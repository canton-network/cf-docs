...
<div className="todo">
**TODO (DevNet, Splice 0.6.x):** Major upgrades on DevNet now use Logical Synchronizer Upgrade (LSU) exclusively; Hard Domain Migration (HDM) and HDM-based disaster recovery were removed in Splice 0.6. The "Major Upgrades" section below still references HDM patterns and needs to be rewritten for LSU. The `serial ID` deployment concept replaces the migration ID in synchronizer naming, helm values, ports, and DBs; the `synchronizers` Helm value supersedes `domain` and the older Scan address fields. See [Splice 0.6 release notes](https://docs.dev.sync.global/release_notes.html) and the [Logical Synchronizer Upgrade](/devnet/global-synchronizer/production-operations/logical-synchronizer-upgrade) page.
</div>
...
Make sure to read the [Release Notes](/devnet/global-synchronizer/release-notes/current-release) to learn
...
Version upgrades can be done by each node independently and only require
a `helm upgrade`. Make sure to read the [Release Notes](/devnet/global-synchronizer/release-notes/current-release) to learn
about changes you may need to make as part of the upgrade.

Protocol upgrades are performed through [logical synchronizer upgrades](/devnet/global-synchronizer/production-operations/logical-synchronizer-upgrade),
...
For the full validator-side procedure (state preservation, migration dumps, catching up, deploying validator app and participant on Kubernetes or Docker Compose), see [Validator Major Upgrades](/devnet/global-synchronizer/production-operations/validator-major-upgrade).
...
For the full SV-side procedure (deploying new Canton components and CometBFT, migration dumps, updating apps, recovering from a failed upgrade, organizational details), see [SV Major Upgrades](/devnet/global-synchronizer/production-operations/sv-major-upgrade).
...
- [Backup and Recovery](/devnet/global-synchronizer/production-operations/backup-and-recovery) — Backup procedures before upgrades
- [Release Notes](/devnet/global-synchronizer/release-notes/current-release) — Current release changes and requirements
