...
- _auth_ (required): Authentication configuration for normal ledger operations. This is the method users go through in the UI — use `authorization_code` for interactive flows or `self_signed` for development
- _adminAuth_ (optional): Authentication configuration for admin operations. Only needed for operations requiring elevated privileges. This is used by the backend only — use `client_credentials` for machine-to-machine authentication
