...
import { networkData as dashboardData } from '/snippets/generated/version-dashboard-data.mdx';


export const componentDescriptions = {
  splice: { text: 'Canton Network infrastructure including validator apps, wallet, and governance tools.', link: 'https://github.com/hyperledger-labs/splice/releases', linkText: 'View releases' },
  ...
  pqs: { text: 'Recommended Participant Query Store / scribe runtime version for the Canton version currently deployed on Canton Network.', link: 'https://docs.digitalasset.com/build/3.5/component-howtos/pqs/download.html', linkText: 'Documentation' },
  ...
  walletGateway: { text: 'Remote client package for wallet gateway integrations.', link: 'https://www.npmjs.com/package/@canton-network/wallet-gateway-remote', linkText: 'npm package' },
}
...
export const darTooltips = {
  ...
  'splice-amulet-name-service': 'Name service contracts for registering and resolving human-readable names.',
  'splice-wallet': 'Wallet application contracts for user balances, payment requests, and transaction history.',
  'splice-wallet-payments': 'Payment workflow contracts used by wallet payment and settlement flows.',
  'splice-dso-governance': 'Decentralized Synchronizer Operator governance contracts for voting, proposals, and network rules.',
  'splice-validator-lifecycle': 'Validator onboarding and lifecycle management contracts.',
}
...
export const VersionDashboard = ({ data }) => {
  const [showAllEndpoints, setShowAllEndpoints] = React.useState(false)
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
        <NetworkCard network="mainnet" data={data.mainnet} />
        <NetworkCard network="testnet" data={data.testnet} />
        <NetworkCard network="devnet" data={data.devnet} />
      </div>
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {Object.entries(data).map(([key, network]) => (
              <tr key={key} className="border-t border-gray-800">
                <td className="py-2 font-medium" style={{ color: network.color }}>{network.name}</td>
                <td className="py-2 font-mono text-xs text-gray-400 break-all">{network.endpoint}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {[
            ...
            { name: 'Splice Releases', url: 'https://github.com/hyperledger-labs/splice/releases' },
            { name: 'Daml SDK Releases', url: 'https://github.com/digital-asset/daml/releases' },
            { name: 'Docker Images', url: 'https://ghcr.io/digital-asset/decentralized-canton-sync' },
            { name: 'Wallet Kernel', url: 'https://github.com/hyperledger-labs/splice-wallet-kernel' },
          ].map((link) => (
            <a
              key={link.name}
              href={link.url}
...
<Info>Generated from live Canton Network docs, the matching release bundle, and published package versions.</Info>

<VersionDashboard data={dashboardData} />
