<!-- source: daml / daml/3.5/sdk/component-howtos/smart-contracts/daml-studio.rst -->
...
Install Daml Studio by [installing the DPM](../../dpm/dpm.md#dpm), and ensuring you
...
provided in `build-options`, See [Daml Yaml Build Options](assistant/config-files.md#daml-yaml-build-options).
...
for use with [environment variable interpolation support](assistant/config-files.md#id1). To make this work in Daml Studio,
...
See [Daml Assistant Upgrade](assistant.md#daml-assistant-upgrade) and the `--replace` flag in [Reference](#daml-studio-cli-options) for more details.
...
Follow the [Daml Assistant Decomission](assistant.md#daml-assistant-decomission) for removal Daml itself.
...
- `--replace`
  ...
  - `always` will use the extension bundled with whichever SDK version is being used (as selected by the `daml.yaml`, See [Daml Assistant version management](assistant.md#daml-assistant-version-management).
