...
The server streams a sequence of `Either_JsCantonError_JsGetUpdatesResponse` messages. Each message is one of:

**Success** — a `JsGetUpdatesResponse` containing a single update:
