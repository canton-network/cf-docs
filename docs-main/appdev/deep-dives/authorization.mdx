---
title: "Authorization"
description: "Access tokens, identity providers, scopes, and rights for the Canton Ledger API."
---
...
![A flowchart illustrating the process of authentication described in the two paragraphs immediately above.](./images/Authentication.svg)
...
### Scope-Based Tokens (deprecated)

**Deprecated** since Canton 3.5.1: using (only) `scope` to target a specific API with an optional `aud` does not conform to JWT best practices.
Consider using Audience-Based Tokens instead.
...
Canton 3.6 and later can also accept self-issued JWTs.
This allows users to authenticate to the Ledger API using a JWT signed by the private key held by the primary (external) party.
To enable this, first update the participant configuration and add a `type = party-jwt` authentication entry:
...
The users for which the Party JWTs are issued should not be associated with any IDP, in other words, `identityProviderId` must not be set.
In addition to that, the user attribute `primaryPartyAuthentication` must be set to `true`.

The external private key can then be used to mint a JWT of the shape:
...
Note that this access token can be used to perform any action that the user has the rights for,
so it is not limited to the signing party.
...
## Enabling authorization on a participant node

Authorization is configured on the participant node operator's side, not by the application developer.

To enable it, set `auth-services` under the participant's `ledger-api` configuration block to point at your identity provider's JWKS endpoint (or a shared secret, for simpler setups). See [gRPC Ledger API Configuration](/global-synchronizer/reference/api-configuration#grpc-ledger-api-configuration) for the config block and the available `auth-services` and `jwt-timestamp-leeway` parameters.

For OAuth 2.0/OIDC identity provider setup specifically (Keycloak, Auth0, and similar), see [OIDC Providers](/global-synchronizer/deployment/oidc-providers).
