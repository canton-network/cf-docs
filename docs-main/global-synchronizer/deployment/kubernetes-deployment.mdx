...
4)  Please download the release artifacts containing the sample Helm value files, from here: <a href="https://github.com/digital-asset/decentralized-canton-sync/releases/download/v0.6.12/0.6.12_splice-node.tar.gz">Download Bundle (DevNet 0.6.12)</a>, and extract the bundle:
...
4)  Please download the release artifacts containing the sample Helm value files, from here: <a href="https://github.com/digital-asset/decentralized-canton-sync/releases/download/v0.6.11/0.6.11_splice-node.tar.gz">Download Bundle (TestNet 0.6.11)</a>, and extract the bundle:
...
4)  Please download the release artifacts containing the sample Helm value files, from here: <a href="https://github.com/digital-asset/decentralized-canton-sync/releases/download/v0.6.10/0.6.10_splice-node.tar.gz">Download Bundle (MainNet 0.6.10)</a>, and extract the bundle:
...
cd cometbft
# Init the node
docker run --rm -v "$(pwd):/init" ghcr.io/digital-asset/decentralized-canton-sync/docker/cometbft:0.6.12 init --home /init
# Read the node id and keep a note of it for the deployment
docker run --rm -v "$(pwd):/init" ghcr.io/digital-asset/decentralized-canton-sync/docker/cometbft:0.6.12 show-node-id --home /init
```
...
cd cometbft
# Init the node
docker run --rm -v "$(pwd):/init" ghcr.io/digital-asset/decentralized-canton-sync/docker/cometbft:0.6.11 init --home /init
# Read the node id and keep a note of it for the deployment
docker run --rm -v "$(pwd):/init" ghcr.io/digital-asset/decentralized-canton-sync/docker/cometbft:0.6.11 show-node-id --home /init
```
Please keep a note of the node ID printed out above.
...
cd cometbft
# Init the node
docker run --rm -v "$(pwd):/init" ghcr.io/digital-asset/decentralized-canton-sync/docker/cometbft:0.6.10 init --home /init
# Read the node id and keep a note of it for the deployment
docker run --rm -v "$(pwd):/init" ghcr.io/digital-asset/decentralized-canton-sync/docker/cometbft:0.6.10 show-node-id --home /init
```
Please keep a note of the node ID printed out above.
...
- `https://scan.sv.<YOUR_HOSTNAME>/registry` should be routed to `/registry` at port 5012 in service `scan-app` in the `sv` namespace. This endpoint should be publicly accessible without any IP restrictions.
...
* **For Transfer Factory endpoints** (`/registry/transfer-instruction/v1/transfer-factory` and `/registry/transfer-instruction/v2/transfer-factory`):
    ...
    * **Per-IP limit:** 50 requests per minute
* **For all other `/registry/*` endpoints:**
    * **Global limit:** 200 requests per minute
    * **Per-IP limit:** 30 requests per minute
