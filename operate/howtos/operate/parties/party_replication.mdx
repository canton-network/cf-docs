{/* source: canton / canton/3.4/participant/howtos/operate/parties/party_replication.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="party-replication"></a>
...
[simple party replication](#replicate-before-party-is-used) steps.

Otherwise, you must apply an
[offline party replication](#offline-party-replication)
procedure.

<Note>**Party replication** is different from **party migration**. A party
migration includes an additional final step, that is removing (or *offboarding*)
the party from its original participant.

Party offboarding, and thus party migration, is currently not supported.</Note>

<a id="party-replication-authorization"></a>
...
[party-to-participant mapping topology transaction](#multi-hosting-authorization).
...
external party onboarding documentation.
...
party-to-participant mapping topology transaction.
...
<a id="replicate-before-party-is-used"></a>
...
<Warning>If a party has already participated in any Daml transaction, you must use
[offline party replication](#offline-party-replication)
instead.</Warning>
...
1. [Create the party](parties.mdx#party-management), either in the namespace of a
   participant or in a dedicated namespace.
2. [Vet packages](../packages/packages.mdx#package-vetting).
3. [Authorize](#party-replication-authorization) one or more additional participants to host the party.
4. Use the party.
...
[Create a party](parties.mdx#party-management) Alice:
...
<Note>In this example, the **local party Alice** is owned by the `source` participant,
...
namespace, or create an external party.</Note>

### 2. Vet packages

[Vet packages](../packages/packages.mdx#package-vetting) on the target participant(s) **before** proceeding.

<Note>If you are unfamiliar with this process, read this general explanation of
package vetting.</Note>

<a id="multi-hosting-authorization"></a>
...
A participant can host a party with different permissions.
...
See onboarding process for more details.
...
<Note>The participant permission here must be the same as in the previous step.
For external parties in particular this must be either `Confirmation` or `Observation`.</Note>
...
<Note>For **external parties**, the threshold is defined during the onboarding process already,
so this section is not relevant to them.</Note>
...
<a id="offline-party-replication"></a>

## Offline party replication

Offline party replication is a [multi-step, manual process](#offline-party-replication-steps).

Before replication can start, both the target participant and the party itself must
[explicitly consent to the new hosting arrangement](#party-replication-authorization).
...
<Note>* Connect a single Canton console to both the source and target participants
...
* While you onboard the party on the target participant you may detect ACS
  ...
  during the party replication procedure.</Note>
<Warning>**Be advised: You must back up the target participant before you start the ACS import!**
...
participant and **still complete the ongoing offline party replication**.</Warning>

<a id="offline-party-replication-steps"></a>
...
2. **Source: Stop Pruning** – Turn off automatic pruning on the source participant.
3. **Target: Authorization** - Target participant authorizes new hosting with the onboarding flag set.
4. **Target: Disconnect** - Target participant disconnects from all synchronizers.
...
11. **Source: Resume Pruning** - Reschedule the automatic pruning on the source participant.

<Warning>Offline party replication must be performed with care, strictly following
...
adjustments. Test thoroughly in a test environment before production use.</Warning>
...
Ensure the target participant [vets all packages](../packages/packages.mdx#package-vetting) associated with
...
@ target.dars.upload("dars/CantonExamples.dar")
    res7: String = "75a1d6515f6e1557025463a78a808e690422669368b3cc1eb946907520af1d65"
```
...
Stop any, automatic or manual pruning
...
<Warning>Please ensure the onboarding flag is set with `requiresPartyToBeOnboarded = true`.</Warning>
```none
@ target.topology.party_to_participant_mappings
    .propose_delta(
      party = alice,
...
<Warning>Again, please ensure the onboarding flag is set with `requiresPartyToBeOnboarded = true`
for a local party, and with `onboarding = HostingParticipant.Onboarding()` for external party.</Warning>
...
@ source.topology.party_to_participant_mappings
    ...
    res14: SignedTopologyTransaction[TopologyChangeOp, PartyToParticipant] = SignedTopologyTransaction(
      TopologyTransaction(
        PartyToParticipant(
          Alice::12201ff69b1d...,
          PositiveNumeric(1),
...
<Warning>**Please back up the target participant before importing the ACS!**</Warning>
...
@ val (onboarded, minimalSafeClearingTs) = target.parties
    ...
    (onboarded, minimalSafeClearingTs) : (Boolean, Option[com.digitalasset.canton.data.CantonTimestamp]) = (false, Some(value = 2026-01-10T22:27:19.914303Z))
```

The command returns a tuple indicating the status:

- `(true, None)`: The onboarding flag is cleared. Proceed to the next step.
- `(false, Some(CantonTimestamp))`: The onboarding flag is still set. Removal is safe
...
Because this command is idempotent, you can call it repeatedly. Thus, you may also poll
this command until it confirms that the onboarding flag has been cleared. The following
snippet demonstrates how this command can be polled.
...
<Note>The `timeout` is based on the default *decision timeout* of 1 minute.</Note>
