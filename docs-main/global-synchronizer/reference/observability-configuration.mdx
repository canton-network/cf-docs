...
To avoid becoming overwhelmed by the number of metrics and log messages, read the section Golden Signals and Key Metrics Quick Start below for an overview of how most metrics are exposed. It describes the philosophy behind metric naming and labeling.
...
The gRPC protocol is layered on top of HTTP/2, so certain labels (such as the `service`) from the above section are included. The labels added by default to each gRPC API metric are as follows:
...
- Each participant node measures the count of the inflight (dirty) requests so the user can see if the `maxInflightValidationRequests` resource limit is close to being hit. The metrics are: `daml_participant_inflight_validation_requests` and `daml_participant_max_inflight_validation_requests`.
...
The submission of the first of the three Daml commands (the creation of the Ping contract) starts at span 3 in the example trace. Span 2 will only complete once the three Daml commands have finished.
...
- `Ports`: the ports on which the participant node exposes the Ledger API, the Admin API and, if enabled, the JSON API
...
- `Version`: the release version of the node
- `Supported protocol version(s)`: the protocol versions supported by the node

A sequencer node responds with a message containing:

- `Sequencer id`: the unique identifier of the sequencer
- `Synchronizer id`: the unique identifier of the synchronizer
- `Uptime`: the uptime of this node
- `Ports`: the ports on which the sequencer exposes the Public and the Admin API
- `Connected participants`: the list of connected participants
- `Connected mediators`: the list of connected mediators
- `Sequencer`: the sequencer health status, with an `active` flag and optional `details`
- `Accepts admin changes`: a boolean flag indicating whether the sequencer accepts admin changes
- `Version`: the release version of the node
- `Protocol version`: the protocol version of the synchronizer

A mediator node responds with a message containing:

- `Node uid`: the unique identifier of the node
- `Synchronizer id`: the unique identifier of the synchronizer
- `Uptime`: the uptime of this node
- `Ports`: the port on which the mediator exposes the Admin API
- `Active`: true if this instance is the active replica (It can be false in the case of the passive instance of a high-availability deployment.)
- `Version`: the release version of the node
- `Protocol version`: the protocol version of the synchronizer
