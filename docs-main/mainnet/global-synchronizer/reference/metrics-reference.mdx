...
This section will be expanded in a future update. Topology metrics track synchronizer membership changes and party-to-participant mappings. For monitoring guidance, see [Performance Optimization](/mainnet/global-synchronizer/production-operations/performance-optimization).

## Key participant metrics

These are the most operationally significant metrics from the participant node. For the full catalog of several hundred metrics, see the [Canton 3.4 metrics reference](https://docs.digitalasset.com/canton/3.4/participant/reference/metrics).
