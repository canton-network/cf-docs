---
title: "allocation instruction off-ledger API"
description: "Generated lifecycle view for an OpenAPI specification"
---

Generated from supplied versioned OpenAPI artifacts.

## Table of Contents

| Name | Kind | Summary | Introduced | Changed | Deprecated | Removed |
| --- | --- | --- | --- | --- | --- | --- |
| [`/registry/allocation-instruction/v1/allocation-factory`](#endpoint-path-registry-allocation-instruction-v1-allocation-factory) | - | Get the factory and choice context for creating allocations using the `AllocationFactory_Allocate` choice. | `0.5.10` | - | - | - |

## Version Change Summary

| Version | Added | Changed | Removed |
| --- | --- | --- | --- |
| `0.5.10` | `9` | `0` | `0` |
| `0.5.11` | `0` | `0` | `0` |
| `0.5.12` | `0` | `0` | `0` |
| `0.5.13` | `0` | `0` | `0` |
| `0.5.14` | `0` | `0` | `0` |
| `0.5.15` | `0` | `0` | `0` |
| `0.5.16` | `0` | `0` | `0` |
| `0.5.17` | `0` | `0` | `0` |
| `0.5.18` | `0` | `0` | `0` |

## Reference


<a id="endpoint-path-registry-allocation-instruction-v1-allocation-factory"></a>

### `/registry/allocation-instruction/v1/allocation-factory`

<a id="endpoint-post-registry-allocation-instruction-v1-allocation-factory"></a>

- Method: `POST`

- Operation ID: `getAllocationFactory`
- Description: Get the factory and choice context for creating allocations using the `AllocationFactory_Allocate` choice.

**Request Body**

- Required: `yes`

| Content Type | Schema | Required Fields |
| --- | --- | --- |
| `application/json` | `object` | `choiceArguments` |

**Request Example: `application/json`**

```json
{
  "choiceArguments": {}
}
```

**Responses**

| Code | Description | Content Types | Schemas |
| --- | --- | --- | --- |
| `200` | ok | `application/json` | `application/json:object` |
| `400` | bad request | `application/json` | `application/json:object` |
| `404` | not found | `application/json` | `application/json:object` |

## Changed Entities

No entity-level lifecycle changes in the selected version range.

## Latest Operations

| Operation | Introduced | Changed In | Removed |
| --- | --- | --- | --- |
| `POST /registry/allocation-instruction/v1/allocation-factory` | `0.5.10` | - | - |

## Latest Components

| Component | Introduced | Changed In | Removed |
| --- | --- | --- | --- |
| `responses.400` | `0.5.10` | - | - |
| `responses.404` | `0.5.10` | - | - |
| `schemas.ChoiceContext` | `0.5.10` | - | - |
| `schemas.DisclosedContract` | `0.5.10` | - | - |
| `schemas.ErrorResponse` | `0.5.10` | - | - |
| `schemas.FactoryWithChoiceContext` | `0.5.10` | - | - |
| `schemas.GetFactoryRequest` | `0.5.10` | - | - |

## Spec Metadata

- Canonical spec id: `allocation-instruction-v1.yaml`
- Latest source path: `openapi/allocation-instruction-v1.yaml`
- OpenAPI version (latest): `3.0.0`
- Introduced: `0.5.10`
- Changed in versions: -
- Removed: -

## Entity Summary

- Total entities tracked: `9`
- Entities introduced after spec introduction: `0`
- Entities changed at least once: `0`
- Entities removed: `0`
- Latest operations: `1`
- Latest paths: `1`
- Latest components: `7`
- Latest tags: `0`
