...
PQS runs as a sidecar service alongside a validator's participant node. It connects to the participant's transaction stream and maintains PostgreSQL tables that reflect the current ledger state.

```mermaid
flowchart LR
    PART[Participant Node] -->|Transaction stream| PQS[PQS Service]
