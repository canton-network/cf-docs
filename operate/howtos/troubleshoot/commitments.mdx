{/* source: canton / canton/3.4/participant/howtos/troubleshoot/commitments.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="troubleshoot-commitments"></a>
...
More information on commitments is available in the Pruning overview section.
...
[Error codes reference section](../../reference/error_codes.mdx#error-codes):
...
* `ACS_COMMITMENT_MISMATCH`: This warning indicates a fork in the common ACS state of the participant and one or more
  counter-participants. Please consult the [runbook for inspecting commitment mismatches](#troubleshooting-forks).
...
* `ACS_COMMITMENT_DEGRADATION`: This warning indicates that the participant node is behind some of its counter-participants
  in commitment computation, perhaps due to heavy load, and enters catch-up mode.
...
* The operator suspects that agreement is slower than it could or should be, e.g., because [commitment monitoring](../observe/commitments.mdx#observe-commitments)
...
Assume that the operator observes via [commitment monitoring](../observe/commitments.mdx#observe-commitments) that the participant’s own
...
[admin console](../../reference/console.mdx#canton-console-reference) (or via gRPC) to understand
...
<a id="lookup-sent-acs-commitments"></a>

As another example, assume that the operator observes an `ACS_COMMITMENT_MISMATCH` in the logs. The operator can use
`commitments.lookup_sent_acs_commitments` in the [admin console](../../reference/console.mdx#canton-console-reference) (or via gRPC) as a first
...
<Note>Disabling waiting for commitments disregards these counter-participants w.r.t. non-repudiation when the participant
is pruned, but increases pruning resilience to failures and slowdowns of those counter-participants and/or the network.</Note>
...
<Note>A participant added to the no-wait will not affect the *Default* or *Distinguished*
[counter-participant latency monitoring](../observe/commitments.mdx#observe-commitments).</Note>

## Handle slow commitment computation

Catch-up mode is a behavior that automatically trigger when a participants detects
...
<Note>Troubleshooting a mismatch requires that the two participant operators collaborate in order to identify what caused the
mismatch. Mismatches can happen due to misconfiguration, e.g., when changing contract stores via the repair service, bugs, or
malicious behavior. The non-repudiation section provides more detail on forks.
...
gives the type of mismatch for each contract that only one participant believes it shares with the other participant.</Note>
...
1. **Inspect the local shared contract metadata**
   ...
   this set by first obtaining the commitment it sent using [lookup_sent_acs_commitments](#lookup-sent-acs-commitments), and then opening
   the commitment using the command `commitments.open_commitment` in the [admin console](../../reference/console.mdx#canton-console-reference)
   ...
   <Note>The files used for commitment mismatch inspection use a `.json` format; for some files, each line is a JSON object, rather than
   the file as a whole.
   We recommend, nonetheless, to use the provided tooling for reading / writing these files, as the contents of the files might evolve.</Note>
