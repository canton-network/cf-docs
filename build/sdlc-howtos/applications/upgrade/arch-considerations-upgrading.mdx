...
* A high-level overview of the recommended approach for upgrading a Canton Network application
* Key upgrading challenges
* Methods to ensure backward compatibility
* Strategies for testing and rollback

It also serves as a cross-reference to the *Upgrading Daml Applications* lesson in the [Technical Solution Architect certification path](https://daml.talentlms.com/plus/catalog/courses/161).

{/* todo: consider adding back once we have better upgrading docs: For a walkthrough of the upgrade process with a specific workflow example, refer to :ref:`Upgrading Daml Applications <upgrade-overview>`. */}
...
1. Asynchronous Rollout:
   ...
   3. The app provider and users asynchronously roll out the upgraded backends and frontends and audit the upgraded DAR packages.
...
1. Synchronous Switch-Over:
   1. The app provider publishes a target date for app users to complete the upgrade and decommission v1.
   2. Shortly before the target upgrade date, the app provider and users coordinate to deploy the v2 DAR files to their Canton participant nodes.
...
The recommended approach is designed to address several key challenges that arise due to the distributed nature of Canton Network applications and their deployment across organizational boundaries.
...
* Mixed-Version Deployments
  ...
  * Requirement: Ensure backward compatibility between the frontends/backends and the DAR workflows from the previous version, after following the [recommended approach](#recommended-approach).
* Zero-Downtime Upgrades
  * Problem: Certain workflows may need to progress 24/7 without interruption.
  * Requirement: Ensure zero-downtime upgrades by following the [recommended approach](#recommended-approach) along with other additional measures. One such measure is [Smart Contract Upgrade (SCU)](https://docs.daml.com/upgrade/smart-contract-upgrades.html#what-is-smart-contract-upgrade-scu), which is introduced in the following [Backward Compatibility](#backward-compatibility) section. These practices allow workflows to continue uninterrupted during the upgrade process.
...
Starting with [Daml version 2.9](https://blog.digitalasset.com/developers/release-notes/2.9.1#:~:text=Smart%20Contract%20Upgrading%20(Beta)), [Smart Contract Upgrade (SCU)](https://docs.daml.com/upgrade/smart-contract-upgrades.html#what-is-smart-contract-upgrade-scu) supports several backward-compatible changes to facilitate zero-downtime cross-version interactions:
...
![v1 Contract Created Before the New Field Existed](sdlc-howtos/applications/upgrade/images/optional-1-1.png)
...
![v1 Contract Created Before the New Field Existed](sdlc-howtos/applications/upgrade/images/optional-1-2.png)
...
![v2 Contract Created with a New Optional Field](sdlc-howtos/applications/upgrade/images/optional-2-1.png)
...
![v2 Contract Created with a New Optional Field](sdlc-howtos/applications/upgrade/images/optional-2-2.png)

This failure prevents data loss in workflows like archive-and-recreate:

![v2 Contract Created with a New Optional Field](sdlc-howtos/applications/upgrade/images/optional-2-3.png)
...
![Avoid Package Name Conflicts](sdlc-howtos/applications/upgrade/images/package-name.png)
...
To successfully upgrade Canton Network applications, it is crucial to ensure data continuity, minimize downtime, and maintain compatibility across distributed deployments. Follow the best practices to mitigate challenges and employ strategic approaches to support backward compatibility, testing, and rollback.
