...
Every [topology namespace](topology.md#topology-namespaces) has a public signing key, called the namespace root key.
...
A namespace can have [further signing keys](topology.md#topology-delegation) and operators can use them as well to authorize topology transactions for the namespace.

Examples of topology transactions that a namespace key (root or not) can authorize include:

- [NamespaceDelegation](topology.md#topology-delegation): authorize further keys to authorize topology transactions related to the namespace,
- [OwnerToKeyMapping](topology.md#topology-cryptographic-keys): associate a key with a Canton node,
- [PartyToParticipant](topology.md#topology-parties): associate a party with a participant.
...
Alternatively, the submitting party (referenced as [external party](external-party.md#overview-canton-external-parties)) may
