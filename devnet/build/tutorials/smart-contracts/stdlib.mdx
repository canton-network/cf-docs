...
In [Data types](data.md) and [Functional programming 101](functional-101.md) you learned how to define your own data types and functions. However, you don’t have to implement everything from scratch. Daml comes with the [Daml standard library](../../reference/daml/stdlib/index.md#stdlib-reference-base), which contains types, functions, and typeclasses that cover a large range of use cases.
...
To go in depth on some of these topics, the literature referenced in [The Haskell connection](functional-101.md#haskell-connection) covers them in much greater detail. The standard library typeclasses like `Applicative`, `Foldable`, `Traversable`, `Action` (called `Monad` in Haskell), and many more, are the bread and butter of Haskell programmers.
...
You’ve already used a lot of functions, types, and typeclasses without importing anything. Functions like `create`, `exercise`, and `(==)`, types like `[]`, `(,)`, `Optional`, and typeclasses like `Eq`, `Show`, and `Ord`. These all come from the [Prelude](../../reference/daml/stdlib/Prelude.md#module-prelude-72703). The Prelude is module that gets implicitly imported into every other Daml module and contains both Daml specific machinery as well as the essentials needed to work with the inbuilt types and typeclasses.

## Important types from the Prelude

In addition to the [Native types](data.md#native-types), the Prelude defines a number of common types:
...
consisting of several pieces or passing around data in folds, as you saw in [Folds](functional-101.md#folds).
An example of a relatively wide Tuple can be found in the test modules of the [Handle exceptions (Deprecated)](exceptions.md) project.
...
You’ve seen typeclasses in use all the way from [Data types](data.md). It’s now time to look under the hood.
...
The `Eq` typeclass allows values of a type to be compared for (in)-equality. It makes available two function: `==` and `/=`. Most data types from the standard library have an instance of `Eq`. As you already learned in [Data types](data.md), you can let the compiler automatically derive instances of `Eq` for you using the `deriving` keyword.
...
[Functors](../../reference/daml/stdlib/Prelude.md#class-ghc-base-functor-31205) are the closest thing to “containers” that Daml has. Whenever you see a type with a single type parameter, you are probably looking at a `Functor`: `[a]`, `Optional a`, `Either Text a`, `Update a`. Functors are things that can be mapped over and as such, the key function of `Functor` is `fmap`, which does generically what the `map` function does for lists.

Other classic examples of Functors are Sets, Maps, Trees, etc.

### Applicative Functor

[Applicative Functors](../../reference/daml/stdlib/Prelude.md#class-da-internal-prelude-applicative-9257) are a bit like Actions, which you met in [Add constraints to a contract](constraints.md), except that you can’t use the result of one action as the input to another action. The only important Applicative Functor that isn’t an action in Daml is the `Commands` type submitted in a `submit` block in Daml Script. That’s why in order to use `do` notation in Daml Script, you have to enable the `ApplicativeDo` language extension.

### Action

[Actions](../../reference/daml/stdlib/Prelude.md#class-da-internal-prelude-action-68790) were already covered in [Add constraints to a contract](constraints.md). One way to think of them is as “recipes” for a value, which need to be “executed to get at that value. Actions are always Functors (and Applicative Functors). The intuition for that is simply that `fmap f x` is the recipe in `x` with the extra instruction to apply the pure function `f` to the result.

The really important Actions in Daml are `Update` and `Script`, but there are many others, like `[]`, `Optional`, and `Either a`.

### Semigroup and Monoid

[Semigroups and monoids](../../reference/daml/stdlib/Prelude.md#class-da-internal-prelude-semigroup-78998) are about binary operations, but in practice, their important use is for `Text` and `[]`, where they allow concatenation using the `{<>}` operator.

### Additive and Multiplicative

[Additive and Multiplicative](../../reference/daml/stdlib/Prelude.md#class-ghc-num-additive-25881) abstract out arithmetic operations, so that `(+)`, `(-)`, `(*)`, and some other functions can be used uniformly between `Decimal` and `Int`.
...
- [DA.List](../../reference/daml/stdlib/DA-List.md#module-da-list-85985) for Lists
- [DA.Optional](../../reference/daml/stdlib/DA-Optional.md#module-da-optional-38505) for `Optional`
- [DA.Tuple](../../reference/daml/stdlib/DA-Tuple.md#module-da-tuple-81988) for Tuples
- [DA.Either](../../reference/daml/stdlib/DA-Either.md#module-da-either-91022) for `Either`
- [DA.Functor](../../reference/daml/stdlib/DA-Functor.md#module-da-functor-63823) for Functors
- [DA.Action](../../reference/daml/stdlib/DA-Action.md#module-da-action-7169) for Actions
- [DA.Functor](../../reference/daml/stdlib/DA-Functor.md#module-da-functor-63823) and [DA.Semigroup](../../reference/daml/stdlib/DA-Semigroup.md#module-da-semigroup-27147) for Monoids and Semigroups
- [DA.Text](../../reference/daml/stdlib/DA-Text.md#module-da-text-83238) for working with `Text`
- [DA.Time](../../reference/daml/stdlib/DA-Time.md#module-da-time-32716) for working with `Time`
- [DA.Date](../../reference/daml/stdlib/DA-Date.md#module-da-date-80009) for working with `Date`

You get the idea, the names are fairly descriptive.

Other than the typeclasses defined in Prelude, there are two modules generalizing concepts you’ve already learned, which are worth knowing about: `Foldable` and `Traversable`. In [Loops](functional-101.md#loops) you learned all about folds and their Action equivalents. All the examples there were based on lists, but there are many other possible iterators. This is expressed in two additional typeclasses: [DA.Traversable](../../reference/daml/stdlib/DA-Traversable.md#module-da-traversable-75075), and [DA.Foldable](../../reference/daml/stdlib/DA-Foldable.md#module-da-foldable-94882). For more detail on these concepts, please refer to the literature in [The Haskell connection](functional-101.md#haskell-connection), or [https://wiki.haskell.org/Foldable_and_Traversable](https://wiki.haskell.org/Foldable_and_Traversable).

## Search the standard library

Being able to browse the standard library starting from [The Standard Library](../../reference/daml/stdlib/index.md#stdlib-reference-base) is a start, and the module naming helps, but it’s not an efficient process for finding out what a function you’ve encountered does, even less so for finding a function that does a thing you need to do.
...
Let’s try another search. Suppose you didn’t want the first element, but the one at index `n`. Remember that `(!!)` operator from [Functional programming 101](functional-101.md)? There are now two possible signatures we could search for:  `[a] -> Int -> a` and `Int -> [a] -> a`. Try searching for both. You’ll see that the search returns `(!!)` in both cases. You don’t have to worry about the order of arguments.
