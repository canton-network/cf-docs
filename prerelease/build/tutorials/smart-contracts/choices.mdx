...
As mentioned in [Data types](data), within a choice we use `create`
...
In [Basic contracts](contracts) you learned about the high-level structure of a Daml ledger. With choices and the exercise function, you have the next important ingredient to understand the structure of the ledger and transactions.
...
With the power of choices, you can build your first interesting model: issuance of cash IOUs (I owe you). The model presented here is simpler than the one in [Data types](data) as it’s not concerned with the location of the physical cash, but merely with liabilities:
...
In [Add constraints to a contract](constraints), you will learn how to restrict data and transformations further. In that context, you will also learn about time on Daml ledgers, `do` blocks, and `<-` notation within those.
