<!-- source: canton / canton/3.5/participant/howtos/secure/keys/key_management.rst -->
...
There is a separate section on the [management of namespace keys](namespace_key.md#namespace-key-management).
...
#### NOTE
Key rotation should be [interleaved with taking a backup](../../recover/backup-restore.md#backup-restore-private-key-rotation).
...
Refer to the page on [key restrictions](key_restrictions.md#key-restrictions) for further information on `SigningKeyUsage`.
Keys are generated according to specific cryptographic key formats. Please refer to the
following [tables](../../../reference/crypto_schemes.md#canton-supported-key-formats) for more information on the expected key formats in Canton.
...
Canton supports several [cryptographic schemes](../../../reference/crypto_schemes.md#crypto-schemes).
...
in [Session Keys](../../../explanations/security.md#canton-security).
...
[external KMS (Key Management Service) provider](../kms/mode/external_key_storage.md#external-key-storage).
