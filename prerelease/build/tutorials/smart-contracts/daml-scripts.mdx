...
In this section we test the `Token` model from [Basic contracts](contracts) using the [Daml Script](../../reference/glossary.md#daml-script) integration in [Daml Studio](../../component-howtos/smart-contracts/daml-studio.md#script-results). This includes:
...
You’ll learn all about the syntax `Token with owner = alice` in [Data types](data).
...
In [Basic contracts](contracts) you learned that creating a `Token` requires the authority of its owner. In other words, it should not be possible for Alice to create a token for another party, e.g. Bob, or vice versa. A reasonable attempt to test that would be:
...
To `archiveCmd` the token Alice has created, you need the contract id. Retrieve the contract id from the ledger with the `<-` notation. How this works is discussed in [Add constraints to a contract](constraints).
...
In [Build the Daml Archive (.dar) file](archive) you will build a Daml archive, that you can upload to a Canton participant node to interact with its ledger.
