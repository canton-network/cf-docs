...
template AuthorizedAgent
  ...
  where
    ...
    choice ActOnBehalf : ContractId SomeResult
      controller agent
      do -- agent acts with principal's authority
         ...
```
...
- For the gRPC Ledger API, configure call credentials with the token. For the JSON API, use the `Authorization: Bearer <token>` header
...
For LocalNet development, TLS is typically disabled or uses self-signed certificates. Do not carry this configuration into production.
...
- Use Hardware Security Modules (HSM) or cloud Key Management Services (KMS) for private keys
...
If you operate your own validator, the participant node's signing keys are the most critical secrets. Anyone with access to these keys can submit transactions as your parties. Ensure they are stored in an HSM or KMS and that access is restricted to the validator's runtime environment.
...
- Place your validator's participant node in a private network segment
- Expose only the Ledger API port (gRPC or JSON API) to your application servers
- Use firewall rules or security groups to restrict which systems can reach the participant node's Admin API
...
- [Package Management](/docs-main/appdev/modules/m7-package-management) — Securing DAR distribution and deployment
