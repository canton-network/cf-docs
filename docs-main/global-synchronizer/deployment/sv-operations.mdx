...
per network. Once enabled, traffic-based rewards replace the legacy
on-ledger activity marker mechanism permanently.

### Recommended Migration Procedure

To mitigate the risk of switching reward mechanisms on a live network, SVs
should use the dry-run mode before enabling traffic-based minting:

1. **Enable dry-run**: vote to set `RewardConfig.dryRunVersion` to
   `RewardVersion_TrafficBasedAppRewards` while keeping `mintingVersion` at
   `RewardVersion_FeaturedAppMarkers`. This runs the traffic-based
   computation in parallel through all steps except final coupon creation.
...
3. **Switch minting**: once satisfied, vote to set `RewardConfig.mintingVersion`
   to `RewardVersion_TrafficBasedAppRewards`. Optionally disable the dry-run
   by clearing `dryRunVersion`.

### Configuration Fields

| Field | Description |
|-------|-------------|
| `RewardConfig.mintingVersion` | Set to `RewardVersion_TrafficBasedAppRewards` to enable traffic-based rewards for minting. Set to `RewardVersion_FeaturedAppMarkers` for the legacy on-ledger mechanism. |
| `RewardConfig.dryRunVersion` | Optionally set to `RewardVersion_TrafficBasedAppRewards` to run the traffic-based computation in parallel without affecting actual reward minting. This allows SVs to verify the computation before switching `mintingVersion`. |
| `RewardConfig.rewardCouponTimeToLive` | Time-to-live for `Splice.Amulet.RewardCouponV2` contracts (default: 36 hours). |
| `RewardConfig.appRewardCouponThreshold` | Minimum reward amount in USD below which a party does not receive a coupon (default: 0.5 USD). |
| `RewardConfig.batchSize` | Batch size for the Merkle tree used in reward computation (default: 100). |

### Effect of Enabling

Once `mintingVersion` is set to `RewardVersion_TrafficBasedAppRewards`:
