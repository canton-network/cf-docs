{/* source: daml / daml/3.4/overview/explanations/ledger-model/time.rst */}
{/* Copyright (c) 2023 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}
<Note>* Move LAPI parts to BUILD subsite
* Integrate time monotonicity into the ledger model
* WHERE SHOULD THE SKEWs go?</Note>

<a id="time"></a>

# Time on Daml Ledgers

The Daml language contains a function getTime which returns the “current time”.
...
It is a timestamp that defines the value of all getTime calls in the given transaction,
...
2. **Bounded skew**: `rt_TX - skew_min <= lt_TX <= rt_TX + skew_max`,
...
Daml applications should not interpret the value returned by getTime as a precise timestamp.

<a id="assigning-ledger-time"></a>
...
3. Since the set of commands used by a given transaction can depend on the chosen time,
   the above process might need to be repeated until a suitable ledger time is found.
4. If no suitable ledger time is found after 3 iterations, the submission is rejected.
...
5. At this point, the ledger time may lie in the future (e.g., if a large value for `min_ledger_time_rel` was given).
