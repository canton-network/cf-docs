{/* source: daml / daml/3.4/sdk/reference/daml/data-types.rst */}
...
<a id="daml-ref-built-in-types"></a>
...
| `RelTime`   | models differences between time values | `seconds 1`, `seconds (-2)`       | `RelTime` values have microsecond precision with allowed range from -9,223,372,036,854,775,808ms to 9,223,372,036,854,775,807ms<br/>There are no literals for `RelTime`. Instead they are created using one of `days`, `hours`, `minutes`, `seconds`, `milliseconds` and `microseconds`  (to get these functions, import `DA.Time`).                                                                |
...
To sum a list, use a *fold* (because there are no loops in Daml). See [Fold](working-with.mdx#daml-ref-folding) for details.

{/* TODO - reference std lib documentation */}

<a id="daml-ref-record-types"></a>
...
<a id="daml-ref-data-constructors"></a>
...
<Note>In `data Confusing = Int`, the `Int` is a data constructor with no arguments. It has nothing to do with the built-in `Int` type.</Note>
...
<Note>The `with` keyword binds more strongly than function application. So for a function, say `return`, either write `return IntegerCoordinate with first = 1; second = 5` or `return (IntegerCoordinate {first = 1; second = 5})`, where the latter expression is enclosed in parentheses.</Note>

<a id="daml-ref-parameterized-types"></a>
...
You can use the `type` keyword for any type, including [Built-in Types](#daml-ref-built-in-types).
...
To get around this, wrap the values in a [record](#daml-ref-record-types):
...
the [Daml standard library](stdlib/index.mdx#stdlib-reference-base).
...
<Note>An underscore was used in place of a variable name. The reason for this is that [Daml Studio](../../component-howtos/smart-contracts/daml-studio.mdx#daml-studio) produces a warning for all variables that are not being used. This is useful in detecting unused variables. You can suppress the warning by naming the variable with an initial underscore.</Note>
