...
<div class="x2mdx-ref-hero">
  ...
  <p class="x2mdx-ref-summary">Returns the ACS in creation date ascending order, paged, for a given migration id and record time. Unlike /v0/state/acs, every contract is identified by an (optional) update_id (as opposed to the event ID in /v0/state/acs, which was not BFT-safe). The update_id is the ID of the update in which the contract was created, and can be used to correlate with updates returned by /v2/updates.</p>
...
```bash cURL
curl --request POST \
  --url 'https://scan.sv-1.global.canton.network.sync.global/api/scan/v2/state/acs' \
...
const response = await fetch('https://scan.sv-1.global.canton.network.sync.global/api/scan/v2/state/acs', {
  method: 'POST',
  headers: {
...
curl_setopt_array($curl, [
    CURLOPT_URL => 'https://scan.sv-1.global.canton.network.sync.global/api/scan/v2/state/acs',
...
func main() {
  req, _ := http.NewRequest("POST", "https://scan.sv-1.global.canton.network.sync.global/api/scan/v2/state/acs", bytes.NewBufferString(`{
  "migration_id": 123,
  "record_time": "2026-01-01T00:00:00Z",
  "record_time_match": "exact",
  "after": "<string>",
...
var request = HttpRequest.newBuilder()
    .uri(URI.create("https://scan.sv-1.global.canton.network.sync.global/api/scan/v2/state/acs"))
    .header("Content-Type", "application/json")
...
uri = URI('https://scan.sv-1.global.canton.network.sync.global/api/scan/v2/state/acs')
...
Pagination token for the next page of results. For this to be valid, this must be the `next_page_token` from a prior request with identical parameters aside from `after` and `page_size`; the response may be invalid otherwise. This token is opaque and not meant to be edited by users.
