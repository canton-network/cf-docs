...
description: "How to connect a single validator to the Global Synchronizer and dedicated synchronizers simultaneously"
---
...
Canton validators can connect to multiple synchronizers at the same time. This is how the MainNet is extended — your
validator participates in the Global Synchronizer for network-wide interoperability while also connecting to one or more dedicated
synchronizers for specialized workflows with dedicated bandwidth.

## How multi-synchronizer connections work

Each synchronizer connection is independent. Your validator maintains a separate communication channel to each synchronizer's
sequencer, and contracts are assigned to a specific synchronizer. When you create a contract, it is assigned to one synchronizer.
You can later move it to another synchronizer using the unassignment/reassignment protocol.

The validator tracks which synchronizer each contract belongs to and routes transactions accordingly. If a transaction involves
contracts on different synchronizers, Canton reassigns the contracts to a common synchronizer that the transaction occurs on —
though this adds latency compared to single-synchronizer transactions.

## Adding a synchronizer connection

### Via Canton Console

If your validator is already running and connected to the Global Synchronizer, you can add a dedicated synchronizer connection at runtime:
...
<Note>
The `splice-validator` Helm chart's `synchronizer` value only configures a single synchronizer connection at deployment time — there is no Helm-level way to declare additional connections. Add a second or subsequent synchronizer connection to a running validator through the Canton Console or Admin API, as above.
</Note>
...
- **Who needs to see it?** All validators hosting the contract's stakeholders must be connected to the contract's synchronizer.
- **Transaction frequency** — High-frequency bilateral workflows run more efficiently on a dedicated synchronizer where you control capacity with reduced traffic fees.
...
- **Privacy requirements** — Contracts on a dedicated synchronizer are processed exclusively by infrastructure you control. On the Global Synchronizer, the sequencer and mediator are operated by super-validators.
