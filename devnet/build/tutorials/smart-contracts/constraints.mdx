...
To make sense of the latter, you’ll also learn more about the `Update` and `Script` types, and `do` blocks, which will be good preparation for [Compose choices](compose.md), where you will use `do` blocks to compose choices into complex transactions.
...
Suppose, for example, that the `SimpleIou` contract from [A simple cash model](choices.md#simple-iou) should only be able to store positive amounts. You can enforce this using the `ensure` keyword:
...
For example, the simple Iou in [A simple cash model](choices.md#simple-iou) allowed the no-op where the `owner` transfers to themselves. You can prevent that using an `assert` statement, which you have already encountered in the context of scripts.
...
More generally, if you want to learn more about Actions (aka Monads), we recommend a general course on functional programming, and Haskell in particular. See [The Haskell connection](functional-101.md#haskell-connection) for some suggestions.
...
You can now specify a precise data and data-transformation model for Daml ledgers. In [Parties and authority](parties.md), you will learn how to properly involve multiple parties in contracts, how authority works in Daml, and how to build contract models with strong guarantees in contexts with mutually distrusting entities.
