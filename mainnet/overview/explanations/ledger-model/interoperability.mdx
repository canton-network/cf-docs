...
Interoperability may limit the visibility a Participant Node has into a party’s ledger projection, i.e., its [local ledger](local-ledger.md#local-ledger), when the party is hosted on multiple Participant Nodes.
...
* [Local ledgers and causality graphs](local-ledger.md#local-ledger)
...
For example, suppose that the [split paint counteroffer workflow](local-ledger.md#split-counteroffer-ledger) from the causality examples is committed as follows:
...
So Alice merely *witnesses* the archival even though she is an [informee](ledger-privacy.md#def-informee) of the exercise.
...
This section generalizes [causality graphs](local-ledger.md#causality-graph) to the interoperability setting.
...
The [action order](local-ledger.md#def-action-order) generalizes to multi-ledger causality graphs accordingly.
...
The first three conditions mimic the conditions of [causal consistency](local-ledger.md#def-causal-consistency-contract) for ordinary causality graphs.
...
The notions [minimally consistent](local-ledger.md#minimal-consistent-causality-graph) and [reduction](local-ledger.md#def-reduction-causality-graph) therefore generalize from ordinary causality graphs accordingly.
...
* In the Painter’s projections, the ShowIou transaction tx3 is unordered w.r.t. to the CounterOffer acceptance in tx4
  like in the [case of ordinary causality graphs](local-ledger.md#counteroffer-causality-projections).
...
The [ordering guarantees](local-ledger.md#ordering-guarantees) for single Daml ledgers extend accordingly.
