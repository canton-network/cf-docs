<!-- source: canton / canton/3.5/overview/explanations/canton/protocol.rst -->
...
[Multi-synchronizer](multi-synchronizer.md#multiple-synchronizers) support is discussed separately.
...
* Multi-cast
  ...
  * The Sequencers assign a timestamp to each batch of messages (see section about [Sequencers](synchronizers.md#synchronizers-sequencer))
...
* P confirms transaction on behalf of Alice if she is hosted with at least [confirmation permission](topology.md#topology-parties).
* P submits transactions authorized by Alice if she is hosted with at least [submission permission](topology.md#topology-parties).
...
* [Two-phase commit](#protocol-two-phase-commit) to coordinate changes on state (for example, [Daml transactions](#canton-overview-tx-processing)
  to change the state of a contract, or [reassignments](multi-synchronizer.md#reassignment-protocol) to change assignation of a contract).
...
* [Topology state](topology.md#identity-manager-1) is broadcasted to all members of the synchronizer.
...
One use case of point-to-point communication is [ACS commitments](pruning.md#pruning-non-repudiation).
