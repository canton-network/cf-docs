...
If you do not specify a tag, Docker defaults to pulling `:latest`; that tag is used representatively in this documentation. If you need a pinned version, use an immutable image digest (for example, `europe-docker.pkg.dev/da-images/public/docker/darsyncer@sha256:<digest>`) rather than a floating tag.
