{/* source: canton / canton/3.4/participant/tutorials/install.rst */}
...
<a id="tutorial-hardware-resources"></a>
...
You can also use the Daml Enterprise Canton Docker images by following our [Docker instructions](../howtos/install/docker.mdx#install-with-docker).
...
For this purpose, there are some storage [mixin configurations](../howtos/configure/general/conf_file.mdx#configuration-mixin) (`config/storage/`) defined.
...
<Note>Please also consult the more [detailed section on storage configurations](../howtos/configure/storage/storage.mdx#storage-config).</Note>

Canton will manage the database schema for you. You don’t need to create any tables or indexes.

<a id="tutorial-persistence-using-postgres"></a>
...
<a id="tutorial-postgres-helper"></a>
...
the [Postgres performance guide](../howtos/optimize/storage.mdx#postgres-performance-tuning) for further information.
...
associated with the node’s identity. The Ledger API supports [JWT based authentication](../howtos/secure/apis/jwt.mdx#ledger-api-jwt-configuration).
On the Admin API, you can enable mTLS. Please consult the [TLS documentation section](../howtos/secure/apis/tls.mdx#tls-configuration) for
further information.
...
various command line options available, for example to further tune the [logging configuration](../howtos/observe/monitoring.mdx#logging).
...
the name used in the configuration file. Please consult the [identity management section](../howtos/operate/identity_management.mdx#identity-management-user-manual)
...
2. All nodes are managed through the administration API. Whenever you use the console, almost all requests will
   ...
   the [TLS documentation section](../howtos/secure/apis/tls.mdx#tls-configuration).
3. Applications and users interact with the Participant Node using the gRPC Ledger API. We recommend that you secure your
   API by using TLS. You should also [authorize your clients using JWT tokens](../howtos/secure/apis/jwt.mdx#ledger-api-jwt-configuration).
...
* [Static configuration](../howtos/configure/general/conf_file.mdx#static-configuration) are items which are not supposed to change and are therefore captured in the configuration file.
...
* Dynamic configuration are items such as Daml archives (DARs), synchronizer connections, or parties. All such changes are effected
  through [console commands](../howtos/operate/console/console.mdx#canton-console) (or the [administration APIs](../howtos/operate/console/console.mdx#administration-apis)).
...
The final step is to connect the participant to the synchronizer. Refer to [the connectivity guide](../howtos/operate/synchronizers/connectivity.mdx#synchronizer-connections)
...
2. The administration API should be secured using client certificates as described in [TLS documentation section](../howtos/secure/apis/tls.mdx#tls-configuration).
