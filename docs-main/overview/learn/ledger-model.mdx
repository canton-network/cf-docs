...
| **Observer** | No | Contract + consuming choices | If controller | No |
...
Each party sees a **view** of the transaction: only the parts they're entitled to.
...
Canton uses **ledger time** for time-based contract logic.
...
- [Ledger Model reference](/overview/reference/ledger-model/structure) — the formal model: structure, privacy, integrity, time, causality
