<!-- source: canton / canton/3.5/participant/reference/error_codes.rst -->
...
- use the `RequestInfo.id` as the [correlation-id](../howtos/observe/monitoring.md#tracing), included as `Status.details`.
...
<a id="error-categories-inventory-transientserverfailure"></a>
...
<a id="error-categories-inventory-contentiononsharedresources"></a>
...
<a id="error-categories-inventory-deadlineexceededrequeststateunknown"></a>
...
<a id="error-categories-inventory-systeminternalassumptionviolated"></a>
...
<a id="error-categories-inventory-authinterceptorinvalidauthenticationcredentials"></a>
...
<a id="error-categories-inventory-insufficientpermission"></a>
...
<a id="error-categories-inventory-unredactedsecurityalert"></a>
...
<a id="error-categories-inventory-securityalert"></a>
...
<a id="error-categories-inventory-invalidindependentofsystemstate"></a>
...
<a id="error-categories-inventory-invalidgivencurrentsystemstateother"></a>
...
<a id="error-categories-inventory-invalidgivencurrentsystemstateresourceexists"></a>
...
<a id="error-categories-inventory-invalidgivencurrentsystemstateresourcemissing"></a>
...
<a id="error-categories-inventory-invalidgivencurrentsystemstateseekafterend"></a>
...
> **Description**: The request failed because it resulted in an operation beyond the current upper bound (or ‘end’) defined by the system’s state. For example, supplying a ledger offset which is larger than the current ledger end, or a record time that is in the future.

> **Resolution**: Resolution can occur naturally as the system progresses. The requested operation may become valid eventually once the system’s state has advanced further. For example, when new ledger entries are added. If however the situation does not resolve as expected, operator intervention may be required.

> **Retry strategy**: Wait and retry. For example, retry a limited number of times with potentially increasing backoff. Hint: Inspect the retryable value of the error code to decide on the particular retry duration.

<a id="error-categories-inventory-backgroundprocessdegradationwarning"></a>
...
<a id="error-categories-inventory-internalunsupportedoperation"></a>
...
> **Resolution**: This error is caused by a ledger-level misconfiguration or by an implementation bug. Resolution requires node operator intervention.
...
<a id="manual-logical-synchronizer-upgrade-error-canton-error-codes"></a>
...
* **Category**: InvalidGivenCurrentSystemStateOther
* **Conveyance**: This error is logged with log-level INFO on the server side and exposed on the API with grpc-status FAILED_PRECONDITION including a detailed error message.
* **Scaladocs**: [MANUAL_LOGICAL_SYNCHRONIZER_UPGRADE_ERROR](../scaladoc/com/digitalasset/canton/participant/admin/repair/RepairServiceError$$SynchronizerUpgradeError$.html)
...
<a id="duplicate-confirmation-request-uuid-canton-error-codes"></a>
...
* **Category**: UnredactedSecurityAlert
* **Conveyance**: This error is logged with log-level INFO on the server side and exposed on the API with grpc-status INVALID_ARGUMENT including a detailed error message.
* **Scaladocs**: [DUPLICATE_CONFIRMATION_REQUEST_UUID](../scaladoc/com/digitalasset/canton/error/MediatorError$$DuplicateConfirmationRequest$.html)
