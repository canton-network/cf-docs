---
title: "Privacy Model Differences"
description: "Understanding how Canton's privacy model differs fundamentally from Ethereum"
---
...
| Aspect | ZK Solutions | Canton |
|--------|--------------|--------|
| **Complexity** | ZK circuits are difficult to write | Standard programming model |
| **Performance** | Proof generation is expensive | No proof generation needed |
| **Expressiveness** | Limited by ZK constraints | Full programming language |
| **Setup** | Often requires trusted setup | No cryptographic trusted setup (though users trust their validator) |
...
## Composite Ledger Per Party

In Canton, each party has their own **composite ledger view**—the collection of all contracts where they're a stakeholder.
...
| Total supply is known | Total supply may not be visible |
...
| Timestamps | Ordering |
| Confirmation results (yes/no) | Finality |
...
| Party identifiers | Not revealed |
| Contract data | Never transmitted |
...
| Ethereum | Canton |
|----------|--------|
| Trust validators not to censor | Trust synchronizer not to censor |
| Validators see everything | Synchronizer sees only encrypted views |
| MEV is possible | MEV is significantly harder due to encryption |
| Front-running is structural | Front-running is much harder (transactions are encrypted) |
