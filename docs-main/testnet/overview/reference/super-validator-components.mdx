...
Every SV includes the full validator stack. For a detailed breakdown, see the [Validator Node Components](/testnet/overview/reference/validator-node-components) reference.
...
For more on how ordering consensus works, see [Ordering Consensus](/testnet/overview/reference/ordering-consensus).
