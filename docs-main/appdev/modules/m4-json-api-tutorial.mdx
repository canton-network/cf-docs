...
- **Dpm** — Install following [these instructions](/sdks-tools/cli-tools/dpm#installation)
- `canton` — Canton release with installation and examples, check Canton installation for details
...
### Next steps
#### Canton examples
For a more advanced scenario involving two parties, explore the examples provided with Canton installation: `<canton_installation>/examples/09-json-api`

A [TypeScript version](#get-started-with-canton-the-json-ledger-api-typescript-and-websockets) is also available that demonstrates how to create a JSON Ledger API client for use in a web browser.

#### OpenAPI and AsyncAPI
Read more about the [OpenAPI](/reference/json-api-reference/overview) and [AsyncAPI](/reference/json-api-asyncapi-reference/index) specifications for the Canton JSON Ledger API.

#### Authentication and security
Read how to configure and use jwt token to access JSON Ledger API: [Authentication](/sdks-tools/api-reference/json-api#authentication).

#### Error codes
For more information about error codes returned by the JSON Ledger API, see [Errors](/sdks-tools/api-reference/json-api#errors).
...
- **Canton** — Includes pre-built examples. See the Canton installation for details.

#### Example TypeScript Project
Open a terminal and navigate to the JSON Ledger API example folder:

``` 
cd <canton_installation>/examples/09-json-api
```
...
``` 
cd <canton_installation>/examples/09-json-api/typescript
```
...
1.  Open the `Iou.daml` file in `<canton_installation>/examples/09-json-api/model`.
...
{/* COPIED_START source="docs-website:docs/replicated/canton/3.5/sdk/tutorials/json-api/canton_and_the_json_ledger_api_ts_websocket.rst" hash="e0ce792c" */}
...
### Overview
You will use and modify an existing example project provided with the Canton distribution. The example project, the setup steps and the code highlights are the same as in the TypeScript tutorial above; this section covers the WebSocket variant.
...
The WebSocket connection establishment logic is in `src/websocket_client.ts`:
...
### Extending the Example
The steps to extend the `Iou` template are the same as in the TypeScript tutorial above; for the WebSocket scenario, the `showAcs` call to modify is in `src/index_websocket.ts`. After rebuilding, run the WebSocket example:
