...
description: "Complete five-phase lifecycle of a Canton transaction — from preparation through sequencing, validation, and final commit"
---
...
3. **Sequencing and Distribution** — the sequencer timestamps and delivers messages to recipients
4. **Validation and Confirmation** — each confirming participant validates its views and responds to the mediator
5. **Aggregation and Commit** — the mediator collects responses, renders a verdict, and participants apply (or discard) the result

```mermaid
sequenceDiagram
    participant App as Application
    participant SP as Submitting<br>Participant
    participant SEQ as Sequencer
    participant MED as Mediator
    participant CP as Confirming<br>Participant(s)

    rect rgb(240, 240, 255)
    note over App,SP: Phase 1 — Preparation
    App->>SP: Command via Ledger API (gRPC)
    SP->>SP: Execute Daml engine locally
    SP->>SP: Produce transaction tree, decompose into views
    SP->>SP: Encrypt each view for its informees
    SP->>SP: Compute root hash over full tree
    end

    rect rgb(240, 255, 240)
    note over SP,SEQ: Phase 2 — Submission
    SP->>SEQ: Confirmation request batch<br>(encrypted views + root hash messages + informee messages)
    end

    rect rgb(255, 245, 230)
    note over SEQ,CP: Phase 3 — Sequencing & Distribution
    SEQ->>SEQ: Assign timestamp (BFT-write)
    SEQ->>CP: Encrypted views + root hash message<br>(only views where CP hosts an informee)
    SEQ->>SP: Encrypted views + root hash message<br>(for submitter's own informees)
    SEQ->>MED: Informee messages<br>(informee list + confirmation policy per view)
    MED->>MED: Record request metadata, start deadline timer
    end

    rect rgb(255, 240, 240)
    note over CP,MED: Phase 4 — Validation & Confirmation
    CP->>CP: Decrypt views, validate (conformance, authorization, consistency, time bounds, root hash)
    SP->>SP: Same validation on submitter's own views
    CP->>SEQ: Mediator response (approve/reject)
    SP->>SEQ: Mediator response (approve/reject)
    SEQ->>SEQ: Timestamp response (BFT-write)
    SEQ->>MED: Deliver mediator responses
    end

    rect rgb(245, 240, 255)
    note over MED,CP: Phase 5 — Aggregation & Commit
    MED->>MED: Collect responses, evaluate confirmation policy
    MED->>SEQ: Transaction result (approve or reject)
    SEQ->>SP: Deliver verdict
    SEQ->>CP: Deliver verdict
    SP->>SP: Commit or discard
    CP->>CP: Commit or discard
    end
```
...
- **Determines informees** for each action node in the tree. Informees are the parties who are entitled to see a given action — signatories, observers, and actors, as defined by the Daml template. The informee set for each action governs who receives which parts of the transaction.
...
If the transaction touches contracts on multiple synchronizers, the participant may need to reassign those contracts to a common synchronizer before submission. This is handled by the [Reassignment Protocol](/docs-main/overview/reference/reassignment-protocol).
...
- **Conformance** — the participant re-executes the Daml contract logic for each action in the view it received. The re-execution must produce the same consequences (created contracts, archived contracts) as claimed in the view. If the results diverge, the view is malformed or the submitter is dishonest.
...
- **Preparation failure**: the Daml engine rejects the command (e.g., contract not found, authorization failure, assertion violation). The command is rejected synchronously before any messages are sent. No other nodes are involved.
