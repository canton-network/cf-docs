...
The validator tracks which synchronizer each contract belongs to and routes transactions accordingly. If a transaction involves contracts on different synchronizers, Canton coordinates the transaction across both — though this adds latency compared to single-synchronizer transactions.
