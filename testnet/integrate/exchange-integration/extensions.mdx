...
beyond the MVP described in the [Overview](overview.md#exchange-integration-overview) section:

## Optimizing App Rewards

The MVP for all CN tokens described in the [Overview](overview.md#exchange-integration-overview) section
...
3. Change the [Ledger API user setup](node-operations.md#setup-ledger-api-users)
   such that the
   1. the user used by Withdrawal Automation also has the `readAs(exchangeParty)` right
   2. the user that performs the [exchange parties setup](node-operations.md#exchange-parties-setup)
      also has the `canActAs(exchangeParty)` right.
4. Add a step to the [treasury party setup](node-operations.md#treasury-party-setup) to also create a `DelegateProxy` contract with
...
The Tx History Ingestion as [described here](workflows.md#one-step-transfer-parsing) does not need changing,
...
1. Setup multiple treasury parties instead of using a single `treasuryParty`.
   Use the setup described in the [Setup the treasury party](node-operations.md#treasury-party-setup) section for each of them.
...
The [documentation on setting up the exchange party](node-operations.md#treasury-party-setup) describes how to setup a party with a single
...
As part of the [initial treasury party setup](../party-management/index.md#create-an-external-party), you generate the `PartyToParticipant`
...
command deduplication at all in favor of [UTXO and max record time based deuplication](non-functionals.md#withdrawal-automation).
