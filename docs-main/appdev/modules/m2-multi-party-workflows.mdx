...
|--------|----------|--------|
| **Multi-sig creation** | Deploy contract, collect signatures over time | All signatories must authorize at creation |
...
The Canton version:
- Cannot be manipulated (protocol enforces)
- Cannot be in inconsistent state
- Automatically private to involved parties
...
1. **Multi-party is first-class**: Canton builds multi-party agreement into the protocol, not as a pattern.

2. **Propose-accept is standard**: Since all signatories must authorize, use proposal contracts.

3. **Atomicity is guaranteed**: Multi-contract operations are all-or-nothing.

4. **Privacy is automatic**: Each party sees only their portion of the workflow.

5. **Delegation is explicit**: Authority transfers are controlled and auditable.
