...
Released versions of darsyncer are browseable through the Google Artifact Registry UI (accessible to all, but does require you to login to Google).
...
If you do not specify a version, `latest` will be returned by default, and that version is used representatively in this documentation.  As with most docker images, if you want to ensure pinned versions it is recommended you resolve to a specific sha and use that within your application, if you do not wish to pull the latest rolling version.

Generally speaking, versions of darsyncer are compatible across participant / splice versions, and it is advantageous to stay current to benefit from any security vulnerability fixes and patches.
