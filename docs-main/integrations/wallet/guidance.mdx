...
{/* DIVERGENCE: updated locally to reflect CIP-0096 (validator liveness rewards discontinued, $0 cap effective 2026-04-30); upstream splice-wallet-kernel source (docs/wallet-integration-guide/src/tokenomics-and-rewards/index.rst, hash 08ff71a4) still describes liveness rewards as an active mechanism. See cf-docs issue #1049. */}

### Validator & Super Validator Rewards

Validators earn CC through activity rewards: burning CC for traffic purchases or transfers generates reward coupons that can be minted. [CIP-0096](https://github.com/canton-foundation/cips/blob/main/cip-0096/cip-0096.md) discontinued the earlier liveness rewards that paid validators just for being online, capping them at $0 effective 2026-04-30. Super Validators separately generate reward coupons for operating synchronizer infrastructure. The coupons are paid out to the validator adminstration party. For local parties onboarded to a validator, the validator application runs background automation to mint all activity records automatically. An external party signs transactions using a key they control. As a consequence, the validator automation is not able to perform minting for external parties.
