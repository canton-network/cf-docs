...
export const NetworkVariables = ({
  ...
  const replaceTextNode = (value, substitutions, keyPrefix = 'token') => {
    ...
    while ((match = tokenPattern.exec(value)) !== null) {
      ...
      if (replacement && typeof replacement === 'object' && replacement.href) {
        ...
        const networkLabel = activeData?.versions?.splice
          ? `${activeData.name ?? activeNetwork} ${activeData.versions.splice}`
          : activeData?.name ?? activeNetwork;
        pieces.push(
  ...
  React.useEffect(() => {
    ...
    const replaceTokenText = (value) =>
      value.replace(/\|([A-Za-z0-9_]+)\|/g, (match, token) =>
        replacementForToken(token, activeData.substitutions) ?? match
      );
    ...
    replaceRenderedCode();
    observer.observe(panelRef.current, { childList: true, characterData: true, subtree: true });
  ...
  return (
    <div data-component="network-variables" className="my-6 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-950">
      ...
      <div
        ...
        ref={panelRef}
        role="tabpanel"
        className="space-y-4 px-4 py-4 text-sm leading-6 text-gray-800 dark:text-gray-200"
      >
