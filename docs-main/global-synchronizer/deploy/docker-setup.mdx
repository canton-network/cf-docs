...
description: "Deploy a Canton Network validator node using Docker Compose"
---
...
- Production validators, with the following caveats:
  - The default deployment is highly insecure. Authentication should be enabled as described in the [authentication section](#authentication) below.
...
{/* TODO: link to section that explains what this onboarding means */}
...
Please refer to the [Kubernetes authentication section](/docs-main/global-synchronizer/deploy/k8s-setup#authentication) for instructions on how to set up an OAuth provider for your validator. The URLs to configure for callbacks are `http://wallet.localhost` and `http://ans.localhost`.
