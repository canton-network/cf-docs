...
| `daml ledger allocate-parties` | [JSON API or gRPC](#daml-ledger-allocate-parties) | Allocate parties on a ledger |
| `daml ledger list-parties` | [JSON API or gRPC](#daml-ledger-list-parties) | List parties on a ledger |
| `daml ledger upload-dar` | [JSON API or gRPC](#daml-ledger-upload-dar) | Upload (and vet) DARs on a ledger |
| `daml ledger fetch-dar` | [JSON API or gRPC](#daml-ledger-fetch-dar) | Fetch a DAR from a ledger |
| `daml packages` | [JSON API or gRPC](#daml-packages) | List packages on a ledger |
| `daml start` | [`dpm sandbox` + `dpm start`](#daml-start) | Start a local Daml Ledger |
...
Replaced with `dpm sandbox` combined with `dpm build`. Use the Declarative API or JSON/gRPC API to upload DARs and allocate parties as needed.
