...
| Change of Signatories | Template Definition | No | A new template version needs to return the same signatory result for a persisted contract, otherwise the contracts cannot be used by an upgraded package (this fails at runtime).<br/><br/>One can change the function that computes the signatories (for example to take into account new fields) when creating contracts. |
| Change of | Template | No | The key's type cannot be changed, like adding a new field in the record it contains.<br/><br/>One can change the function that computes the key in the same way the function computing the signatories can be changed. A persisted contract can be used by an upgrade only if the result of the function does not change. |
