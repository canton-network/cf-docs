{/* source: canton / canton/3.4/overview/explanations/canton/topology.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="identity-manager-1"></a>

# Topology management

<Note>* Replace with [https://docs.google.com/document/d/1z2yNEUdVQ7bt8wDjPWrXaEh6XnkNvta7uumrQdnq33Q/edit?tab=t.0#heading=h.j1o9vy5fqmrz](https://docs.google.com/document/d/1z2yNEUdVQ7bt8wDjPWrXaEh6XnkNvta7uumrQdnq33Q/edit?tab=t.0#heading=h.j1o9vy5fqmrz)
...
* Decentralization is out of scope here and will be dealt with in a separate section</Note>
...
controlling key(see [Decentralization](decentralization.mdx#decentralization)).
...
additional keys(see [Authorization chains](#topology-authorization-chains)).

<a id="topology-unique-identifiers"></a>
...
[Validity / effectiveness](#topology-effectiveness)).
...
<a id="topology-delegation"></a>

### Namespace definition and delegation

As outlined in the section [Namespaces](#topology-namespaces), a namespace is defined by
...
[Authorization chains](#topology-authorization-chains) and [Key revocation](#topology-key-revocation) for more
...
<a id="topology-cryptographic-keys"></a>
...
parties (see [Local and external parties](external-party.mdx#overview-canton-external-parties)). Canton key holders use the keys listed in
these topology mappings for various use cases across the layers of Canton’s
communication protocols (see [Protocols on One Synchronizer](protocol.mdx#protocols)) except for topology management.
...
Participant Node is expected to participate in the [two-phase
commit](protocol.mdx#protocol-two-phase-commit) protocol for the party’s transactions and
...
<a id="topology-participant-permission"></a>
...
* **Confirmation**: the Participant Node confirms or rejects Daml Transactions
  as part of the [two-phase commit](protocol.mdx#protocol-two-phase-commit) protocol.
...
Nodes (see [Decentralized parties](decentralization.mdx#decentralized-parties)).

<a id="topology-package-vetting"></a>
...
* **Sequencer**: a Sequencer is considered a member of a Synchronizer, if the
  ...
  refer to dynamically_adding_sequencers.
* **Mediator**: the membership of Mediators works similar to the membership of
  ...
  to multiple Mediator groups (see optimize-mediator). A Mediator can
  only be in single Mediator group. For more information on how to onboard a
  Mediator, please refer to dynamically_adding_mediators.
...
<a id="topology-proposals-example"></a>
...
{/* https://lucid.app/lucidchart/03fbc838-4153-4442-8e6f-fc91ee9313a4/ */}![image](explanations/canton/images/overview/topology-simple-proposal.svg)
...
{/* https://lucid.app/lucidchart/03fbc838-4153-4442-8e6f-fc91ee9313a4/ */}![image](explanations/canton/images/overview/topology-competing-proposals.svg)

<a id="topology-effectiveness"></a>
...
[Sequential validation](#topology-sequential-validation) and [Future dating](#topology-future-dating).
...
<a id="topology-sequential-validation"></a>
...
The various layers of Canton’s protocol (see [Protocols on One Synchronizer](protocol.mdx#protocols)) rely on topology
...
time*  was already introduced in the section [Validity / effectiveness](#topology-effectiveness).
...
effective time of the topology transaction at t<sub>3</sub> is computed to be t<sub>5</sub>. Therefore, once the processing of the topology transaction
completes, the processing of the Daml Transaction at t<sub>4</sub> can
access the topology snapshot at t<sub>4</sub>, because the Canton node knows, that it
has seen all changes to the topology state that are relevant for a snapshot up
to t<sub>5</sub>.

{/* https://lucid.app/lucidchart/03fbc838-4153-4442-8e6f-fc91ee9313a4/edit?page=_4giX7nLVzKD */}![image](explanations/canton/images/overview/topology-change-delay.svg)

<a id="authorization"></a>

## Authorization

The section [Signatures](#topology-signatures) mentions that topology mappings define
...
example for evolving topology mappings (see [Example](#topology-proposals-example)).
...
[two-phase commit protocol](protocol.mdx#protocol-two-phase-commit) for Daml
transactions that involve the party.

<a id="topology-authorization-chains"></a>

### Authorization chains

As described in the section [Namespaces](#topology-namespaces) and
[Namespace definition and delegation](#topology-delegation), the root certificate of a namespace serves as the
...
{/* https://lucid.app/lucidchart/03fbc838-4153-4442-8e6f-fc91ee9313a4/edit?page=2oJVpWUfm8eF */}![image](explanations/canton/images/overview/topology-certificate-chain.svg)
...
<a id="topology-key-revocation"></a>
...
Continuing the example from the [preceding section](#topology-authorization-chains), the key holder for `abc123` revokes the
...
{/* https://lucid.app/lucidchart/03fbc838-4153-4442-8e6f-fc91ee9313a4/edit?page=3O-ioZ3C0FPK */}![image](explanations/canton/images/overview/topology-broken-certificate-chain.svg)
