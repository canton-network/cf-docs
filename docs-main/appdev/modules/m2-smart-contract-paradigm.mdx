...
In Daml, contracts are immutable data. State changes create new contracts:
...
```mermaid
flowchart TB
    subgraph Solidity[Solidity State Change]
        S1[Contract State: Alice=100]
        S1 --> |transfer 30 to Bob| S2[Contract State: Alice=70, Bob=30]
        S2 --> |same contract, mutated|S2
    end
```

```mermaid
flowchart TB
    subgraph Daml[Daml State Change]
        D1[Contract: Alice owns 100]
        D1 --> |Transfer choice| D2[ARCHIVED]
        D1 --> |creates| D3[Contract: Alice owns 70]
        D1 --> |creates| D4[Contract: Bob owns 30]
    end
```
...
|---------|----------|------|
| **Conditionals** | if/else, ternary | if/then/else, guards |
| **Loops** | for, while | Recursion, map/fold |
| **Error handling** | require/revert | assert, exceptions |
| **Early return** | return | Pattern matching |
...
| Try/catch everywhere | Exceptions for genuine errors only |

## Next Steps

<CardGroup cols={2}>

<Card title="Network Architecture" icon="network-wired" href="/docs-main/appdev/modules/m2-network-architecture">
  Compare network architecture and topology.
</Card>

<Card title="Module 3: Daml" icon="code" href="/docs-main/appdev/modules/m3-daml">
