...
3. **Switch minting**: once satisfied, vote to set `rewardConfigMintingVersion`
...
| Field | Description |
|-------|-------------|
| `rewardConfigMintingVersion` | Set to `RewardVersion_TrafficBasedAppRewards` to enable traffic-based rewards for minting. Set to `RewardVersion_FeaturedAppMarkers` for the legacy on-ledger mechanism. |
| `rewardConfigDryRunVersion` | Optionally set to `RewardVersion_TrafficBasedAppRewards` to run the traffic-based computation in parallel without affecting actual reward minting. This allows SVs to verify the computation before switching `rewardConfigMintingVersion`. |
| `rewardConfigRewardCouponTimeToLive` | Time-to-live for `Splice.Amulet.RewardCouponV2` contracts (default: 36 hours). |
| `rewardConfigAppRewardCouponThreshold` | Minimum reward amount in USD below which a party does not receive a coupon (default: 0.5 USD). |
| `rewardConfigBatchSize` | Batch size for the Merkle tree used in reward computation (default: 100). |
