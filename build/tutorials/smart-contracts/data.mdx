{/* source: daml / daml/3.4/sdk/tutorials/smart-contracts/data.rst */}
{/* Copyright (c) 2023 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

# Data types

In [Basic contracts](contracts.mdx), you learnt about contract templates, which specify the types of contracts that can be created on the ledger, and what data those contracts hold in their arguments.

In [Test templates using Daml scripts](daml-scripts.mdx), you learnt about the script view in Daml Studio, which displays the current ledger state. It shows one table per template, with one row per contract of that type and one column per field in the arguments.
...
<Tip>Remember that you can load all the code for this section into a folder called `intro-data` by running `dpm new intro-data  --template daml-intro-data`</Tip>

<a id="native-types"></a>

## Native types

You have already encountered a few native Daml types: `Party` in [Basic contracts](contracts.mdx), and `Text` and `ContractId` in [Test templates using Daml scripts](daml-scripts.mdx). Here are those native types and more:
...
In [Transform contracts using choices](choices.mdx) you’ll learn how to define data transformations and give other parties the right to manipulate data in restricted ways.
