...
<Note>
On MainNet and TestNet, the active CLI for Daml development is the daml assistant. `dpm` (Daml Package Manager) is the DevNet path; it ships with Daml SDK 3.4.x. If your project targets MainNet or TestNet (Daml SDK 3.3.x snapshots), use the daml assistant instead.
</Note>
...
- [Daml SDK](/mainnet/sdks-tools/sdks/daml-sdk) -- What the SDK includes and how versions work
- [Sandbox](/mainnet/sdks-tools/development-tools/sandbox) -- Local testing environment started by `dpm sandbox`
- [Daml Studio](/mainnet/sdks-tools/development-tools/daml-studio) -- VS Code extension launched by `dpm studio`
- [Daml Script](/mainnet/sdks-tools/cli-tools/daml-script) -- Writing and running tests with `dpm test`
...
The settings under `auth-services` are described in detail in [API configuration documentation](/mainnet/global-synchronizer/reference/security-configuration#jwt-authorization)
