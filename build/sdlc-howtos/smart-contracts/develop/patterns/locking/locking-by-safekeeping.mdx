...
* *LockedCoinV2* represents *Coin* in the locked state. It is fairly similar to the *LockedCoin* described in [Consuming Choice](locking-by-archiving.mdx#lockingbyconsumingchoice).  The additional logic is to transfer ownership from the locker back to the owner when *Unlock* or *Clawback* is called.
