...
For the configuration options that apply only to specific APIs, please refer to the [Administration API](admin_api.mdx#admin-api-configuration),
the [gRPC Ledger API](ledger_api.mdx#ledger-api-configuration), and the [JSON Ledger API](json_api.mdx#json-api-configuration) pages.
...
the connection is terminated with a `GOAWAY` error with “too_many_pings” as the debug data.</Note>
<Note>Setting `permit-keep-alive-without-calls` to `true` allows clients to send ping messages outside of any
ongoing gRPC call. Such a ping otherwise results in a `GOAWAY` error.</Note>
