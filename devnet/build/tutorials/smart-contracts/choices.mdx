...
In the example in [Manipulate contracts](data.md#manipulate-contracts) the accountant party wanted to change some data on a contract. They did so by archiving the contract and re-creating it with the updated data. That works because the accountant is the sole signatory on the `Account` contract defined there.
...
Take as an example a `Contact` contract on which the contact owner wants to be able to change the telephone number, just like on the `Account` in [Manipulate contracts](data.md#manipulate-contracts). Rather than requiring them to manually look up the contract, archive the old one, and create a new one, you can provide them a convenience method on `Contact`:
...
As mentioned in [Data types](data.md), within a choice we use `create`
...
In [Basic contracts](contracts.md) you learned about the high-level structure of a Daml ledger. With choices and the exercise function, you have the next important ingredient to understand the structure of the ledger and transactions.
...
With the power of choices, you can build your first interesting model: issuance of cash IOUs (I owe you). The model presented here is simpler than the one in [Data types](data.md) as it’s not concerned with the location of the physical cash, but merely with liabilities:
...
In [Add constraints to a contract](constraints.md), you will learn how to restrict data and transformations further. In that context, you will also learn about time on Daml ledgers, `do` blocks, and `<-` notation within those.
