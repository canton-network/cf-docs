...
The **synchronizer** (sequencer + mediator nodes) coordinates transaction ordering and confirmation without seeing transaction content.
...
| **Identify users** | Doesn't know which users are involved |
