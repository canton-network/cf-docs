...
def overview_page_ref(output_dir: Path, docs_json_path: Path, overview_name: str) -> str:
    return canton_protobuf_history.docs_json_page_ref(output_dir / overview_name, docs_json_path)


def spec_page_ref(output_dir: Path, docs_json_path: Path, *, spec_dir_name: str, spec_filename: str) -> str:
    return canton_protobuf_history.docs_json_page_ref(
        output_dir / spec_dir_name / f"{slugify(spec_filename)}.mdx",
        docs_json_path,
    )
...
def build_nav_group(
    *,
    ...
    overview_ref = overview_page_ref(output_dir, docs_json_path, overview_name)
    spec_refs = [
        spec_page_ref(
            output_dir,
            docs_json_path,
    ...
    refs = {overview_ref, *spec_refs}
    return {"group": nav_group_label, "pages": [overview_ref, *spec_refs]}, refs
...
def build_command(
    *,
    ...
    spec_filenames = source_config.get("spec_filenames")
    if not isinstance(spec_filenames, list) or not all(isinstance(item, str) and item for item in spec_filenames):
        raise ValueError("spec_filenames must be a list of non-empty strings")
    overview_title = source_config.get("overview_title")
    if not isinstance(overview_title, str) or not overview_title:
        raise ValueError("overview_title must be a non-empty string")
...
def render_reference(
    *,
    ...
    command = build_command(
        ...
        manifest_path=manifest_path,
        output_dir=output_dir,
        overview_name=DEFAULT_OVERVIEW_NAME,
        spec_dir_name=DEFAULT_SPEC_DIR_NAME,
    ...
    for spec_filename in spec_filenames:
        page_path = output_dir / DEFAULT_SPEC_DIR_NAME / f"{slugify(spec_filename)}.mdx"
        if not page_path.exists():
            raise FileNotFoundError(f"Expected generated spec page not found: {page_path}")
