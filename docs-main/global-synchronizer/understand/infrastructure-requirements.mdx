...
<Note>
Storage requirements grow over time based on transaction volume. Plan for growth and consider implementing pruning.
</Note>
...
| **Java** | 17 or 21 | Java 25 is not supported |
...
flowchart TB
    subgraph Primary
        P1[Participant Node<br>Active]
        DB1[(Database<br>Primary)]
    end

    subgraph Secondary
        P2[Participant Node<br>Standby]
...
| **Database replication** | Synchronous for consistency |
| **State management** | Ensure clean handoff |
| **DNS/Load balancing** | Health-aware routing |
...
<Card title="Validator Setup" icon="server" href="/global-synchronizer/deploy/validator-setup">
