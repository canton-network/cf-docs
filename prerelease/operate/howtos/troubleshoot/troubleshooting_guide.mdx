<!-- source: canton / canton/3.5/participant/howtos/troubleshoot/troubleshooting_guide.rst -->
...
* **Create Health Dumps**
  > Ensure that you can create [Health Dumps](../observe/monitoring.md#creating-dumps). You need to share these health dumps during any support request. The health dumps provide a lot of diagnostic information that we need to troubleshoot issues.
* **Turn on Debug Logging**
  ...
  > The `-v` flag will turn on debug logging of all `com.digitalasset.canton` loggers, whereas `--debug` will turn on debug logging of all libraries too. Please also see [Logging](../observe/monitoring.md#logging).
* **Turn on Detailed API Logging**
  > You might want to turn on [detailed API logging](../observe/monitoring.md#detailed-logging). This writes all incoming commands and outgoing data into the log file, allowing you to inspect the details of a command that leads to failures. Beware that if your commands contain sensitive data, this data is written to the log file.
...
* **Canton Error Codes**
  > All non-internal warnings and errors are [logged consistently](../../reference/error_codes.md#machine-readable-information) (or at least we aspire to do).The error code information listed in the documentation should contain all information you need in order to understand and possibly resolve the issue.

## Log Files

* **Canton Trace Ids**
  > All Canton log statements contain a [trace-id](../observe/monitoring.md#tracing). This tracing is turned on by default and the `trace-id` is passed between the distributed processes:
  > ```default
...
* **Setup and Use LNAV**
  > Setup lnav for viewing logs as described in [viewing logs](../observe/monitoring.md#lnav). It will require a few minutes to get used to it, but the payoff of this investment is great and comes fast. **In particular get familiar with loading multiple files, filtering, searching and jumping to errors**.
...
* Are the nodes [connected to a sequencer](../operate/synchronizers/connectivity.md#synchronizer-connections)? Errors that often happen here are:
...
> * overloaded - resolve by tuning according to our [performance configuration guide](../optimize/performance.md#performance-configuration).
> * unable to complete the transaction processing within the given time (i.e. transactions are too big) - resolve by increasing the timeouts as described in our [performance configuration guide](../optimize/performance.md#performance-configuration).
...
approach can be used for other Canton APIs. This is described in the dedicated [keep-alive article](../configure/apis/apis.md#keepalive-configuration).
...
How to obtain a performant system is [extensively documented](../optimize/performance.md#performance-configuration).

If you have followed that documentation, we can assume that:

> * Your database pools are sufficiently sized: check metric `db-storage.queue` and you’ve [correctly setup your database](../optimize/storage.md#postgres-performance-tuning).
...
> * You have [optimized your model](../optimize/performance.md#model-tuning) to reduce the transaction size.
...
> * verify whether you might be affected by [contention on shared resources in a shared environment](../optimize/storage.md#shared-env-performance).
