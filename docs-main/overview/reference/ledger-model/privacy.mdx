---
title: "Privacy"
description: "Who sees which changes and data on a Canton Ledger: informees, witnesses, projection, divulgence, and disclosure."
---
...
The [ledger structure section](/overview/reference/ledger-model/structure) answered the question "What does the Ledger looks like?" by introducing a hierarchical format to record the party interactions as changes.
...
**Definition: The informees of a node are the union of the sets marked with X.**
...
To illustrate the concept of informees, we use the [running example of Alice and Bob swapping their assets](/overview/reference/ledger-model/structure#running-workflow-example).
...
as mentioned in the [ledger structure section](/overview/reference/ledger-model/structure#hierarchical-structure);
...
For example, the diagram below extends the [subaction diagram](/overview/reference/ledger-model/structure#subactions) with the informees shown in the top right corner of the borderless box for each subaction.
...
<a id="da-dvp-acceptandsettle-projection"></a>

{/* https://lucid.app/lucidchart/9b3762db-66b4-4e72-94cb-bcefd4c1a5ea/edit */}

<img src="/images/docs_website/dvp-acceptandsettle-projection.svg" className="align-center" style={{width: "100.0%"}} alt="" />
...
1. Else, if *act* has consequences, replace *act* by the projection (for *P*) of its consequences,
   which might be empty.
1. Else, drop *act* including its consequences.
...
We defer defining the edges in the projection to the [causality section](/overview/reference/ledger-model/causality).
...
<a id="da-dvp-ledger-projections"></a>

{/* https://lucid.app/lucidchart/90b7f155-aadc-4bde-9c5c-b8198b824384/edit */}

<a id="da-ledgers-projections-example"></a>

<img src="/images/docs_website/dvp-ledger-projections.svg" className="align-center" style={{width: "100.0%"}} alt="Time sequences for each party's projection, explained in-depth in the numbered list below." />

Examine each party's projection in turn:

1. Alice sees all of the first, thrid, and forth commit
   ...
   This effect is discussed below under [retroactive divulgence](#da-model-divulgence).

1. Bob's projection is analogous to Alice's:
...
1. Banks 1 and 2 only see the commits in which they create their `SimpleAsset` and the `Transfer` Exercises on them.
...
[updates tree stream](/sdks-tools/api-reference/ledger-api-services#update-service).

</Note>

<a id="da-model-divulgence"></a>
...
- **Immediate divulgence** refers to witnesses seeing contract creations they are not an informee of.
  In the example of [ledger projections of the DvP](#da-dvp-ledger-projections),
...
- **Retroactive divulgence** refers to an input contract being shown to the non-informee witnesses of a node using this contract.
  ...
  (see [Consistency](/overview/reference/ledger-model/integrity#consistency) for ledger integrity).
...
Via the Ledger API's [update service](/sdks-tools/api-reference/ledger-api-services#update-service),
...
Recall from the [running example](/overview/reference/ledger-model/structure#running-workflow-example)
...
For example, the above [transaction projection for Bank 2](#da-dvp-acceptandsettle-projection) omits what happens between the Fetch and the Exercise on contract #2
...
<a id="def-tx-projection-shape"></a>
...
For example, the shape-revealing projection of the [above DvP ledger](#da-dvp-ledger-projections) for Bank 2 looks as follows.
Node shapes are shown as empty boxes with a <em>?</em> inside and annotated with the informees and the required confirmers (underlined).
...
<a id="da-ledgers-projections-example-shape"></a>

<img src="/images/docs_website/dvp-ledger-projection-reveal-shape.svg" className="align-center" style={{width: "100.0%"}} alt="Shape-revealing projection for Bank 2" />
