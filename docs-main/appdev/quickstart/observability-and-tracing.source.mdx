...
<Snippet
  source="https://github.com/digital-asset/cn-quickstart/blob/77e46ee7ddbe8bf1b921111778e33eb399e33ad3/sdk/docs/sharable/sdk/quickstart/observe/observability-troubleshooting-overview.rst"
  lines="164..165"
  normalize="preserve"
  language="none"
/>

For example: if you connect to the `postgres-splice-app-provider` container (default username `cnadmin`, dbname `scribe`, and password `supersafe`; then you can use the SQL interface to PQS to examine the app-provider’s participant’s local ledger. The SQL API to PQS is documented in the [PQS SQL reference](/appdev/reference/pqs-sql-reference).

#### Interactive debugger

The remote-debug configuration for backend-service lives in `docker/backend-service/debug.yaml`, which is only applied to the deployment when `DEBUG_ENABLED=true` is set (for example, in `.env.local`). If you review that file you will see the lines:

<Snippet
  source="https://github.com/digital-asset/cn-quickstart/blob/41f2d75cd16eff28aedfaf2e9a2278a881b1c71a/quickstart/docker/backend-service/debug.yaml"
  normalize="baseline"
  trim="true"
  language="yaml"
/>
