{/* source: daml / daml/3.4/overview/explanations/ledger-model/local-ledger.rst */}
...
So different parties may observe two transactions on different Participant Nodes in different orders via the gRPC Ledger API.
Moreover, different Participant Nodes may output two transactions for the same party in different orders.
This document explains the ordering guarantees that Daml ledgers do provide, by [example](#causality-examples) and formally via the concept of [causality graphs](#causality-graph) and [local ledgers](#local-ledger-structure).

The presentation assumes that you are familiar with the following concepts:

* The Ledger API
* The [Daml Ledger Model](index.mdx#da-ledgers)

<a id="causality-examples"></a>
...
They are based on the paint counteroffer workflow from the Daml Ledger Model’s [privacy section](ledger-privacy.mdx#da-model-privacy),
...
{/* https://www.lucidchart.com/documents/edit/c4df0455-13ab-415f-b457-f5654c2684be */}![The time sequences for individual parties in the paint counteroffer workflow, with disclosures, as described in the Privacy section linked above.](explanations/ledger-model/images/divulgence-for-disclosure-counteroffer.svg)

<a id="causality-example-create-archive"></a>
...
<a id="causality-example-create-use-archive"></a>
...
<a id="causality-example-commit-atomic"></a>
...
<a id="causality-example-non-consuming"></a>
...
{/* https://app.lucidchart.com/documents/edit/1923969f-7bf2-45e0-a68d-6a0b2d308883/0_0 */}![The shared ledger view of the time sequence for the paint counteroffer workflow.](explanations/images/counteroffer-double-fetch.svg)
...
<a id="causality-example-out-of-band"></a>
...
![A time sequence for a counteroffer workflow with multiple commits: The first from the bank, the second and third by Alice, and the final one by the painter.](explanations/images/counteroffer-split-commit.svg)
...
<a id="causality-divulgence-example"></a>
...
In practice, this can happen in a setup where two Participant Nodes N<sub>1</sub> and N<sub>2</sub> host the painter.
He sees the divulged Iou and the created CounterOffer through N<sub>1</sub>‘s transaction tree stream
and then submits the acceptance through N<sub>1</sub>.
Like in the previous example, N<sub>2</sub> does not know about the dependence of the two commits.
Accordingly, N<sub>2</sub> may output the accepting transaction *before* the ShowIou contract on the transaction stream.
...
<a id="causality-example-depend-on-party"></a>
...
: A **causality graph** is a finite directed acyclic graph G of transactions that is transitively closed.
  Transitively closed means that whenever v<sub>1</sub> -> v<sub>2</sub> and v<sub>2</sub> -> v<sub>3</sub> are edges in G,
  then there is also an edge v<sub>1</sub> -> v<sub>3</sub> in G.
...
: For a causality graph G,
  ...
  It is the least partial order that includes the following ordering relations between two actions act<sub>1</sub> and act<sub>2</sub>:
  <br/>
  * act<sub>1</sub> and act<sub>2</sub> belong to the same transaction and act<sub>1</sub> precedes act<sub>2</sub> in the transaction.
  * act<sub>1</sub> and act<sub>2</sub> belong to different transactions in vertices tx<sub>1</sub> and tx<sub>2</sub> and there is a path in G from tx<sub>1</sub> to tx<sub>2</sub>.
    <Note>Checking for an *edge* instead of a *path* in G from tx<sub>1</sub> to tx<sub>2</sub> is equivalent
    because causality graphs are transitively closed.
    The definition uses *path* because the figures below omit transitive edges for readability.</Note>

The action order is a partial order on the actions in a causality graph.
For example, the following diagram shows such a causality graph for the ledger in the above [Out-of-band causality example](#causality-example-out-of-band).
...
![A causality graph for the counteroffer ledger, as described in the paragraph above.](explanations/images/counteroffer-split-action-order.svg)

<a id="causality-graph-consistency"></a>

### Consistency

Consistency ensures that a causality graph sufficiently orders all the transactions.
It generalizes [ledger consistency](ledger-integrity.mdx#da-model-consistency) from the Daml Ledger Model as [explained below](#causality-consistency-ledger-model).
...
![The causality graph for the counteroffer workflow, with the following highlighted in red: Iou $Bank A, Fetch A (Iou $Bank A), Exe A (Iou $Bank A) and Iou $Bank P.](explanations/images/causality-counteroffer-Iou-minimal.svg)
...
{/* https://app.lucidchart.com/documents/edit/b9d84f0f-e459-427c-86b8-c767662af326 */}![A minimal causality graph for a workflow in which the painter receives an Iou from Alice and has an Account contract to hold the Iou created automatically, as described above.](explanations/images/causality-consistency-examples.svg)
...
<a id="causality-consistency-ledger-model"></a>
...
For example, the transactions on the [ledger](#split-counteroffer-ledger) in the [out-of-band causality example](#causality-example-out-of-band) are a topological sort of the [corresponding causality graph](#causality-graph-counteroffer-split).
...
: For an X-consistent causality graph G, there exists a unique minimal X-consistent causality graph reduce<sub>X</sub>(G) with the same vertices and the edges being a subset of G.
  The graph reduce<sub>X</sub>(G) is called the X-**reduction** of G.
...
It is also the reduction of the topological sort, i.e., the [ledger](#split-counteroffer-ledger) in the [out-of-band causality example](#causality-example-out-of-band).

<Note>The reduction reduce<sub>X</sub>(G) of an X-consistent causality graph G can be computed as follows:

1. Set the vertices of G’ to the vertices of G.
2. The causal consistency conditions for contracts and keys demand that certain pairs of actions
   act<sub>1</sub> and act<sub>2</sub> in X must be action-ordered.
...
3. reduce<sub>X</sub>(G) is the transitive closure of G’.</Note>
...
Conversely, a sequence of commits L yields a causality graph G<sub>L</sub> by taking the transactions as vertices and adding an edge from tx1 to tx2 whenever tx1‘s commit precedes tx2‘s commit in the sequence.

There are now two consistency definitions:

* [Ledger Consistency](ledger-integrity.mdx#da-model-ledger-consistency) according to Daml Ledger Model
...
Conversely, if the sequence of commits L is ledger consistent, G<sub>L</sub> is a consistent causality graph, and so is the reduction reduce(G<sub>L</sub>).

<a id="local-ledger-structure"></a>

## Local Ledgers

As explained in the Daml Ledger Model, parties see only a [projection](ledger-privacy.mdx#da-model-projections) of the shared ledger for privacy reasons.
...
: The **projection** proj<sub>P</sub>(G) of a consistent causality graph G to a party P is the P-reduction of the following causality graph G’:
  <br/>
  * The vertices of G’ are the vertices of G projected to P, excluding empty projections.
  * There is an edge between two vertices v<sub>1</sub> and v<sub>2</sub> in G’ if there is an edge from the G-vertex corresponding to v<sub>1</sub> to the G-vertex corresponding to v<sub>2</sub>.
...
![The split counteroffer causality graph divided according to the viewpoints of different parties, as described in the paragraph below.](explanations/images/counteroffer-causality-projection.svg)
...
This difference explains the [divulgence causality example](#causality-divulgence-example).

<a id="ordering-guarantees"></a>

### Ledger API Ordering Guarantees

The Update Service provides the updates as a stream of Daml transactions
and the State Service summarizes all the updates up to a given point
...
<Note>The transaction trees of the Update Service omit **Fetch** and **NoSuchKey** actions
that are part of the transactions in the local ledger.
The **Fetch** and **NoSuchKey** actions are thus removed before the Update Service outputs the transaction trees.</Note>
...
<Note>The virtual shared causality graph exists only as a concept, to reason about Daml ledger guarantees.
...
That is, all the local ledgers can in theory be combined into a consistent single causality graph of which they are projections.</Note>
...
1. [Stakeholders of a Contract See Creation and Archival in the Same Order](#causality-example-create-archive)
...
2. [Signatories of a Contract and Stakeholder Actors See Usages After the Creation and Before the Archival](#causality-example-create-use-archive)
   ...
   the stakeholder’s local ledgers impose this order on the actions.
3. [Commits Are Atomic](#causality-example-commit-atomic)
...
4. [Non-Consuming Usages in Different Commits May Appear in Different Orders](#causality-example-non-consuming)
...
5. [Out-of-Band Causality Is Not Respected](#causality-example-out-of-band)
   Out-of-band data flow is not captured by causal consistency and therefore does not induce ordering.
6. [Divulged Actions Do Not Induce Order](#causality-divulgence-example)
...
7. [The Ordering Guarantees Depend on the Party](#causality-example-depend-on-party)
