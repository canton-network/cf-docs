...
Since this is a plain Canton participant with no Splice-specific components, configure it using Canton's native HOCON configuration format rather than the Splice Helm charts. See [Docker Deployment](/global-synchronizer/deployment/docker) for the container invocation pattern.
...
This configuration does not establish a synchronizer connection by itself — there is no static config key for it. After the participant starts, register the connection through the Canton Console or Admin API; see [Connecting to a Dedicated Synchronizer](/global-synchronizer/extension-synchronizers/dedicated-synchronizers#connecting-to-a-dedicated-synchronizer) and [Synchronizer Operations](/global-synchronizer/canton-console/essential-commands#synchronizer-operations) for the commands. Verify the connection:
