...
<Note>
For an end-to-end private synchronizer walkthrough that starts with configuration, see [Deploying a Private Synchronizer](/global-synchronizer/extension-synchronizers/deployment).
</Note>
...
``` none
canton.sequencers.acme.public-api {
  port = 5028
...
The information the client provides in step 2 is verifiable since it is a certificate chain of keys. The synchronizer rejects this if the namespace root key fingerprint included is not permissioned or if the topology state provided is invalid.
...
The expiration of the token generated in step 3 is valid for one hour by default. The nodes automatically renew the token in the background before it expires. The lifetime of the tokens and of the nonce can be reconfigured using
...
At this point, other nodes should be able to connect to the new Sequencer. To avoid problems, the best practice is to wait at least for the "maximum decision duration" (the sum of the `confirmationResponseTimeout` and `mediatorReactionTimeout` dynamic synchronizer parameters with a default of 30 seconds each) before connecting nodes to a newly-onboarded Sequencer.
...
// * write the topology snapshot, sequencer snapshot and static synchronizer parameters to files
{
  ...
  utils.retry_until_true(consoleEnvironment.commandTimeouts.bounded) {
    sequencer1.topology.sequencers
...
<Note>
Complete a basic deployment first ([Deploying a Private Synchronizer](/global-synchronizer/extension-synchronizers/deployment)), then apply HA for production redundancy.
</Note>
...
{/* COPIED_START source="docs-website:docs/replicated/canton/3.5/synchronizer/howtos/decommission/index.rst" hash="064f999f" */}
...
- Change the sequencer connection on the mediators connected to the sequencer to be decommissioned to use another active sequencer, as per [mediator connectivity](/global-synchronizer/reference/canton-console-commands#sequencer_connection.set):

```scala
mediator1.sequencer_connection.set(SequencerConnections.single(sequencer1.sequencerConnection))
```

- Reconnect participants to the synchronizer, as described in [Synchronizer connectivity](/global-synchronizer/reference/canton-console-commands#synchronizer-connectivity), using a sequencer connection to another active sequencer:
...
A quorum of synchronizer owners proposes the sequencer out of the synchronizer topology. The remaining active sequencers must still meet the threshold of the sequencer synchronizer state; otherwise, lower the threshold in the same proposal.
...
// remove the decommissioned sequencer from the synchronizer state
val threshold = sequencerSynchronizerState.item.threshold
val activeSequencers = sequencerSynchronizerState.item.active.filterNot(_ == sequencer3.id)
val newSerial = Some(sequencerSynchronizerState.context.serial.increment)
sequencer1.topology.sequencers.propose(
  synchronizerId,
  threshold,
...
If the synchronizer runs on BFT sequencers, wait until the ordering topology of the remaining sequencers no longer lists the decommissioned sequencer before stopping it; you can check that with the `bft.get_ordering_topology` command. Stopping it earlier can leave a BFT ordering network of three or fewer sequencers without a quorum.

The cryptographic material of a decommissioned sequencer must also be disposed of:
...
Mediators are also part of a synchronizer’s messaging infrastructure and do not store application contracts, so they are disposable as long as precautions are taken to avoid disrupting the synchronization services. This means ensuring that at least one mediator remains in its mediator group and that the remaining active mediators still meet the group’s threshold; otherwise, lower the threshold in the same proposal.

A quorum of synchronizer owners proposes the mediator out of the synchronizer topology:

```scala
// find the mediator synchronizer state of the group containing the decommissioned mediator
val mediatorSynchronizerState =
  sequencer1.topology.mediators
    .list(synchronizerId)
    .find(_.item.active.contains(mediator2.id))
    .getOrElse(sys.error("Did not find the mediator in a mediator synchronizer state on the synchronizer"))

// remove the decommissioned mediator from the synchronizer state
val threshold = mediatorSynchronizerState.item.threshold
val activeMediators = mediatorSynchronizerState.item.active.filterNot(_ == mediator2.id)
val newSerial = Some(mediatorSynchronizerState.context.serial.increment)
sequencer1.topology.mediators.propose(
  synchronizerId,
  threshold,
  activeMediators,
  group = mediatorSynchronizerState.item.group,
  serial = newSerial,
)
sequencer2.topology.mediators.propose(
  synchronizerId,
  threshold,
  activeMediators,
  group = mediatorSynchronizerState.item.group,
