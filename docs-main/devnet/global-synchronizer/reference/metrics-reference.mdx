...
This section will be expanded in a future update. Topology metrics track synchronizer membership changes and party-to-participant mappings. For monitoring guidance, see [Performance Optimization](/devnet/global-synchronizer/production-operations/performance-optimization).

## Key participant metrics

These are the most operationally significant metrics from the participant node. For the full catalog of several hundred metrics, see the [Canton 3.5 metrics reference](https://docs.digitalasset.com/canton/3.5/participant/reference/metrics).
