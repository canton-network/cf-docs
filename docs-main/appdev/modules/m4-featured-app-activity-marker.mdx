...
It is possible to share the attribution of activity for the `FeaturedAppActivityMarker`. The `FeaturedAppRight_CreateActivityMarker` choice accepts a list of `AppRewardBeneficiary <type-splice-api-featuredapprightv1-apprewardbeneficiary-32645>` contracts. Then a `FeaturedAppActivityMarker` is created for each `beneficiary` with the `weight` field set appropriately.
