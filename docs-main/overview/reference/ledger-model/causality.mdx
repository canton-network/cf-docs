---
title: "Causality"
description: "The partial ordering guarantees Canton ledgers provide, formalized through causality graphs and local ledgers."
---

{/* COPIED_START source="docs-website:docs/replicated/daml/3.5/overview/explanations/ledger-model/local-ledger.rst" hash="7610acb7" */}

Canton ledgers do not totally order all transactions.
So different parties may observe two transactions on different Participant Nodes in different orders via the [gRPC Ledger API](/sdks-tools/api-reference/ledger-api-services).
Moreover, different Participant Nodes may output two transactions for the same party in different orders.
This document explains the ordering guarantees that Canton ledgers do provide, by [example](#causality-examples) and formally via the concept of [causality graphs](#causality-graphs) and [local ledgers](#local-ledgers).

The presentation assumes that you are familiar with the following concepts:

- The [Ledger API](/sdks-tools/api-reference/ledger-api-services)

- The [Canton Ledger Model](/overview/reference/ledger-model/structure)

## Causality Examples

A deployed Canton Ledger need not totally order all transactions,
unlike the totally ordered Ledger assumed in the
[Structure](/overview/reference/ledger-model/structure#ledger) section of the Canton Ledger Model.
The following examples illustrate these ordering guarantees of the Ledger API.

They use a paint counteroffer workflow:
the Bank creates an *Iou* for Alice (A);
Alice creates a *CounterOffer* for the painter (P) and a *ShowIou* contract;
exercising *ShowIou* fetches the *Iou* and discloses it to the painter;
the painter accepting the *CounterOffer* transfers the *Iou* and creates a *PaintAgree* contract.
The party projections are as follows:

{/* https://www.lucidchart.com/documents/edit/c4df0455-13ab-415f-b457-f5654c2684be */}

<img src="/images/docs_website/divulgence-for-disclosure-counteroffer.svg" className="align-center" style={{width: "100.0%"}} alt="The time sequences for individual parties in the paint counteroffer workflow, with disclosures." />

### Stakeholders of a Contract See Creation and Archival in the Same Order

Every Canton Ledger orders the creation of the *CounterOffer A P Bank* before the painter exercising the consuming choice on the *CounterOffer*.
(If the **Create** was ordered after the **Exercise**, the resulting shared ledger would be inconsistent, which violates the validity guarantee of Canton ledgers.)
...
Yet, the **Fetch** may come before the non-consuming **Exercise** in the Bank's ledger-effects update stream.
...
<a id="split-counteroffer-ledger"></a>

<Frame caption="Counteroffer workflow with four commits.">
  <img src="/images/docs_website/counteroffer-split-commit.svg" className="align-center" style={{width: "100.0%"}} alt="A time sequence for a counteroffer workflow with multiple commits: The first from the bank, the second and third by Alice, and the final one by the painter." />
</Frame>
...
Canton ledgers therefore do not capture data flow through applications.
...
In practice, this can happen in a setup where two Participant Nodes <em>N</em><sub>1</sub> and <em>N</em><sub>2</sub> host the painter.
He sees the divulged *Iou* and the created *CounterOffer* through <em>N</em><sub>1</sub>'s ledger-effects update stream
and then submits the acceptance through <em>N</em><sub>1</sub>.
Like in the previous example, <em>N</em><sub>2</sub> does not know about the dependence of the two commits.
Accordingly, <em>N</em><sub>2</sub> may output the accepting transaction *before* the *ShowIou* contract on the transaction stream.
...
Such witnessed events are included in the ledger-effects update stream as a convenience:
...
The above examples indicate that Canton ledgers order transactions only partially.
Canton ledgers can be represented as finite directed acyclic graphs (DAG) of transactions.

<a id="def-causality-graph"></a>
...
Transitively closed means that whenever <em>v</em><sub>1</sub> -> <em>v</em><sub>2</sub> and <em>v</em><sub>2</sub> -> <em>v</em><sub>3</sub> are edges in *G*,
then there is also an edge <em>v</em><sub>1</sub> -> <em>v</em><sub>3</sub> in *G*.

</Note>

<a id="def-action-order"></a>
...
It is the least partial order that includes the following ordering relations between two actions <em>act</em><sub>1</sub> and <em>act</em><sub>2</sub>:

- <em>act</em><sub>1</sub> and <em>act</em><sub>2</sub> belong to the same transaction and <em>act</em><sub>1</sub> precedes <em>act</em><sub>2</sub> in the transaction.
- <em>act</em><sub>1</sub> and <em>act</em><sub>2</sub> belong to different transactions in vertices <em>tx</em><sub>1</sub> and <em>tx</em><sub>2</sub> and there is a path in *G* from <em>tx</em><sub>1</sub> to <em>tx</em><sub>2</sub>.

<Note>

Checking for an *edge* instead of a *path* in *G* from <em>tx</em><sub>1</sub> to <em>tx</em><sub>2</sub> is equivalent
...
For example, the following diagram shows such a causality graph for the ledger in the above [Out-of-band causality example](#out-of-band-causality-is-not-respected).
...
<a id="causality-graph-counteroffer-split"></a>

<Frame caption="Causality graph for the counteroffer workflow with four commits.">
  <img src="/images/docs_website/counteroffer-split-action-order.svg" className="align-center" style={{width: "100.0%"}} alt="A causality graph for the counteroffer ledger, as described in the paragraph above." />
</Frame>

### Consistency

Consistency ensures that a causality graph sufficiently orders all the transactions.
It generalizes [ledger consistency](/overview/reference/ledger-model/integrity#consistency) from the Canton Ledger Model as [explained below](#from-causality-graphs-to-ledgers).

<a id="def-causal-consistency-contract"></a>
...
<a id="def-consistency-causality-graph"></a>
...
<a id="minimal-consistent-causality-graph"></a>
...
For example, the [above causality graph for the split counteroffer workflow](#causality-graph-counteroffer-split) is consistent.
...
<a id="causality-counteroffer-Iou-minimal"></a>

<Frame caption="Minimal consistent causality graph for the highlighted actions.">
  <img src="/images/docs_website/causality-counteroffer-Iou-minimal.svg" className="align-center" style={{width: "100.0%"}} alt="The causality graph for the counteroffer workflow, with the following highlighted in red: Iou $Bank A, Fetch A (Iou $Bank A), Exe A (Iou $Bank A) and Iou $Bank P." />
</Frame>
...
For example, the transactions on the [ledger](#split-counteroffer-ledger) in the [out-of-band causality example](#out-of-band-causality-is-not-respected) are a topological sort of the [corresponding causality graph](#causality-graph-counteroffer-split).
...
<a id="def-reduction-causality-graph"></a>

<Note>

**Definition: Reduction of a consistent causality graph**

For an *X*-consistent causality graph *G*, there exists a unique minimal *X*-consistent causality graph <em>reduce</em><sub>X</sub><em>(G)</em> with the same vertices and the edges being a subset of *G*.
The graph <em>reduce</em><sub>X</sub><em>(G)</em> is called the *X*-**reduction** of *G*.
...
It is also the reduction of the topological sort, i.e., the [ledger](#split-counteroffer-ledger) in the [out-of-band causality example](#out-of-band-causality-is-not-respected).

<Note>

The reduction <em>reduce</em><sub>X</sub><em>(G)</em> of an *X*-consistent causality graph *G* can be computed as follows:

1. Set the vertices of *G'* to the vertices of *G*.
1. The causal consistency conditions for contracts and keys demand that certain pairs of actions
   <em>act</em><sub>1</sub> and <em>act</em><sub>2</sub> in *X* must be action-ordered.
...
1. <em>reduce</em><sub>X</sub><em>(G)</em> is the transitive closure of *G'*.

</Note>

Topological sort and reduction link causality graphs *G* to the ledgers *L* from the Canton Ledger Model.
Topological sort transforms a causality graph *G* into a sequence of transactions; extending them with the requesters gives a sequence of commits, i.e., a ledger in the Canton Ledger Model.
Conversely, a sequence of commits *L* yields a causality graph <em>G</em><sub>L</sub> by taking the transactions as vertices and adding an edge from *tx1* to *tx2* whenever *tx1*'s commit precedes *tx2*'s commit in the sequence.

There are now two consistency definitions:

- [Ledger Consistency](/overview/reference/ledger-model/integrity#definition) according to Canton Ledger Model

- [Consistency of causality graph](#def-consistency-causality-graph)
...
Conversely, if the sequence of commits *L* is ledger consistent, <em>G</em><sub>L</sub> is a consistent causality graph, and so is the reduction <em>reduce(G</em><sub>L</sub><em>)</em>.

## Local Ledgers

As explained in the Canton Ledger Model, parties see only a [projection](/overview/reference/ledger-model/privacy#projection) of the shared ledger for privacy reasons.
...
For example, the [split counteroffer causality graph without the edge tx2 -> tx4](#causality-counteroffer-Iou-minimal) is consistent for the Bank because the Bank is a stakeholder informee of exactly the highlighted actions.
It is also minimal Bank-consistent and the Bank-reduction of the [original split counteroffer causality graph](#causality-graph-counteroffer-split).

<Note>

**Definition: Projection of a consistent causality graph**

The **projection** <em>proj</em><sub>P</sub><em>(G)</em> of a consistent causality graph *G* to a party *P* is the *P*-reduction of the following causality graph *G'*:

- The vertices of *G'* are the vertices of *G* projected to *P*, excluding empty projections.

- There is an edge between two vertices <em>v</em><sub>1</sub> and <em>v</em><sub>2</sub> in *G'* if there is an edge from the *G*-vertex corresponding to <em>v</em><sub>1</sub> to the *G*-vertex corresponding to <em>v</em><sub>2</sub>.

</Note>

For the [split counteroffer causality graph](#causality-graph-counteroffer-split), the projections to Alice, the Bank, and the painter are as follows.

{/* https://app.lucidchart.com/documents/edit/65a83eba-9b09-4003-b824-8e7bec50ce10 */}

<a id="counteroffer-causality-projections"></a>

<Frame caption="Projections of the split counteroffer causality graph.">
  <img src="/images/docs_website/counteroffer-causality-projection.svg" className="align-center" style={{width: "100.0%"}} alt="The split counteroffer causality graph divided according to the viewpoints of different parties, as described in the paragraph below." />
</Frame>
...
This difference explains the [divulgence causality example](#divulged-actions-do-not-induce-order).

### Ledger API Ordering Guarantees

The [Update Service](/sdks-tools/api-reference/ledger-api-services#update-service) provides the updates as a stream of Daml transactions
and the [State Service](/sdks-tools/api-reference/ledger-api-services#state-service) summarizes all the updates up to a given point
...
That is, the ledger-effects update stream (transaction shape `TRANSACTION_SHAPE_LEDGER_EFFECTS`) for a party is a topological sort of the party's local ledger.
The ACS-delta update stream (transaction shape `TRANSACTION_SHAPE_ACS_DELTA`) contains precisely the `CreatedEvent`s and `ArchivedEvent`s
that correspond to **Create** and consuming **Exercise** actions in the ledger-effects update stream where the party is a stakeholder of the affected contract.

<Note>

The transactions output by the [Update Service](/sdks-tools/api-reference/ledger-api-services#update-service) omit **Fetch** and **NoSuchKey** actions
that are part of the transactions in the local ledger.
The **Fetch** and **NoSuchKey** actions are thus removed before the [Update Service](/sdks-tools/api-reference/ledger-api-services#update-service) outputs the transactions.
...
In particular, an application can process all subsequent events from the ACS-delta or ledger-effects update stream without having to take events before the snapshot into account.
...
Nevertheless, Canton ledgers ensure that all local ledgers are projections of a virtual shared causality graph that connects to the Canton Ledger Model as described above.
The ledger validity guarantees therefore extend via the local ledgers to the Ledger API.
These guarantees are subject to the deployed Canton ledger's trust assumptions.

<Note>

The virtual shared causality graph exists only as a concept, to reason about Canton ledger guarantees.
A deployed Canton ledger in general does not store or even construct such a shared causality graph.
...
The [causality examples](#causality-examples) can be explained in terms of causality graphs and local ledgers as follows:

1. [Stakeholders of a Contract See Creation and Archival in the Same Order](#stakeholders-of-a-contract-see-creation-and-archival-in-the-same-order)
...
1. [Signatories of a Contract and Stakeholder Actors See Usages After the Creation and Before the Archival](#signatories-of-a-contract-and-stakeholder-actors-see-usages-after-the-creation-and-before-the-archival)
...
1. [Commits Are Atomic](#commits-are-atomic)
...
1. [Non-Consuming Usages in Different Commits May Appear in Different Orders](#non-consuming-usages-in-different-commits-may-appear-in-different-orders)
...
1. [Out-of-Band Causality Is Not Respected](#out-of-band-causality-is-not-respected)
   Out-of-band data flow is not captured by causal consistency and therefore does not induce ordering.

1. [Divulged Actions Do Not Induce Order](#divulged-actions-do-not-induce-order)
   The painter is not an informee of the **Fetch** and **Exercise** actions on Alice's *Iou*;
   he merely witnesses them.
   The [painter's local ledger](#counteroffer-causality-projections) therefore does not order *tx3* before *tx4*.
   So the painter's transaction stream can output *tx4* before *tx3*.

1. [The Ordering Guarantees Depend on the Party](#the-ordering-guarantees-depend-on-the-party)
   Alice is an informee of the **Fetch** and **Exercise** actions on her *Iou*.
   Unlike for the painter, [her local ledger](#counteroffer-causality-projections) does order *tx3* before *tx4*,
   so Alice is guaranteed to observe *tx3* before *tx4* on all Participant Nodes through which she is connect to the Canton ledger.
