...
- Review the list of changes against the [upgrade limitations](/mainnet/appdev/modules/m6-limitations) to ensure no breaking modifications
...
- [Upgrade Limitations](/mainnet/appdev/modules/m6-limitations) -- Constraints that affect production rollouts
- [Testing Upgrades](/mainnet/appdev/modules/m6-testing-upgrades) -- Testing strategies before going to production
- [Error Handling](/mainnet/appdev/modules/m7-error-handling) -- Handling errors that may arise during mixed-version deployments
- [Deployment Progression](/mainnet/appdev/modules/m5-deployment-progression) -- The DevNet to TestNet to MainNet path
- [Smart Contract Upgrading Reference](/mainnet/appdev/deep-dives/smart-contract-upgrading-reference) — Detailed package validation and runtime upgrade rules.
- [Values in the Ledger API](/mainnet/appdev/deep-dives/values-in-the-ledger-api) — How the Ledger API validates and normalizes values during command submission and query.
