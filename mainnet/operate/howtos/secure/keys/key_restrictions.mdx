...
* `Namespace`: a key that defines a node’s identity and signs topology requests (see [namespace key management](namespace_key.md#namespace-key-management))
...
Set the `usage` parameter with the desired usages when you [generate a new key](key_management.md#deleting-canton-keys).
...
Instead, a new key must be generated and activated, and the old one deactivated. See [key management](key_management.md#key-management)
for more information regarding the key management commands.

If you are using a Key Management Service (KMS) to handle Canton’s keys in [external key storage mode](../kms/mode/external_key_storage.md#external-key-storage),
...
As described in [the identity management page](../../operate/identity_management.md#identity-transactions), Canton uses topology transactions to manage identities,
and these transactions are signed by keys with `Namespace` usage. [Namespace key management](namespace_key.md#namespace-key-management) shows
