...
resume processing transactions from that point on after a [crash or restart](non-functionals.md#crash-fault-tolerance).

Offset checkpoints are in particular required around [Major
Splice Upgrades](node-operations.md#hard-synchronizer-migration) where there is no Daml
...
As part of the [Integration Workflows](workflows.md#integration-workflows), Tx History Ingestion is
expected to extract a number of fields for both deposits and
withdrawals. Below we provide details on the transaction
structure of the [Integration Workflows](workflows.md#integration-workflows) and how to parse it.

#### NOTE
The following code is available to help you implement your own parsing logic:

* **JavaScript/TypeScript**: use the
  [token standard history parser](../token-standard/index.md#list-holding-transactions) provided in the wallet SDK.
