...
description: 'A high-level overview of the Canton Network for wallet providers and exchanges.'
---
...
- The tokenomics operate on 10-minute "mining rounds".
...
- Rewards are minted the following mining round; if rewards are not redeemed they are lost.

You can find more information about the tokenomics in the [Canton Coin Tokenomics section](/overview/reference/canton-coin-tokenomics).

## Traffic

- Traffic fees are paid at the validator node level, not the party level.
- Every validator has a traffic balance at the global synchronizer level, measured in bytes, with a trickle rate of free traffic provided to validator nodes every 10 minutes (each mining round).
- Traffic is deducted every time your node sends a message to the synchronizer, and is obtained by burning Canton Coin (always pre-purchased).

For the full detail — including how to buy more traffic and how to determine the traffic cost of a transaction — see the [Synchronizer Traffic Fees page](/global-synchronizer/deployment/synchronizer-traffic).