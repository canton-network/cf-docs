{/* source: canton / canton/3.4/participant/howtos/upgrade/index.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}
<Note>This page is a work in progress. It may contain incomplete or incorrect information.</Note>

<a id="canton-upgrading"></a>

# Upgrade To a New Release

This section covers the processes to upgrade Canton participant nodes and synchronizers.
Upgrading Daml applications is covered [elsewhere](https://docs.daml.com/upgrade/upgrade.html).

As elaborated in the [versioning guide](../../reference/versioning.mdx#canton-versioning), new features, improvements
...
- [Upgrading the Canton binary](#upgrade-canton-binary) that is used to run a node.
- [Upgrading the protocol version](#change-pv) that defines how nodes interact with each other.
...
A Canton binary supports [multiple protocol versions](../../reference/versioning.mdx#protocol-version), and new protocol
versions are introduced in a backward-compatible way with a new binary
(see [version table](../../reference/versioning.mdx#release-version-to-protocol-version-table)). Therefore, any upgrade
...
<Warning>Upgrading requires care and preparation.
: * **Please back up your data before any upgrade.**
  <br/>
    Note that the [order of the backups](../recover/backup-restore.mdx#order-of-backups) is important.
  ...
  * **Once a migration has started it cannot be aborted or rolled back.**
  <br/>
    If you encounter an issue, you must restore the old version using the backups.</Warning>

<a id="upgrade-canton-binary"></a>
...
The schema in the database is versioned and managed using [Flyway](https://flywaydb.org/). Detecting
...
```none
@ participant.start()
    ERROR com.digitalasset.canton.integration.EnvironmentDefinition$$anon$3:UpgradingDocumentationIntegrationTest - failed to initialize participant: There are 4 pending migrations to get to database schema version 3.0. Currently on version 1.1. Please run `participant.db.migrate` to apply pending migrations
...
You can also [configure the migrations to be applied automatically](../operate/console/console.mdx#migrate-and-start-mode).
...
@ participant.synchronizers.list_connected()
    res5: Seq[ListConnectedSynchronizersResult] = Vector(
      ListConnectedSynchronizersResult(
        ...
        physicalSynchronizerId = testdomain::1220327f98bd...::34-0,
        healthy = true
      )
    )
```
...
@ participant.health.ping(participant)
    res6: Duration = 1577 milliseconds
```
...
<a id="canton-synchronizer-protocol-version-upgrading"></a>
...
A hard synchronizer connection upgrade can be performed using the [respective migration command](../../reference/console.mdx#repair-migrate-synchronizer).
...
[setting the resource limits](../../reference/console.mdx#resources-set-resource-limits) to 0 on all participants:
...
@ participant.synchronizers.list_connected()
    res8: Seq[ListConnectedSynchronizersResult] = Vector(
      ListConnectedSynchronizersResult(
        ...
        physicalSynchronizerId = newdomain::12201275995a...::34-0,
        healthy = true
      )
    )
```
...
```none
@ participant.health.ping(participant)
    res10: Duration = 1344 milliseconds
```

<Note>Note that currently, the hard migration is the only supported way to migrate a production system.
This is because unique contract keys are restricted to a single synchronizer.</Note>
...
<Note>The soft synchronizer connection upgrade is currently only supported as an alpha feature.</Note>
