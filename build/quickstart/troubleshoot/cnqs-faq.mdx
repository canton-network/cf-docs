{/* source: quickstart / quickstart/3.4/sdk/quickstart/troubleshoot/cnqs-faq.rst */}

<a id="quickstart-faq"></a>

# Canton Network App Quickstart FAQ

**Contents**

[CN App Quickstart Frequently Asked Questions](#cn-qs-frequently-asked-questions)
...
> pitfalls](#best-practices-common-pitfalls)

> [Database & query access](#database-query-access)

[CN App Quickstart Make target reference](#cn-qs-make-target-reference)
...
If you need additional support, the [Nix reference manual](https://nix.dev/manual/nix/2.24/command-ref/conf-file.html#conf-ssl-cert-file)
...
[corporate TLS certificates](https://determinate.systems/posts/zscaler-macos-and-nix-on-corporate-networks/)
problems on MacOS.
The NixOS team forked this solution as an [experimental installer](https://github.com/NixOS/experimental-nix-installer)
...
Please refer to the [MainNet](https://docs.dev.sync.global/app_dev/overview/version_information.html)
and [TestNet](https://docs.test.sync.global/app_dev/overview/version_information.html)
...
All Splice DARs are available in the [Splice repository](https://github.com/hyperledger-labs/splice/tree/main/daml/dars).
...
<Note>`USER` in `/Users/USER/` will display your username. Copy and paste from your terminal. NOT this FAQ.</Note>
...
![Docker Desktop version 4.39.0](quickstart/troubleshoot/images/docker-desktop-v4390-about.png)

If you are running Version 4.38.0 then upgrade Docker Desktop and make a new attempt.

If you’d like to learn more about this issue, visit [JDK Bug System](https://bugs.openjdk.org/browse/JDK-8348566).
...
You can use Grafana at [http://localhost:3030/](http://localhost:3030/) to monitor system metrics if `observability` is enabled.
...
2. Grafana dashboards: [http://localhost:3030/](http://localhost:3030/)
...
Go to [https://sync.global/sv-network/](https://sync.global/sv-network/) and look for the `migration_id` value.
...
Refer to the CN Utilities documentation for application-specific deployment instructions with an example.
...
<a id="make-target-reference"></a>

## **CN App Quickstart Make target reference**

| **Target**                          | **Description**                                                                                                                                                                                                |
|-------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| build                               | Build frontend, backend, Daml model and docker<br/>images.                                                                                                                                                     |
| build-backend                       | Build the backend service.                                                                                                                                                                                     |
| build-daml                          | Build the Daml model.                                                                                                                                                                                          |
| build-frontend                      | Build the frontend application.                                                                                                                                                                                |
| canton-console                      | Start the Canton console. Connects to the<br/>running app provider, app-user, sv ledgers.                                                                                                                      |
| capture-logs                        | Consumes Docker events and starts capturing<br/>logs to `/logs` directory for each service<br/>when a `start` Docker event is observed.<br/>Ideal for diagnostic purposes.                                     |
| clean                               | Clean the build artifacts.                                                                                                                                                                                     |
| clean-all                           | Stop and remove all build artifacts, Docker<br/>containers and volumes.                                                                                                                                        |
| clean-application                   | Like `clean-docker`, but leaves<br/>observability services running.                                                                                                                                            |
| clean-canton-<br/>console           | Stop and remove the Canton console container.                                                                                                                                                                  |
| clean-docker                        | Stop and remove application Docker containers<br/>and volumes.                                                                                                                                                 |
| clean-shell                         | Stop and remove the Daml Shell container.                                                                                                                                                                      |
| compose-config                      | Displays finalized configuration for each<br/>service initiated by `make start`.<br/>Dynamic environment variables<br/>e.g. `APP_PROVIDER_PARTY` are resolved at<br/>run time and not included in this output. |
| create-<br/>app-install-request     | Submit an App Install Request from the App<br/>User participant node.                                                                                                                                          |
...
| shell                               | Start Daml Shell. Connects to the running<br/>application PQS database.                                                                                                                                        |
| start                               | Start the application and observability<br/>services if enabled.                                                                                                                                               |
| status                              | Show status of Docker containers.                                                                                                                                                                              |
| stop                                | Stop the application and observability<br/>services.                                                                                                                                                           |
| stop-application                    | Stop only the application, leaving<br/>observability services running.                                                                                                                                         |
| tail                                | Tail logs of Docker containers.                                                                                                                                                                                |
| update-env-<br/>sdk-runtime-version | Helper to update DAML_RUNTIME_VERSION in .env<br/>based on daml/daml.yaml sdk-version.                                                                                                                         |
...
| open-sv-wallet            | Open the Super Validator wallet UI in the active<br/>browser    |
| o<br/>pen-sv-interface    | Open the Super Validator interface UI in the<br/>active browser |
| open-sv-scan              | Open the Super Validator Scan UI in the active<br/>browser      |
| open<br/>-app-user-wallet | Open the App User wallet UI in the active browser               |

## **LocalNet URLs**

| **URL**                                                      | **Description**                                                    |
|--------------------------------------------------------------|--------------------------------------------------------------------|
| [http://localhost:3000](http://localhost:3000)               | Main application UI                                                |
| [http://localhost:3030](http://localhost:3030)               | Grafana observability dashboard (if<br/>enabled)                   |
| [http://localhost:4000](http://localhost:4000)               | Super Validator gateway - lists<br/>available web UI options       |
| [http://wallet.localhost:2000](http://wallet.localhost:2000) | Canton Coin wallet interface                                       |
| [http://sv.localhost:4000](http://sv.localhost:4000)         | Super Validator Operations                                         |
| [http://scan.localhost:4000](http://scan.localhost:4000)     | Canton Coin Scan web UI - shows<br/>balances and validator rewards |
| [http://localhost:7575](http://localhost:7575)               | Ledger API service                                                 |
| [http://localhost:5003](http://localhost:5003)               | Validator API service                                              |
