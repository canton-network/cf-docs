...
The SV node includes a CometBFT node for BFT ordering. You need to generate CometBFT keys and configure peer connections.

### Key Generation

Generate a CometBFT node key and validator key. These are separate from your SV identity keys and are specific to the CometBFT consensus layer.

### State Sync

For joining an existing network, configure CometBFT state sync to bootstrap your node from the current network state rather than replaying from genesis. This significantly reduces initial sync time.

### BFT Sequencer Connections

Configure your node to connect to the BFT sequencer endpoints of other SVs. The sequencer connections enable your node to participate in the ordering protocol.

## Installing PostgreSQL

The SV node requires PostgreSQL databases for the participant, sequencer, mediator, and application components. You can deploy PostgreSQL in-cluster or use a managed cloud service (AWS RDS, GCP Cloud SQL, Azure Database for PostgreSQL).
