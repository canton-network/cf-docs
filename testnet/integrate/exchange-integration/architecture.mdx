...
* The **Exchange Validator Node** is a [Splice validator node](../integrating-with-canton-network/index.md#validator-nodes) that hosts
  your `treasuryParty`, which is the party you setup to control funds, receive deposits, and
  execute transfers for withdrawals. See [Setup Exchange Parties](node-operations.md#exchange-parties-setup) for details on how to setup the `treasuryParty`.
...
in the [Integration Workflows](workflows.md#integration-workflows) section.
...
2. **Withdrawal Automation**:
   ...
   to simplify [disaster recovery](disaster-recovery.md#disaster-recovery).
...
We explain them in the [Integration Workflows](workflows.md#integration-workflows) section.
