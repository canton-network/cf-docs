...
When a local party is hosted on multiple validators with Confirmation permission, any of those validators can submit transactions on behalf of the party (an external party can submit on any validator). The synchronizer delivers transaction views to all hosting validators, so each one maintains a consistent view of the party's contracts.
