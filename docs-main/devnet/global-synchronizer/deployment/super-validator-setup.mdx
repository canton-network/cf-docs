...
SV deployment is significantly more involved than a standard validator. If you are setting up a regular validator, see the [Installation](/devnet/global-synchronizer/deployment/deployment-options) page instead.
...
SV nodes expose web UIs that require authentication. The setup is similar to standard validator authentication (see [Authorization Setup](/devnet/global-synchronizer/deployment/authorization-setup)), with additional clients for the SV-specific UIs.
...
The Splice node bundle contains Helm charts and sample values for SV deployment. Download the bundle as described in the [Installation](/devnet/global-synchronizer/deployment/deployment-options) guide.
...
For the Helm-chart-based Kubernetes deployment of a Super Validator node — including authentication, CometBFT, Postgres, ingress, and helm chart installation — see [Kubernetes Deployment](/devnet/global-synchronizer/deployment/kubernetes-deployment).
