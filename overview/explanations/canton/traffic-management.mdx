{/* source: canton / canton/3.4/overview/explanations/canton/traffic-management.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="overview-explanation-traffic-management"></a>
...
Refer to the dynamic synchronizer parameters documentation for more details on these parameters.
...
{/* https://lucid.app/lucidchart/075a2a25-0fc2-4263-a7dc-283aecd00b4d/edit */}![image](explanations/canton/images/traffic_control_enforcement.svg)
...
How fast traffic accumulates and the maximum amount of base traffic a member can accumulate is determined by the synchronizer’s traffic [configuration](#traffic-config).
...
Therefore, in order to add extra traffic for a member on a synchronizer, the same traffic purchase request must be submitted to at least threshold sequencers within a [configurable](#traffic-config) time window.

<Danger>The request sets the new absolute value of total extra traffic purchased for the member. **NOT** a delta in traffic.
...
If later P1 wants an additional `500` extra traffic credit, a new `SetTrafficPurchased` request with an amount of `1500` must be successfully processed.</Danger>
...
If the serial is too low, the request fails and must be retried with the correct serial. To obtain the current serial, see [observing traffic](#observing-traffic).
