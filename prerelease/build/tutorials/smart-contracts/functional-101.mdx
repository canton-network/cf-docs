<!-- source: daml / daml/3.5/sdk/tutorials/smart-contracts/functional-101.rst -->
...
If you no longer have your [Compose choices](compose.md) and [Work with dependencies](dependencies.md) projects set up, and want to look back at the code, please follow the setup instructions in [Work with dependencies](dependencies.md) to get hold of the code for this chapter.
...
In [Data types](data.md) you learnt about one half of Daml’s type system: Data types. It’s now time to learn about the other, which are Function types. Function types in Daml can be spotted by looking for `->` which can be read as “maps to”.
...
Precedence](../../reference/fixity.md#reference-fixity-and-associativity)

### Type constraints

The `Additive a =>` part of the signature of `add` is a type constraint on the type parameter `a`. `Additive` here is a typeclass. You already met typeclasses like `Eq` and `Show` in [Data types](data.md). The `Additive` typeclass says that you can add a thing, i.e. there is a function `(+) : a -> a -> a`. Now the way to read the full signature of `add` is “Given that `a` has an instance for the `Additive` typeclass, `a` maps to a function which maps `a` to `a`”.
...
You met pattern matching in [Data types](data.md), using `case` expressions which is one way of pattern matching. However, it can also be convenient to do the pattern matching at the level of function arguments. Think about implementing the function `uncurry`:
...
We can use function level pattern matching together with a feature called *Record Wildcards* to write the function `issueAsset` in [Work with dependencies](dependencies.md):
...
More often it makes sense to define functions locally, inside a `let` clause or similar. Good examples of this are the `validate` and `transfer` functions defined locally in the `Trade_Settle` choice of the  model from [Work with dependencies](dependencies.md):
...
Until [Compose choices](compose.md) the only real kind of control flow introduced has been `case`, which is a powerful tool for branching.

#### If-else expression

[Add constraints to a contract](constraints.md) also showed a seemingly self-explanatory `if ... else` expression, but didn’t explain it further. Let’s implement the function `boolToInt : Bool -> Int` which in typical fashion maps `True` to `1` and `False` to `0`. Here is an implementation using `case`:
...
The folds and `map` function above are pure in the sense introduced in [Add constraints to a contract](constraints.md): The functions used to map or process items have no side effects. If you have looked at the [Work with dependencies](dependencies.md) models, you’ll have noticed `mapA`, `mapA_`, and `forA`, which seem to serve a similar role but within `Action`s . A good example is the `mapA` call in the `testMultiTrade` script:
...
You now know the basics of functions and control flow, both in pure and Action contexts. The [Work with dependencies](dependencies.md) example shows just how much can be done with just the tools you have encountered here, but there are many more tools at your disposal in the Daml standard library. It provides functions and typeclasses for many common circumstances and in [The Daml standard library](stdlib.md), you’ll get an overview of the library and learn how to search and browse it.
