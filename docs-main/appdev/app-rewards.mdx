...
The Global Synchronizer's reward system uses several coupon types, each representing a different kind of network contribution:

- **ValidatorRewardCoupon** -- Issued to validators for processing transactions. Every transaction that commits on the Global Synchronizer generates a validator reward for the submitting validator.
- **AppRewardCoupon** -- Issued to application providers whose applications generate qualifying activity on the network. This is the primary reward mechanism for app developers.
- **ValidatorLivenessActivityRecord** -- Tracks validator uptime and availability. Validators that maintain consistent liveness earn rewards through this mechanism.
- **SvRewardCoupon** -- Issued to Super Validators for operating the Global Synchronizer infrastructure (sequencer and mediator nodes).
