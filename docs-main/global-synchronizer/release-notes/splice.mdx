  ...
  **Action recommended for app devs:** app's with Daml code that statically depends on ``splice-amulet``
  ...
  - Daml
    ...
    - Add a new field `rewardConfig` to the `AmuletConfig` for configuring whether rounds should use traffic-based app rewards or on-ledger reward accounting, and whether traffic-based app reward coupon creation should be simulated in a dry-run mode.
      See the `RewardConfig` data type definition for the list reward configuration fields and their semantics.
...
- SV app
    ...
    - Amulet-based expiry triggers now skip batches whose preferred amulet package version is listed in the `ignored-amulet-versions` configuration and prior to the minimal supported version.
      ...
      This allows sv-1 to better handle participants that have not yet vetted the latest amulet package and avoid repeated failures.
