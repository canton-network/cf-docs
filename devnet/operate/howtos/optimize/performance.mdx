...
through a [set of console commands](../../reference/console.md#ledger-pruning-commands) and allows removal of past events and archived contracts
...
Please check the [Postgres persistence section](storage.md#postgres-performance-tuning) for tuning instructions.
...
up ([check the troubleshooting guide](../troubleshoot/troubleshooting_guide.md#how-to-diagnose-slow-db-queries)).

Detailed instructions on handling this issue can also be found in [Max Connection Settings](storage.md#max-connection-settings).

**Size of database task queue.** If you are seeing frequent `RejectedExecutionExceptions` when Canton queries the database,
increase the size of the task queue, as described in [Database task queue full](../troubleshoot/FAQ.md#database-task-queue-full). The rejection is otherwise
...
as described in Section [Java Virtual Machine Arguments](../configure/general/command_line.md#jvm-arguments).
