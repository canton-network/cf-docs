...
See [Configure a KMS](../configuration/kms_configuration.md#kms-configuration).
...
[Migrate to external key storage with a KMS](../migration/external_key_storage_migration.md#external-key-storage-migration).
...
([AWS](../configuration/kms_aws_config.md#kms-aws-config), [GCP](../configuration/kms_gcp_config.md#kms-gcp-config) or [Driver](../configuration/kms_driver_config.md#kms-driver-config)).
...
[Key configuration for external keys](../../../../reference/crypto_schemes.md#key-configuration-external-keys) and record the generated keys identifier.

To be able to use these keys, you must configure manual initialization in the Participant.
This is explained in detail in [Initialize node identity manually](../../../operate/identity/manual_identity_init.md#setup-manual-identity).
...
(see [Signing Key Usage](../../keys/key_restrictions.md#signing-key-usage-restrictions)).
