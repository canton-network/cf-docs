...
These requirements are largely identical between the docker-compose based deployment and the k8s deployment but exclude overhead from k8s itself or ingress.
...
| Option | Pros | Cons |
|--------|------|------|
| **Managed (RDS, Cloud SQL, Azure Database)** | Automated backups, high availability | Cost, less operational control |
| **Self-managed PostgreSQL** | Full control, cost-effective | Operational overhead |
| **Container-local PostgreSQL** | Simple setup | Not suitable for production |
...
| **PQS offloading** | Move read queries to a separate service |
...
<Card title="Onboarding Process" icon="user-plus" href="/testnet/global-synchronizer/deployment/onboarding-process">
  Get onboarded with an SV sponsor.
</Card>

<Card title="Installation" icon="download" href="/testnet/global-synchronizer/deployment/deployment-options">
