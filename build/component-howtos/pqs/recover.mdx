{/* source: pqs / pqs/3.4/component-howtos/pqs/recover.rst */}
...
`validate_offset_exists()` in [Offset management](references/sql-api.mdx#pqs-references-offset-management) for more details.
...
The following [Configuration options](references/configuration-options.mdx#pqs-references-configuration-options) are available to control the retry behavior of PQS:
...
The following metrics are available to monitor stability of PQS’ dependencies. See [Application metrics](observe.mdx#pqs-application-metrics)
