{/* source: daml / daml/3.4/overview/explanations/ledger-model/ledger-structure.rst */}
{/* Copyright (c) 2023 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="ledger-structure"></a>
...
The basic building blocks of the recorded interactions are [actions](#actions), which get grouped into [transactions](#transactions), *updates*, *commits*, and the [Ledger](#da-ledger-definition).

<a id="ledger-structure-running-example"></a>

## Running workflow example

Most of the examples in this section look at the following Daml Script scenarios
based on the templates from the [running example](index.mdx#da-ledgers-running-example).
...
<a id="actions-hierarchical-structure"></a>
...
{/* https://lucid.app/lucidchart/f3f49c7c-d257-4136-9dcb-39750f45c24d/edit */}![The settlement action on the ``SimpleDvp`` contract between Alice and Bob, with the two legs of the swap as consequences.](explanations/ledger-model/images/dvp-settle-action.svg)
...
2. The right subtree is analogous:
...
The [integrity section](ledger-integrity.mdx#da-model-integrity) goes into the details of this.
...
This hierarchical structure induces a [subaction relationship explained below](#da-ledger-subaction)
and forms the basis for the [privacy model](ledger-privacy.mdx#da-model-privacy).
...
1. A **Create** node records the creation of the contract.
   ...
   * The **contract ID** is a unique identifier of the contract.
     It is equivalent to the transaction output (TxO) in ledgers based on [unspent transaction outputs (UTxO)](#da-ledger-input-output).
   ...
   * The **contract observers**, or just observers for short, are the set of parties that will be informed about the contract creation and archival, in addition to the signatories.
   ...
   {/* https://lucid.app/lucidchart/31888b88-d836-457d-a4a8-05e3e161e07f/edit */}![The structure of a **Create** node.](explanations/ledger-model/images/create-node.svg)
2. An **Exercise** node records the parameters of a choice that one or more parties have exercised on a contract.
   It contains the following pieces of information:
   ...
   * The **exercise result** as the Daml value returned by evaluating the choice body.
   ...
   {/* https://lucid.app/lucidchart/ce3c7eb2-081e-4ac4-af92-5efc11d21c17/edit */}![The structure of an **Exercise** node.](explanations/ledger-model/images/exercise-node.svg)
3. A **Fetch** node on a contract, which demonstrates that the contract exists and is active at the time of fetching.
   ...
   **contract ID**, **interface ID**, **template ID**, and the **actors**, namely the parties who fetch the contract.
   ...
   {/* https://lucid.app/lucidchart/27844d5e-0cdb-4f22-8f67-e97f3839e613/edit */}![The structure of a **Fetch** node.](explanations/ledger-model/images/fetch-node.svg)
...
{/* https://lucid.app/lucidchart/556f7b43-565f-4b94-b670-719135a77bec/edit */}![The accept action on Alice's ``ProposeSimpleDvP`` exercised by Bob.](explanations/ledger-model/images/dvp-propose-accept-action.svg)
...
{/* https://lucid.app/lucidchart/fdcc5894-e013-499e-ba85-de16300381a8/edit */}![The accept-and-settle action on Alice's ``ProposeSimpleDvP`` exercised by Bob.](explanations/ledger-model/images/dvp-propose-accept-and-settle-action.svg)

<a id="da-ledger-subaction"></a>
...
{/* https://lucid.app/lucidchart/dbe05602-46b8-4b23-8957-a8e15af912e6/edit */}![The subactions of Bob exercising the ``Settle` choice on the DvP contract.](explanations/ledger-model/images/dvp-settle-subactions.svg)
...
That is, for a transaction tx = act<sub>1</sub>, …, act<sub>n</sub>, every act<sub>i</sub> is a root action.
...
{/* https://lucid.app/lucidchart/acb71942-2a11-417c-ae0a-003c8ea2da69/edit */}![A transaction with two top-level actions where Bob accepts two DvP proposal, one from Alice and one from Charlie.](explanations/ledger-model/images/dvp-accept-two.svg)
...
{/* https://lucid.app/lucidchart/b8f2c6d1-654b-4658-adc5-77eb59e27d05/edit */}![The consequences of the ``Settle`` action are a transaction of two actions, namely the two ``Transfer`` legs of the DvP.](explanations/ledger-model/images/dvp-settle-consequences-are-transactions.svg)
...
{/* https://lucid.app/lucidchart/c5ff472e-3161-42a0-ac2d-275774a2b9b8/edit */}![All proper subtransactions of the consequences of the ``Settle`` action.](explanations/ledger-model/images/dvp-settle-consequences-subtransactions.svg)

The [privacy model](ledger-privacy.mdx#da-model-privacy) uses the concept of subtransactions to define the visibility rules.

<a id="da-ledger-input-output"></a>
...
Due to the [privacy model](ledger-privacy.mdx#da-model-privacy), not everyone sees all parts of a party interaction.
...
We say that commit c<sub>1</sub> *happens before* c<sub>2</sub> if and only if the ledger contains a non-empty path from c<sub>1</sub> to c<sub>2</sub>,
or equivalently, the transitive closure of the graph contains an edge from c<sub>1</sub> to c<sub>2</sub>.

<Note>The [integrity conditions](ledger-integrity.mdx#da-model-integrity) on a ledger require that the happens-before order respects the lifecycle of contracts.
...
For the next few sections, we will consider only ledgers that meet these conditions.</Note>
...
{/* https://lucid.app/lucidchart/3ef6e9da-b534-4640-bc19-8fa5c7fb3a71/edit */}![The sequence of commits for the whole DvP workflow. First, banks 1 and 2 issue the assets, then Alice proposes the DvP, and finally Bob accepts and settles it.](explanations/ledger-model/images/dvp-ledger.svg)
<Note>The integrity constraints do not impose an order between independent commits.
...
We discuss the more general partial orders in the [causality section](local-ledger.mdx#local-ledger).</Note>
