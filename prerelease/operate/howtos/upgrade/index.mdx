<!-- source: canton / canton/3.5/participant/howtos/upgrade/index.rst -->
...
As elaborated in the [versioning guide](../../reference/versioning.md#canton-versioning), new features, improvements
...
A Canton binary supports [multiple protocol versions](../../reference/versioning.md#protocol-version), and new protocol
versions are introduced in a backward-compatible way with a new binary
(see [version table](../../reference/versioning.md#release-version-to-protocol-version-table)). Therefore, any upgrade
...
: * **Please back up your data before any upgrade.**
  <br/>
    Note that the [order of the backups](../recover/backup-restore.md#order-of-backups) is important.
...
```none
@ participant.start()
    ERROR com.digitalasset.canton.integration.EnvironmentDefinition$$anon$3:UpgradingDocumentationIntegrationTest - failed to initialize participant: There are 5 pending migrations to get to database schema version 4. Currently on version 1.1. Please run `participant.db.migrate` to apply pending migrations
...
You can also [configure the migrations to be applied automatically](../operate/console/console.md#migrate-and-start-mode).
...
@ participant.synchronizers.list_connected()
    res5: Seq[ListConnectedSynchronizersResult] = Vector(
      ListConnectedSynchronizersResult(
        ...
        physicalSynchronizerId = testdomain::12204e2b7500...::34-0,
        healthy = true
      )
    )
```
...
@ participant.health.ping(participant)
    res6: Duration = 1106 milliseconds
```
...
A hard synchronizer connection upgrade can be performed using the [respective migration command](../../reference/console.md#repair-migrate-synchronizer).
...
[setting the resource limits](../../reference/console.md#resources-set-resource-limits) to 0 on all participants:
...
@ participant.synchronizers.list_connected()
    res8: Seq[ListConnectedSynchronizersResult] = Vector(
      ListConnectedSynchronizersResult(
        ...
        physicalSynchronizerId = newdomain::1220bfd6aeaf...::34-0,
        healthy = true
      )
    )
```
...
```none
@ participant.health.ping(participant)
    res10: Duration = 978 milliseconds
```
