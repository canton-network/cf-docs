...
For application developer configuration (Canton + DPM), see the [AppDev Configuration Reference](/devnet/appdev/reference/configuration-reference).
...
Custom bootstrap scripts run Canton Console commands at node startup. This section will be expanded in a future update. See the [Canton Console Scripting](/devnet/global-synchronizer/canton-console/scripting) page for script syntax and examples.
...
Validator components authenticate to each other and to external users through JWT tokens issued by an OpenID Connect (OIDC) provider. Full setup instructions are in the [deployment section](/devnet/global-synchronizer/deployment/authorization-setup). The key configuration secrets are:
