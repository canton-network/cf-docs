...
In this section, you will learn more about expressing complex logic in a functional language like Daml. Specifically, you'll learn about:

- Function signatures and functions
- Advanced control flow (`if...else`, folds, recursion, `when`)

<Note>
There is a project template `daml-intro-functional-101` for this section, containing the code snippets from it.
</Note>

## The Haskell connection

This section dives deeper into Daml's expression language, the part that allows you to write logic inside `do` blocks and function definitions. We will cover functions, control flow, and functional programming patterns. But we can only scratch the surface here. Daml borrows a lot of its language from [Haskell](https://www.haskell.org). If you want to dive deeper, or learn about specific aspects of the language you can refer to standard literature on Haskell. Some recommendations:
...
In Daml's type system there are two key concepts: Data types (which describe what data looks like) and Function types (which describe how functions transform data). In this section, you'll start with Function types. Function types in Daml can be spotted by looking for `->` which can be read as "maps to".
...
So far we've covered function definitions. Now let's explore control flow—how to direct program execution based on conditions. The simplest form is `case`, which is a powerful tool for branching.
...
With `case`, `if ... else`, `void` and `when`, you can express all branching. However, one additional feature you may want to learn is guards. They are not covered here, but can help avoid deeply nested `if ... else` blocks. Here's just one example. The Haskell sources at the beginning of this section cover this topic in more depth.
...
## Data structures for your functions

Now that you understand how to write functions and control their flow, it's time to learn how to structure the data those functions operate on. Daml provides a rich set of data types—from native types like `Party` and `Decimal`, to records, lists, and variants—that let you model complex domains precisely. The sections below show how to define, assemble, and manipulate these data structures.
...
Contract templates (covered in [Module 3: Contract Templates](/appdev/modules/m3-contract-templates)) specify the types of contracts that can be created on the ledger, and what data those contracts hold in their arguments.

In Daml Studio's script view, you can see the current ledger state displayed as tables: one table per template, with one row per contract of that type and one column per field in the arguments. This provides a useful way of thinking about templates—like tables in databases, where:
...
Throughout this section, you've already seen examples of native Daml types: `Party` for ledger participants, `Text` for strings, and `ContractId` for references to contracts. Here are those native types and more:
