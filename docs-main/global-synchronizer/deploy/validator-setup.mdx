...
description: "Onboard and set up a validator node on the Canton Network Global Synchronizer"
---
...
4.  Request an onboarding secret from your SV sponsor. On DevNet, you can do this yourself through an API call (refer to the [Docker setup](/docs-main/global-synchronizer/deploy/docker-setup) or [Kubernetes setup](/docs-main/global-synchronizer/deploy/k8s-setup) for details). On TestNet and MainNet your SV sponsor needs to provide you with this manually. Note that onboarding secrets are only valid for 48 hours and are one-time use, and self-generated DevNet secrets are only valid for 1 hour. If it expired, you need to request a new one.

5.  Deploy your node either using docker compose or Kubernetes. Refer to the [Docker setup](/docs-main/global-synchronizer/deploy/docker-setup) or [Kubernetes setup](/docs-main/global-synchronizer/deploy/k8s-setup) for information on how to choose between them. You will need to make sure that all IP traffic going from your validator to the SVs uses the egress IP you provided to your SV sponsor and you need to provide the onboarding secret.
...
for url in $($CURL <TRUSTED_SCAN_URL>/api/scan/v0/scans | jq -r '.scans[].scans[].publicUrl'); do
  echo -n "$url: "
...
```bash
(set -o pipefail
for url in $(curl -fsS -m 5 --connect-timeout 5 <TRUSTED_SCAN_URL>/api/scan/v0/dso-sequencers | jq -r '.domainSequencers[].sequencers[].url | sub("https://"; "")'); do
  echo -n "$url: "
...
The default configuration for both of these requires access to at least 2/3 of the SVs for each of scans and sequencers. You may, at your option and own risk, configure connection to a single trusted scan and sequencer as described in the [Kubernetes setup](/docs-main/global-synchronizer/deploy/k8s-setup), at the cost of losing BFT integrity guarantees.
