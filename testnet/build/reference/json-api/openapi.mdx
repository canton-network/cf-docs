...
This specification covers regular HTTP/S endpoints - for streaming endpoints (websockets) please navigate to [JSON Ledger API AsyncAPI definitions (websocket access)](asyncapi.md#reference-json-api-asyncapi).

Properties without type or of type: {} (any Json value) are Daml values as defined in the Daml template and formatted according to [Daml-LF JSON encoding](lf-value-specification.md#reference-json-lf-value-specification).
