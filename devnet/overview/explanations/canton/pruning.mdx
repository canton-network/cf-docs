...
#### NOTE
Pruning means that nodes can forget a prefix of the history, and instead store a state snapshot that summarizes
the history.
...
#### NOTE
The participant retains some of its historical data in a long-term store, called the
...
#### NOTE
Pruning a node at time t deletes the history stored on that node up to time t, but retains the node’s snapshot
at time t.
