...
This page gives reference information on choices. For information on the high-level structure of a choice, see [Overview: Template Structure](structure.mdx).
...
<Warning>You **must** make sure that the controller parties are observers (or signatories) of the contract, otherwise they cannot see the contract (and therefore cannot exercise the choice).</Warning>
...
- `with` keyword.
- Choice arguments are similar in structure to [Template Parameters](templates.mdx#daml-ref-template-parameters): a [record type](data-types.mdx#daml-ref-record-types).
- A choice argument can’t have the same name as any [parameter to the template](templates.mdx#daml-ref-template-parameters) the choice is in.
...
- The choice body contains `Update` expressions. For detail on this, see [Reference: Updates](updates.mdx).
