...
- **Ordering guarantees** — Each synchronizer provides its own total ordering. Cross-synchronizer transactions are coordinated by the Canton protocol but have higher latency than same-synchronizer transactions.
