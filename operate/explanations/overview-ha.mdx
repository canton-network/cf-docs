{/* source: canton / canton/3.4/participant/explanations/overview-ha.rst */}
...
{/* EDIT REQUIRED */}![image](explanations/images/ha/canton-components-for-ha.svg)
...
<Note>An application operating on behalf of a party cannot transparently failover from one participant node to another due to the difference in offsets emitted on each participant.</Note>
...
Canton components expose a [health endpoint](../howtos/observe/monitoring.mdx#health-check), for checking the health of the components and their subcomponents.
