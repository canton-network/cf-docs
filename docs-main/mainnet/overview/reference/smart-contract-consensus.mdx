...
This page specifies the validation logic, confirmation policies, and security properties of the smart contract consensus layer. For transaction ordering and sequencer mechanics, see [Ordering Consensus](/mainnet/overview/reference/ordering-consensus). For the end-to-end message flow, see [Transaction Lifecycle](/mainnet/overview/reference/transaction-lifecycle).
