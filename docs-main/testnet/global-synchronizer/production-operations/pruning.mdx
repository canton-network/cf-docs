...
When sequencer pruning is enabled, you must keep historical backups for each pruning window. The time difference between two historical backups must be smaller than the `retentionPeriod`. See the [Backup and Recovery](/testnet/global-synchronizer/production-operations/backup-and-recovery) page for full backup requirements.
