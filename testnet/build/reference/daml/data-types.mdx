...
To sum a list, use a *fold* (because there are no loops in Daml). See [Fold](working-with.md#daml-ref-folding) for details.
...
the [Daml standard library](stdlib/index.md#stdlib-reference-base).
...
#### NOTE
An underscore was used in place of a variable name. The reason for this is that [Daml Studio](../../component-howtos/smart-contracts/daml-studio.md#daml-studio) produces a warning for all variables that are not being used. This is useful in detecting unused variables. You can suppress the warning by naming the variable with an initial underscore.
