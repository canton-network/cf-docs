...
- Node.js (v18+)
- A running Canton validator node (either self-hosted or via a node-as-a-service provider). Any network validator can be used -- [Splice LocalNet](/mainnet/sdks-tools/development-tools/localnet) is convenient for development and testing, but DevNet, TestNet, or MainNet validators work as well.
...
Community contributions include OIDC configurations for [Okta and Keycloak](/mainnet/global-synchronizer/deployment/oidc-providers).
