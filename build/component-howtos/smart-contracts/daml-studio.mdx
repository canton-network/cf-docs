{/* source: daml / daml/3.4/sdk/component-howtos/smart-contracts/daml-studio.rst */}
...
extension on top of [Visual Studio Code](https://code.visualstudio.com) (VS
Code), a cross-platform, open-source editor providing a
[rich code editing experience](https://code.visualstudio.com/docs/editor/editingevolved).

## Install

Install Daml Studio by [installing the DPM](../../dpm/dpm.mdx#dpm), and ensuring you
have [Visual Studio Code](https://code.visualstudio.com) version 1.87 or
above installed.
...
The options available are listed in the [Reference](#daml-studio-options).
...
provided in `build-options`, See [Daml Yaml Build Options](assistant/config-files.mdx#daml-yaml-build-options).
...
[Creating your own Snippets](https://code.visualstudio.com/docs/editor/userdefinedsnippets) to create an appropriate `daml.json`
snippet file.

<a id="script-results"></a>
...
specified in the relevant packages `daml.yaml` to give diagnostics for that file. See [Automatic SDK Installation](#daml-studio-automatic-sdk-installation)
...
<a id="daml-studio-jump-to-def"></a>
...
<a id="daml-studio-automatic-sdk-installation"></a>
...
for use with [environment variable interpolation support](assistant/config-files.mdx#id1). To make this work in Daml Studio,
...
recommend using [this direnv extension by Martin Kühl](https://marketplace.visualstudio.com/items?itemName=mkhl.direnv), which we have verified is compatible.
...
See [Daml Assistant Upgrade](assistant.mdx#daml-assistant-upgrade) and the `--replace` flag in [Reference](#daml-studio-cli-options) for more details.
...
Follow the [Daml Assistant Decomission](assistant.mdx#daml-assistant-decomission) for removal Daml itself.
...
- Issues with processes left behind, or read/write lock errors on build with Daml Studio open
  ...
  Please report this on the [GitHub Issues page](https://github.com/digital-asset/daml/issues), then disable `Multi-IDE` support as
  shown in the [Reference](#daml-studio-multi-ide-support).
- Any other issues

  It may help to read the Daml Studio output logs, and include them in a bug report on [GitHub Issues page](https://github.com/digital-asset/daml/issues).
...
- `Log Level`
  ...
  <a id="daml-studio-multi-ide-support"></a>
...
<a id="daml-studio-cli-options"></a>
...
- `--replace`
  ...
  - `always` will use the extension bundled with whichever SDK version is being used (as selected by the `daml.yaml`, See [Daml Assistant version management](assistant.mdx#daml-assistant-version-management).
