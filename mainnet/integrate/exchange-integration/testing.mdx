...
Automate the [exchange parties setup](node-operations.md#exchange-parties-setup) as part of your test setup,
...
* [Crash-fault tolerance](non-functionals.md#crash-fault-tolerance), in particular:
...
* [Retrying on RPC errors](non-functionals.md#retryable-errors), in particular:
...
* Does your integration code deal well with high rates of deposits and withdrawals.
  ...
  * Can [UTXO Selection and Management](workflows.md#utxo-management) deal with the case that there are no UTXOs available
    to fund a withdrawal.
  * Does your integration code rate limit executing transactions on the Validator Node
    to avoid running out of traffic with your [automatic traffic configuration](node-operations.md#reward-minting-and-traffic-funding).
    See the [Monitoring](node-operations.md#validator-node-monitoring) section for more information.
  * Does your [UTXO Selection and Management](workflows.md#utxo-management) code handle the case where there are only small UTXOs available,
...
* Does your integration code recover from data loss due to
  * [Restoring the Exchange Validator Node from a Backup](disaster-recovery.md#validator-backup-restore)
  * [Restoring the Canton Integration DB from a Backup](disaster-recovery.md#restore-canton-integration-db)
* Does your integration code handle [Rolling out Major Splice Upgrades](node-operations.md#hard-synchronizer-migration).
  Note that simulating a major Splice upgrade is not easily possible with LocalNet.
  We thus recommend to the check the [schedule for major Splice upgrades](node-operations.md#hard-synchronizer-migration)
