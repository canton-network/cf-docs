...
When using the Oracle JDK, you may get an error such as the following when executing transactions:
...
The Canton JAR embeds the BouncyCastle provider, and the Oracle JVM verifies the signature of third-party security providers. Download the [BouncyCastle provider JAR](https://mvnrepository.com/artifact/org.bouncycastle/bcprov-jdk18on) and place it ahead of the Canton JAR on the class path:

```shell
java -cp bcprov-jdk18on-<version>.jar:./lib/canton-open-source-<canton-version>.jar com.digitalasset.canton.CantonCommunityApp -c <config files...>
```
