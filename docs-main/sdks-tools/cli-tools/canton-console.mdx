...
**Participant node** -- The most commonly used reference. Handles party management, package uploads, ledger operations, and synchronizer connections.

```scala
participant.parties.list()
participant.dars.upload(darPath)
participant.domains.connect_local(synchronizer)
```
...
```
participant.<TAB>
  dars    domains    health    id    keys    parties    topology    ...
```
