...
Every SV includes the full validator stack. For a detailed breakdown, see the [Validator Node Components](/devnet/overview/reference/validator-node-components) reference.
...
For more on how ordering consensus works, see [Ordering Consensus](/devnet/overview/reference/ordering-consensus).
