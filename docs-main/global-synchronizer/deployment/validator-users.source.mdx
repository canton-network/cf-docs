...
3.  Obtain validator operator credentials for interacting with the validator's API. One way of achieving this is by inspecting the network activity logs of the wallet when logged in as the operator, and copying from there the Bearer token used for API requests. Save it in a TOKEN environment variable: `export TOKEN=<content of the token you obtained>`
4.  Obtain the PartyID of the validator operator. You can find that in the wallet UI when logged in as the operator, in the top right corner, next to the logout button. Save it in a PARTY_ID environment variable: `export PARTY_ID=<PartyID of the operator>`
...
<Snippet
  ...
  startAfter="ENABLEWALLET_START"
  endBefore="ENABLEWALLET_END"
  normalize="baseline"
  language="yaml"
/>

Since your wallet is disabled, your validator will not have funds to pay for traffic. You should therefore remove the validator top-up config to prevent its automation from trying to top up the traffic. See [helm_validator_topup](/global-synchronizer/deployment/validator-kubernetes) for more details.
