{/* source: splice-wallet-kernel / splice-wallet-kernel/devnet/src/exchange-integration/disaster-recovery.rst */}

<a id="disaster-recovery"></a>

# Backup and Restore

Recall that the [Integration Architecture](architecture.mdx#integration-architecture), shown in the diagram below, relies on two stateful components:
...
<a id="restore-from-validator-node-backup"></a>
...
As explained in the [Canton Integration Components](architecture.mdx#canton-integration-components) section of the [Integration Architecture](architecture.mdx#integration-architecture),
...
<a id="validator-backup-restore"></a>

## Restoring the Exchange Validator Node from a Backup

Follow the [Splice documentation on how to restore a validator node from a backup](https://docs.dev.sync.global/validator_operator/validator_disaster_recovery.html#restoring-a-validator-from-backups)
...
as described on the [Canton documentation page](https://docs.digitalasset.com/operate/3.3/howtos/recover/backup-restore.html#restore-caveats).

In the context of the recommended [Integration Workflows](workflows.mdx#integration-workflows), this data loss
affects:

* **.dar file uploads**: handle this by repeating the upload of all `.dar` files that were uploaded after the backup.
  This should be a rare event, as [token onboarding](workflows.mdx#token-onboarding) is infrequent.
...
5. Log into the [Canton Console of your validator node](https://docs.dev.sync.global/deployment/console_access.html)
...
<Note>These steps assume that record times assigned to transactions are unique,
...
or [repair commands](https://docs.digitalasset.com/operate/3.4/explanations/repairing.html).
...
>    have been ingested before Tx History Ingestion was stopped in Step 1.</Note>

<a id="restore-canton-integration-db"></a>
...
Likewise, the write in Step 3 of the [1-Step Withdrawal Workflow](workflows.mdx#one-step-withdrawal-workflow)
...
See Step 2 in the [1-Step Withdrawal Workflow](workflows.mdx#one-step-withdrawal-workflow) and
Step 2 in the [Multi-Step Withdrawal Workflow](workflows.mdx#multi-step-withdrawal-workflow) for details.
