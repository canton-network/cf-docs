{/* source: daml / daml/3.4/sdk/sdlc-howtos/smart-contracts/build/how-to-build-dar-files.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="build-howto-build-dar-files"></a>
...
The `dpm new <package-name>` command provides pre-made templates for various package structures and tutorials, see [this DPM page](../../../dpm/dpm.mdx#dpm-operate) for more information.

<a id="multi-package-build"></a>
...
See [Environment Variable Interpolation](../../../component-howtos/smart-contracts/assistant/config-files.mdx#id1) for more information.
...
See [Daml Build Options](../../../component-howtos/smart-contracts/flags.mdx#daml-build-flags) for a full list of dpm build options, or run `dpm build --help`, which includes options for changing the LF version and configuring warnings. All of these options can also be provided via `build-options` above.
Consider reading [Recommended Build Options](../../../component-howtos/smart-contracts/assistant/config-files.mdx#id2) for our recommended set of warning flags.
...
Note the use of `data-dependencies` instead of the previously covered `dependencies` field, the latter is reserved for `daml-prim`, `daml-stdlib`, and the optional testing library daml-script.
Once added to the `daml.yaml`, modules from the dependency .dar can be imported from the modules of this package. In the event of collision between module names, either with this package or other dependencies, see [module-prefixes](../../../component-howtos/smart-contracts/assistant/config-files.mdx#daml-yaml-module-prefixes).
...
For example, by checking it into a `daml/vendored/<vendored-package-name>` directory in your repository. A good example is the Canton Network Token Standard test harness provided by splice here: [https://github.com/DACH-NY/canton-network-node/tree/main/token-standard/splice-token-standard-test](https://github.com/DACH-NY/canton-network-node/tree/main/token-standard/splice-token-standard-test).
