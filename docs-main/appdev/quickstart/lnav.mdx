...
Download `lnav` for your OS at the [lnav download page](https://lnav.org/downloads).
...
If you’re unfamiliar with lnav, read their [docs](https://docs.lnav.org/).

### Install Canton lnav format

The Canton Network has a custom `lnav` configuration to help you view and analyze Canton logs. The configuration is defined in JSON and lives in the open source [Splice repository](https://github.com/canton-network/splice). You can read the schema to understand available values and the construction of logs when debugging with `lnav` in CN applications.
...
`lnav` shortcuts can be found on [`lnav`’s hotkey reference page](https://docs.lnav.org/en/latest/hotkeys.html).
