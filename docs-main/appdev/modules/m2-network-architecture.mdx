...
description: "Understanding how Canton's network architecture differs from Ethereum"
---
...
| Aspect | Ethereum Node | Canton Validator |
|--------|---------------|------------------|
| **State stored** | All network state | Only hosted parties' state |
| **Queries answered** | Any query | Only queries for hosted parties |
| **Consensus role** | Validate all transactions | Validate only relevant transactions |
| **Scaling impact** | More nodes = more replication | More validators = more capacity |

### Consensus Layer

| Aspect | Ethereum | Canton |
|--------|----------|--------|
| **Who participates** | All validators | Only affected validators |
| **What's validated** | Full transaction | Only relevant views |
| **Visibility** | Full transaction visible | Encrypted to participants |
| **Finality** | Probabilistic → finalized | Deterministic |

### State Model

| Aspect | Ethereum | Canton |
|--------|----------|--------|
| **Global state** | Yes, on all nodes | No global state |
| **State visibility** | Public | Private to stakeholders |
| **State queries** | Any node answers | Only hosting validator answers |
| **Total supply visible** | Yes | Only if explicitly designed |
...
| Strategy | Ethereum | Canton |
|----------|----------|--------|
| **More nodes** | More replication | More capacity |
| **L2 solutions** | Move computation off-chain | Not needed the same way |
| **Sharding** | Complex to implement | Native via validators |
...
| **Third-party integrations** | Use Ledger API, not JSON-RPC |
...
<Card title="Start Building" icon="code" href="/docs-main/developer/m3-daml">
