{/* source: daml / daml/3.4/sdk/tutorials/smart-contracts/daml-scripts.rst */}
{/* Copyright (c) 2023 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="test-using-scripts"></a>

# Test templates using Daml scripts

In this section we test the `Token` model from [Basic contracts](contracts.mdx) using the [Daml Script](../../reference/glossary.mdx#daml-script) integration in [Daml Studio](../../component-howtos/smart-contracts/daml-studio.mdx#script-results). This includes:
...
<Tip>Remember that you can load all the code for this section into a folder called `intro-daml-scripts` by running `dpm new intro-daml-scripts  --template daml-intro-daml-scripts`</Tip>
{/* script_basics: */}
...
You’ll learn all about the syntax `Token with owner = alice` in [Data types](data.mdx).
...
- Against a real ledger. See the documentation for [Daml Script](../../reference/glossary.mdx#daml-script) for more information.
...
![Script results indicating that a token has been created for Alice.](tutorials/smart-contracts/images/daml-scripts/scenario_results1.png)
...
![The script view as a separate column, with a table that shows Alice's token. Full display explained immediately below.](tutorials/smart-contracts/images/daml-scripts/tabular_view1.png)
...
<a id="intro-2-failure"></a>

## Test for failure

In [Basic contracts](contracts.mdx) you learned that creating a `Token` requires the authority of its owner. In other words, it should not be possible for Alice to create a token for another party, e.g. Bob, or vice versa. A reasonable attempt to test that would be:
...
![Script failure message indicating that the script failed due to a missing authorization from Bob.](tutorials/smart-contracts/images/daml-scripts/failure.png)
...
![The script view as a separate column with table again. This time there is a column and a row for Bob, as well as Alice. Alice cannot see Bob's tokens and Bob cannot see Alice's tokens, but each can see their own tokens.](tutorials/smart-contracts/images/daml-scripts/failuretable.png)
...
Contracts are also archived whenever a [consuming choice](/daml/intro/transformations.html#choices-in-the-ledger-model) is exercised.

<Danger>Archive choices are present on all templates and cannot be removed.</Danger>
...
To `archiveCmd` the token Alice has created, you need the contract id. Retrieve the contract id from the ledger with the `<-` notation. How this works is discussed in [Add constraints to a contract](constraints.mdx).
...
![The script view as a separate column, with the ``Show Archived`` checkbox selected.](tutorials/smart-contracts/images/daml-scripts/archived.png)
...
![The transaction view with the transaction's information from the point of its creation to exercise.](tutorials/smart-contracts/images/daml-scripts/tx_graph.png)
...
3. In [Archive contracts](#archiving) you tested that Bob cannot archive Alice’s token. Can you guess why the submit fails? How can you find out why the submit fails?

> <Tip>Remember that in [Test for failure](#intro-2-failure) we saw a proper error message for a failing submit.</Tip>

## Next up

In [Build the Daml Archive (.dar) file](archive.mdx) you will build a Daml archive, that you can upload to a Canton participant node to interact with its ledger.
