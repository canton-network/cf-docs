{/* source: canton / canton/3.4/participant/howtos/optimize/performance.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}
<Note>This page is a work in progress. It may contain incomplete or incorrect information.</Note>
<Note>Ensure that the following performance tuning parameters (in ParticipantNodeParameterConfig or CantonParameters) are covered somewhere in the optimize group of docs
- startup parallelism
- timeouts
...
- warn if overloaded
- journal GC delay
- package metadata view</Note>

<a id="scaling-and-performance"></a>
...
<Note>Please note that we do have plans to remove the storage duplication between the sync service and the indexer. Ideally,
will be able to reduce the storage on the participant for this example from 5\*Y down to 3\*Y: once for the unencrypted created contract and twice for the two encrypted transaction views.</Note>
...
through a [set of console commands](../../reference/console.mdx#ledger-pruning-commands) and allows removal of past events and archived contracts
...
Further information can be found in Section [Scaling and Performance](#scaling-and-performance).
...
Please check the [Postgres persistence section](storage.mdx#postgres-performance-tuning) for tuning instructions.

<a id="performance-configuration"></a>
...
<a id="tuning-ledger-api-limits"></a>
...
<a id="tuning-stream-api-limits"></a>
...
<a id="size-of-connection-pools"></a>
...
up ([check the troubleshooting guide](../troubleshoot/troubleshooting_guide.mdx#how-to-diagnose-slow-db-queries)).

Detailed instructions on handling this issue can also be found in [Max Connection Settings](storage.mdx#max-connection-settings).

**Size of database task queue.** If you are seeing frequent `RejectedExecutionExceptions` when Canton queries the database,
increase the size of the task queue, as described in [Database task queue full](../troubleshoot/FAQ.mdx#database-task-queue-full). The rejection is otherwise
...
as described in Section [Java Virtual Machine Arguments](../configure/general/command_line.mdx#jvm-arguments).
...
<Warning>While replication can be turned off to try to obtain performance gains, it must **not** be disabled when running multiple nodes
for HA.</Warning>

<a id="caching-configuration"></a>
