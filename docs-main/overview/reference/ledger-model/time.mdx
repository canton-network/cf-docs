---
title: "Time"
description: "The ledger time and record time assigned to every transaction, the guarantees they carry, and how ledger time is assigned."
---

{/* COPIED_START source="docs-website:docs/replicated/daml/3.5/overview/explanations/ledger-model/time.rst" hash="4dd2e265" */}

The Daml language contains a function [getTime](/appdev/reference/daml-language-reference#gettime) which returns the “current time”.
However, the concept of a “current time” can be challenging in a distributed setting.

This document describes the detailed semantics of time on Canton ledgers,
...
It is a timestamp that defines the value of all [getTime](/appdev/reference/daml-language-reference#gettime) calls in the given transaction,
...
1. **Causal monotonicity**: for any action (create, exercise, fetch, lookup) in `TX`
   on a contract `C`, `lt_TX >= lt_C`,
   where `lt_C` is the ledger time of the transaction that created `C`.

1. **Bounded skew**: `rt_TX - skew_min <= lt_TX <= rt_TX + skew_max`,
...
Daml applications should not interpret the value returned by [getTime](/appdev/reference/daml-language-reference#gettime) as a precise timestamp.
...
1. When submitting commands over the Ledger API,
...
1. The ledger time is set to the highest of the following values:

   1. `max(lt_C_1, ..., lt_C_n)`, the maximum ledger time of all contracts used by the given transaction
   1. `t_p`, the local time on the participant
   1. `t_p + min_ledger_time_rel`, if `min_ledger_time_rel` is given
   1. `min_ledger_time_abs`, if `min_ledger_time_abs` is given

1. Since the set of commands used by a given transaction can depend on the chosen time,
   the above process might need to be repeated until a suitable ledger time is found.

1. If no suitable ledger time is found after 3 iterations, the submission is rejected.
...
1. At this point, the ledger time may lie in the future (e.g., if a large value for `min_ledger_time_rel` was given).
