...
description: "Use external cryptographic keys to sign Canton transactions - overview, onboarding, and submission"
---
...
- [Use the Ledger API to onboard a party using an external key to sign transactions](/appdev/deep-dives/external-signing-onboarding)
- [Use the Ledger API to onboard a multi-hosted external party](/appdev/deep-dives/external-signing-onboarding#onboard-multi-hosted-external-party)
- [Create a contract using external signing](/appdev/deep-dives/external-signing-transactions-part-1)
- [Exercise a choice on a contract using external signing](/appdev/deep-dives/external-signing-transactions-part-2)
...
- [Build, sign and submit topology transactions](/appdev/deep-dives/external-signing-topology)

## Context

The ledger state in Canton is defined by **contracts owned by parties**. Each contract outlines the rights of different parties to unilaterally change the shared ledger. From a party's perspective, there are two key activities: **initiating transactions** and **validating transactions**.

A **party** is a logical actor on the ledger. While a party is an abstract concept, its on-ledger representation and state management are delegated to one or more **validators** of its choice. Because of Canton's privacy properties, only these validators are aware of the full set of contracts owned by a party. This means only they can authoritatively validate and confirm transactions that affect the party's contracts.

Transactions may address only actively hosted parties. Transactions referring to invalid parties will be rejected by the system.

### Topology and Identities

Identities of parties, validators and synchronizers in Canton are expressed as [**unique identifiers**](/overview/reference/topology#unique-identifiers), where each identifier is a pair of a name and a fingerprint of a public key `<prefix>::<fingerprint>`, as explained in the topology management overview. The public key is used to ultimately verify authorizations with respect to identities, as explained below.
...
The topology state can evolve. A party is not tied to its initial configuration. However, replicating parties across nodes is an [operational procedure between validator operators](/global-synchronizer/production-operations/party-management#party-replication), not covered in this section.
...
Please continue with the following tutorial to learn how to [onboard an external party using the Ledger API](/appdev/deep-dives/external-signing-onboarding).

Internal parties are covered in the [party management documentation](/global-synchronizer/production-operations/party-management).
...
Please follow the following tutorial to learn how to [setup a multi-hosted party using the Ledger API](/appdev/deep-dives/external-signing-onboarding#onboard-multi-hosted-external-party)
...
Note that the hosting choices have a significant impact on the security and availability of the party. Please refer to the trust assumptions outlined in the [party trust model](/overview/reference/external-party#trust-model).
...
- [Onboard a multi-hosted external party](/appdev/deep-dives/external-signing-onboarding#onboard-multi-hosted-external-party)
- [Build, sign and submit topology transactions](/appdev/deep-dives/external-signing-topology)
...
Please refer to [Local and external parties](/overview/reference/external-party#submission-flow) for a more detailed explanation.
...
How to submit commands using an external signing key is covered in the following tutorials: [Create a contract using external signing](/appdev/deep-dives/external-signing-transactions-part-1); [Exercise a choice on a contract using external signing](/appdev/deep-dives/external-signing-transactions-part-2)
