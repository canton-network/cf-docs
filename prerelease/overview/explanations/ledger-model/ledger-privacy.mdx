<!-- source: daml / daml/3.5/overview/explanations/ledger-model/ledger-privacy.rst -->
...
The [ledger structure section](ledger-structure.md#ledger-structure) answered the question “What does the Ledger looks like?” by introducing a hierarchical format to record the party interactions as changes.
...
To illustrate the concept of informees, we use the [running example of Alice and Bob swapping their assets](ledger-structure.md#ledger-structure-running-example).
...
as mentioned in the [ledger structure section](ledger-structure.md#actions-hierarchical-structure);
...
For example, the diagram below extends the [subaction diagram](ledger-structure.md#da-ledger-subaction) with the informees shown in the top right corner of the borderless box for each subaction.
...
We defer defining the edges in the projection to the [causality section](local-ledger.md#local-ledger).
...
* **Retroactive divulgence** refers to an input contract being shown to the non-informee witnesses of a node using this contract.
  ...
  (see [Consistency](ledger-integrity.md#da-model-consistency) for ledger integrity).
...
Recall from the [running example](ledger-structure.md#ledger-structure-running-example)
