{/* source: pqs / pqs/3.4/component-howtos/pqs/troubleshoot/runbooks.rst */}
...
configuration [warning](../configure.mdx#pqs-configure-warning).
...
If all else fails, get your hands on a thread dump (for example, through [diagnostics](../observe.mdx#pqs-diagnostics)) and
...
Running logging at `DEBUG` [level](../observe.mdx#pqs-logging) (`--logger-level=Debug`) will provide more detailed
...
[warning](../configure.mdx#pqs-references-configuration-options-transactions-data-source-warning).
...
* purge the PQS database and re-ingest the data either from `Genesis` or from `Latest`
  (see [Ledger streaming & recovery](../recover.mdx#pqs-ledger-streaming-and-recovery)), or
* perform a reset from a particular offset (before the offset of the first event with the new party involved) by
  following the [instructions](../operate.mdx#pqs-resetting)
...
* [History slicing](../operate.mdx#pqs-history-slicing)
* [Ledger streaming & recovery](../recover.mdx#pqs-ledger-streaming-and-recovery)
...
[Recover](../recover.mdx#pqs-recover)).
...
consumption. It is **unadvisable** to do so (see also [High availability](../recover.mdx#pqs-high-availability)).
...
PQS Docker images are published with the preconfigured Prometheus endpoint (see [Observe](../observe.mdx#pqs-observe)). By default,
it listens on `0.0.0.0:9090`, so it is expected to be hooked into existing monitoring infrastructure.

To that extent, PQS releases are accompanied by [dashboards](../observe.mdx#pqs-grafana-dashboard) that can be imported into a
...
Suggested areas of attention (see also [Optimize](../optimize/index.mdx#pqs-optimize)):
...
PQS [Dashboard](../observe.mdx#pqs-grafana-dashboard) is designed to read from top to bottom. It provides information from
...
memory-related [settings](../optimize/index.mdx#pqs-optimize-java-virtual-machine-jvm-configuration) are applied.
