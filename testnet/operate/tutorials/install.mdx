...
You can also use the Daml Enterprise Canton Docker images by following our [Docker instructions](../howtos/install/docker.md#install-with-docker).
...
For this purpose, there are some storage [mixin configurations](../howtos/configure/general/conf_file.md#configuration-mixin) (`config/storage/`) defined.
...
#### NOTE
Please also consult the more [detailed section on storage configurations](../howtos/configure/storage/storage.md#storage-config).
...
the [Postgres performance guide](../howtos/optimize/storage.md#postgres-performance-tuning) for further information.
...
associated with the node’s identity. The Ledger API supports [JWT based authentication](../howtos/secure/apis/jwt.md#ledger-api-jwt-configuration).
On the Admin API, you can enable mTLS. Please consult the [TLS documentation section](../howtos/secure/apis/tls.md#tls-configuration) for
further information.
...
various command line options available, for example to further tune the [logging configuration](../howtos/observe/monitoring.md#logging).

#### NOTE
By default, the node will initialize itself automatically using the identity commands [Topology Administration](../reference/console.md#identity-commands).
As a result, the node will create the necessary keys and topology transactions and will initialize itself using
the name used in the configuration file. Please consult the [identity management section](../howtos/operate/identity_management.md#identity-management-user-manual)
for further information.
...
2. All nodes are managed through the administration API. Whenever you use the console, almost all requests will
   ...
   the [TLS documentation section](../howtos/secure/apis/tls.md#tls-configuration).
3. Applications and users interact with the Participant Node using the gRPC Ledger API. We recommend that you secure your
   API by using TLS. You should also [authorize your clients using JWT tokens](../howtos/secure/apis/jwt.md#ledger-api-jwt-configuration).
...
* [Static configuration](../howtos/configure/general/conf_file.md#static-configuration) are items which are not supposed to change and are therefore captured in the configuration file.
...
* Dynamic configuration are items such as Daml archives (DARs), synchronizer connections, or parties. All such changes are effected
  through [console commands](../howtos/operate/console/console.md#canton-console) (or the [administration APIs](../howtos/operate/console/console.md#administration-apis)).
...
The final step is to connect the participant to the synchronizer. Refer to [the connectivity guide](../howtos/operate/synchronizers/connectivity.md#synchronizer-connections)
...
2. The administration API should be secured using client certificates as described in [TLS documentation section](../howtos/secure/apis/tls.md#tls-configuration).
