...
Keys guarantee **uniqueness** — only one active contract can exist per key value. They support **lookup** so you can find a contract by its key without knowing the contract ID. Every key has a **maintainer**, the party responsible for ensuring uniqueness.
