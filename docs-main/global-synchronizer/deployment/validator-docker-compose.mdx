...
## Wallet Gateway and Portfolio UI

Adding the `-g` flag to the `start.sh` command also deploys the [Wallet Gateway](/integrations/wallet-gateway/overview) and the [Splice Portfolio](/sdks-tools/development-tools/splice-portfolio/overview) alongside the validator:

```bash
./start.sh -s "<SPONSOR_SV_URL>" -o "<ONBOARDING_SECRET>" -p "<party_hint>" -m "<MIGRATION_ID>" -w -g
```

The gateway is accessible at [http://walletgateway.localhost](http://walletgateway.localhost) and the portfolio at [http://portfolio.localhost](http://portfolio.localhost). The gateway talks to the ledger API of the validator's participant, and the portfolio to the validator API and to the Scan instance passed to `start.sh`, which it uses as its token registry. The versions of both are pinned in the `.env` file as `WALLET_GATEWAY_VERSION` and `PORTFOLIO_VERSION`.

The gateway allocates parties for existing ledger users, but does not create users itself. A user therefore has to [log in to the wallet UI](#logging-into-the-wallet-ui) and onboard there once before logging in to the gateway. Parties created through the gateway are additional parties hosted on the validator's participant, next to the user's wallet party.

To use the portfolio, click **Connect Wallet**, enter `http://walletgateway.localhost/api/v0/dapp` as a custom wallet URL in the wallet picker, and log in to the gateway with the `Splice validator` network. Without authentication, the gateway issues self-signed tokens, and its login form asks for the name of a user onboarded via the wallet UI as the **Client ID** and `unsafe` as the **Client Secret**. Once connected, create a party on the gateway's **Parties** page and mark it as primary, as described in [Party management](/integrations/wallet-gateway/use/party-management). The portfolio then lists that party, and on DevNet you can [tap the faucet](/sdks-tools/development-tools/splice-portfolio/devnet-faucet) for it from the portfolio's **Settings** page.

<Note>
The portfolio calls the validator API with the tokens issued by the gateway, so `LEDGER_API_AUTH_AUDIENCE` and `VALIDATOR_AUTH_AUDIENCE` in the `.env` file must be equal. `start.sh` refuses to start with `-g` otherwise.

The gateway keeps its state in databases of its own in the `postgres-splice` container, separate for deployments with and without authentication, so a gateway started with `-a` does not see the parties created without it.
</Note>
...
| WALLET_GATEWAY_UI_CLIENT_ID | The client id of the OAuth app for the wallet gateway. Required only with `-g`. |
