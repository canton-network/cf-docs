...
<Accordion title="How do I tap CC on DevNet?">
**DevNet provides faucet functionality** for obtaining test Canton Coin:
...
<Warning>
Tap only works on DevNet. `AmuletRules` requires the `isDevNet` flag to be `true`, and it is `false` on TestNet, so calling Tap there fails with `UNHANDLED_EXCEPTION/DA.Exception.AssertionFailed The requirement 'isDevNet flag is true' was not met`.
</Warning>

<Info>
Test CC has no real value and is only for testing purposes on DevNet.
</Info>
</Accordion>
