---
title: "Deploying a Dedicated Synchronizer"
description: "Step-by-step guide for deploying sequencer and mediator nodes for a dedicated synchronizer"
---
...
A dedicated synchronizer requires deploying sequencer and mediator nodes, configuring their database backends, and connecting validators. This guide covers the infrastructure setup for both standalone and hybrid deployments.
...
- **Centralized ordering** — A single ordering node acts as the ordering backend. Simpler to operate, suitable for dedicated synchronizers where a single operator manages the infrastructure. Currently in **Alpha**.
...
For most dedicated synchronizer deployments where a single organization controls the infrastructure, the centralized ordering backend is the simpler choice.
...
Confirm end-to-end functionality by allocating a test party and creating a contract on the dedicated synchronizer. Check the sequencer health endpoint (HTTP 200 means ready) and verify the mediator is registered in the synchronizer topology.
