...
3.  Call `/v2/commands/submit-and-wait` on the Ledger [API]() to create a `#splice-wallet:Splice.Wallet.TransferPreapproval:TransferPreapprovalProposal` ([code]()) directly with the `provider` set to your `exchangeParty`.
    ...
    {/* DIVERGENCE: updated locally to reflect CIP-0096 (validator liveness rewards discontinued, $0 cap effective 2026-04-30); upstream splice-wallet-kernel source (docs/wallet-integration-guide/src/exchange-integration/index.rst, hash 07f260be) still describes liveness rewards as an active mechanism. See cf-docs issue #1049. */}

    Note that setting up this transfer preapproval requires the `exchangeParty` to pay a small fee of about 0.25 \$ worth of CC. On DevNet or LocalNet, just "Tap" the required funds from the built-in faucet. On TestNet or MainNet, since [CIP-0096](https://github.com/canton-foundation/cips/blob/main/cip-0096/cip-0096.md) discontinued the validator liveness rewards that previously funded new nodes automatically, transfer CC to the `exchangeParty` from an existing balance before setting up the preapproval.
...
## Rolling out Protocol Upgrades

For non-compatible protocol changes, the global synchronizer undergoes a Logical Synchronizer Upgrade (LSU). The schedule for these upgrades is published by the Super Validators and also announced in the `#validator-operations` slack channel.

As part of an LSU, the synchronizer is upgraded in a coordinated way that preserves state. For a more detailed overview, refer to the [validator upgrade docs](/global-synchronizer/production-operations/logical-synchronizer-upgrade).

We recommend running nodes on DevNet and TestNet so you can observe the procedure before you encounter it on MainNet.

From an integration perspective, there are a few things to keep in mind:

1.  A protocol upgrade only preserves the active contracts but not the full update history. In particular, you will not be able to get transactions from before the upgrade on the update service on the Ledger API.
