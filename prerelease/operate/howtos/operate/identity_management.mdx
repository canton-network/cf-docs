<!-- source: canton / canton/3.5/participant/howtos/operate/identity_management.rst -->
...
of the manual; the related Ledger API authorization configuration is covered in the [Ledger API JWT configuration section](../secure/apis/jwt.md#ledger-api-jwt-configuration).
...
You can manage users through the [Canton console user management commands](../../reference/console.md#ledger-api-users-list), an alpha feature.
...
@ val rootCert = client.topology.namespace_delegations.propose_delegation(Namespace(secret.fingerprint), secret, CanSignAllMappings)
    rootCert : SignedTopologyTransaction[TopologyChangeOp, NamespaceDelegation] = SignedTopologyTransaction(
      TopologyTransaction(
        ...
        serial = 1,
        operation = Replace,
        hash = SHA-256:d9360bd56eb9...
      ),
      signatures = 1220b6d52a3f...
    )
```
...
@ val partyId = PartyId.tryCreate("Client", secret.fingerprint)
    partyId : PartyId = Client::1220b6d52a3f...
```
...
[Hosting a party across multiple participants](parties/parties.md#multi-hosted-parties)
...
[External Key Storage with a Key Management Service (KMS)](../secure/kms/mode/external_key_storage.md#external-key-storage) for more details.
