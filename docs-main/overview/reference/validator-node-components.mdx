...
flowchart TB
    ...
    subgraph SYNC["Synchronizer Connections"]
        GS[Global Synchronizer<br>sequencer &#8226; mediator]
...
- **Confirmation**: participants send approval or rejection responses to the mediator
- **Result**: the mediator aggregates responses and declares commit or rollback; participants apply the outcome

For a full description of these steps, see [Transaction Lifecycle](/docs-main/overview/reference/transaction-lifecycle).
...
The JSON API is an HTTP and WebSocket wrapper around the Ledger API. It translates JSON requests into gRPC calls and returns JSON responses. It is optional for validator deployments -- many applications use the gRPC Ledger API directly. The JSON API endpoint is typically exposed at `/api/json-api` when configured through ingress.
...
- **Topology store** -- identity mappings, key registrations, and party-to-participant assignments (see [Topology](/docs-main/overview/reference/topology))
...
A validator connects to one or more synchronizers. Each connection involves authenticated channels to the synchronizer's sequencer and mediator nodes.
...
<Card title="Transaction Lifecycle" icon="arrows-spin" href="/docs-main/overview/reference/transaction-lifecycle">
  How the Canton protocol processes a transaction from submission to commit.
</Card>

<Card title="Topology" icon="sitemap" href="/docs-main/overview/reference/topology">
  Identity management, key registrations, and party-to-participant mappings.
</Card>

<Card title="Super Validator Components" icon="server" href="/docs-main/overview/reference/super-validator-components">
  Additional components that Super Validators operate beyond a standard validator.
</Card>

<Card title="Canton Coin Tokenomics" icon="coins" href="/docs-main/overview/reference/canton-coin-tokenomics">
