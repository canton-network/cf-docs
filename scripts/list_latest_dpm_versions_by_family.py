...
"""List latest Daml SDK versions from dpm by major.minor family."""
...
# - stable:   3.4.11
# - rc:       3.4.0-rc2
# - snapshot: 3.5.0-snapshot.20260216.14584.0.vb8bd367d
...
def version_sort_key(version: str) -> tuple[Any, ...]:
    if m := SNAPSHOT_VERSION_RE.fullmatch(version):
        ...
        # snapshot < rc < stable within same patch
...
def select_versions(
    ...
    # De-dup while preserving highest rank selection later.
...
def main() -> int:
    ...
    parser.add_argument(
        "count_per_family",
        help="How many latest versions to select per family.",
    )
    parser.add_argument(
        "--families",
        default="3.4,3.3,3.2",
