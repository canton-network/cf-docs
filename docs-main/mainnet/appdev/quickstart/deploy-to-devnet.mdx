...
`quickstart-project-structure-guide`.
...
document, the most up to date version is 0.5.18.
...
The following link downloads the `0.5.18 splice-node` release. Move the
...
[https://github.com/digital-asset/decentralized-canton-sync/releases/download/v0.5.18/0.5.18_splice-node.tar.gz](https://github.com/digital-asset/decentralized-canton-sync/releases/download/v0.5.18/0.5.18_splice-node.tar.gz)

Move the `splice-node` tarball to your desired location and then unzip
it.

``` bash
tar xzvf 0.5.18_splice-node.tar.gz
```
...
saving the most recent version. e.g. `SPLICE_VERSION=0.5.18`
