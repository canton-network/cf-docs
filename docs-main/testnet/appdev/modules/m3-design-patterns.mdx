...
The project both changes and adds to the `Iou` model presented in [Authorization](/testnet/appdev/modules/m3-authorization):
...
This project showcases how you can put the `Update` and `Script` actions you learned about in [Authorization](/testnet/appdev/modules/m3-authorization) to good use. For example, the `Merge` and `Split` choices each perform several actions in their consequences.
...
Blinding
On ledgers with strong privacy, projections (see [Privacy Model](/testnet/overview/learn/privacy-model)) for all involved parties are created. This is also called *projecting*.
