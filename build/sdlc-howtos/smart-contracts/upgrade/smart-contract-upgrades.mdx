{/* source: daml / daml/3.4/sdk/sdlc-howtos/smart-contracts/upgrade/smart-contract-upgrades.rst */}
{/* Copyright (c) 2024 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}
<Note>This page is a work in progress. It may contain incomplete or incorrect information.</Note>

<a id="smart-contract-upgrades"></a>
...
| **Daml Model<br/>Change**                                         | **Scope**            | **Covered<br/>by SCU?**               | **Comments**                                                                                                                                                                                                                                                                                                                                         |
|-------------------------------------------------------------------|----------------------|---------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| Change of Signatories                                             | Template Definition  | No                                    | A new template version needs to return the same signatory result for a persisted contract, otherwise the contracts cannot be used by an upgraded package (this fails at runtime).<br/><br/><br/><br/><br/>One can change the function that computes the signatories (for example to take into account new fields) when creating contracts.<br/><br/> |
| Change of                                                         | Template             | No                                    | The key’s type cannot be changed, like adding a new field in the record it contains.<br/><br/><br/><br/><br/>One can change the function that computes the key in the same way the function computing the signatories can be changed. A persisted contract can be used by an upgrade only if the result of the function does not change.<br/><br/>   |
...
| Change of Choice Return Type                                      | Choice               | Yes, except for Tuple or scalar types | You can change a user data type (Record, Variant, Enum) contained in the return type. In this case you can add a field to the record or a new case to the Variant/Enum.<br/><br/><br/><br/><br/>In particular: you cannot change a collection, Optional, List, or Map; you can change Tuple, (to a triple) if you define your own type.<br/><br/>    |
...
other restrictions covered in [Continuing to Write Your Upgrades](#continuing-to-write-your-upgrades). This
approach is inspired by
[Protobuf](https://protobuf.dev/programming-guides/proto3/#updating)
and Typescript’s ability to ignore [excess
fields](https://www.typescriptlang.org/docs/handbook/2/objects.html#excess-property-checks)
via `as`.
...
This behavior can be influenced by [the package preference](#package-preference).
...
[the PQS section](#pqs) for more information and a work-around.
...
[Rolling out backwards-incompatible changes](#backwards-incompatible-changes).
...
We continue with the example introduced in [Smart Contract Upgrade Basics](#smart-contract-upgrade-basics). Begin by defining the first (old) version
of our package:
...
should contain a version identifier in its name as well, but we omit this - consult the section on [package naming best
...
the ledger - more on this [here](#upgrade-dont-upload-daml-script).
...
Upgrades](#continuing-to-write-your-upgrades) section.
...
[Continuing to Write Your Upgrades](#continuing-to-write-your-upgrades).
...
<Note>Once a DAR is successfully uploaded, it cannot be
...
all subsequent uploads must be compatible with this one as well).</Note>
<Note>Packages stored on the participant must
lead to unique package-id -> (package-name, package-version) relationships
since runtime package-name -> package-id
resolution must be deterministic (see [Ledger API](#ledger-api)). For this
...
rejection with error code KNOWN_DAR_VERSION.</Note>
...
Upgradable packages are also subject to package vetting
restrictions:
...
Upgrade](#writing-your-first-upgrade) section above, but overwrite
...
<a id="limitations-in-upgrading-variants"></a>
...
For this reason, it is [strongly recommended that interfaces always be defined
...
Following the [best practices](#upgrades-best-practices) outlined below and the [testing recommendations](#upgrades-testing) leads to a
proliferation of packages in your project.  Use [Multi-Package builds](../build/how-to-build-dar-files.mdx#multi-package-build) to reliably rebuild these packages as you iterate on your
project. Multi-package builds also enable cross-package navigation in Daml
Studio.

The [Multi-package builds for upgrades](#upgrades-multi-package) section
...
For the same reason, the [Daml sandbox](../../../component-howtos/application-development/dpm-sandbox.mdx#sandbox-manual) does not support
...
<a id="package-preference"></a>

### Package Preference

On processing a command submission, there are scenarios where the Ledger API server needs
to resolve by-package-name template IDs to by-package-id template IDs (see [Dynamic Package Resolution in Command Submission](#dynamic-package-resolution-in-command-submission)).
...
- The package-id selection preference list specified in the submitted command’s
  [package_id_selection_preference](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-commands-package-id-selection-preference) in a command submission.
  ...
  > - See [here](#daml-script-package-preference) for how to provide this in Daml-Script
  > - **Note:** The [package_id_selection_preference](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-commands-package-id-selection-preference)
...
<a id="dynamic-package-resolution-in-command-submission"></a>
...
- For command submissions that use a by-package-name template ID
  ...
  create command [here](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-createcommand))
...
In these situations, the [package preference](#package-preference) is used for
...
in the [template-id request filter field](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-templatefilter-template-id).
...
<a id="upgrades-best-practices"></a>
...
<a id="upgrades-separate-interfaces-and-exceptions"></a>
...
To manage the complexity of working with multiple packages at once, we recommend using [multi-package builds](../build/how-to-build-dar-files.mdx#multi-package-build).
...
<a id="upgrade-dont-upload-daml-script"></a>
...
<Note>Following these recommendations decouples the application’s behavior from the DAR uploads
(see [the note regarding Package Preference](#package-preference)),
as package resolution is fully determined by the explicit
and exhaustive package preference provided in command submissions.</Note>
...
<Note>Ledger API clients or participant nodes that do not finish the components rollout before the update switch-over time
may not be able to participate in the upgraded workflow.</Note>

<a id="backwards-incompatible-changes"></a>
...
- Introduce the target template as a new template following the [Breaking changes via Explicit Package Version](#upgrade-package-naming)
...
<Note>Rolling out backwards-incompatible changes as described in this section
incurs downtime on affected workflows until their contracts have been converted by the automation.
In order to not disrupt business, such rollouts should be executed during maintenance windows.</Note>
<Note>Note that this kind of upgrade requires O(number-of-active-contracts) of transactions to roll out.
Depending on the size of your ACS this can take a long time and consume significant compute and storage resources.
In contrast, the backwards-compatible upgrades can be rolled out with constant cost, independent of the size of your ACS.</Note>

<a id="upgrades-testing"></a>

## Testing

### Standalone Upgradeability Checks

<a id="upgrade-check-tool"></a>
...
prefer full [Workflow Testing](#workflow-testing).
...
ledger in future versions (Daml 3). More information about this limitation [here](#upgrade-dont-upload-daml-script).
...
- Run your existing test suite for V1 but updated to call V2 choices. This can be
  done with a rewrite, or by passing down a [package preference](#daml-script-package-preference)
...
- Assuming your data change affects a template payload, write separate setup code for V1 and V2, populating new fields
  ...
  <br/>
- Update your existing test suite from V1 to take a [package preference](#daml-script-package-preference),
...
[Multi-package builds](../build/how-to-build-dar-files.mdx#multi-package-build) help
...
<Note>If you still need to perform a query with an explicit package ID, you can either use
a previous version of PQS or add the following filter predicate to your query:
`SELECT \* FROM active('my_package:My.App:MyTemplate') WHERE package_id = 'my_package_id'`</Note>
...
<a id="daml-script-package-preference"></a>

**Daml Script Package Preference**

A submission can specify a [package preference](#package-preference)
