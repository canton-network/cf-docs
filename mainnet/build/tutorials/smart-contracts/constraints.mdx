...
To make sense of the latter, you’ll also learn more about the `Update` and `Script` types, and `do` blocks, which will be good preparation for [Compose choices](compose), where you will use `do` blocks to compose choices into complex transactions.
...
You can now specify a precise data and data-transformation model for Daml ledgers. In [Parties and authority](parties), you will learn how to properly involve multiple parties in contracts, how authority works in Daml, and how to build contract models with strong guarantees in contexts with mutually distrusting entities.
