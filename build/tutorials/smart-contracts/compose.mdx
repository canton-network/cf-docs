{/* source: daml / daml/3.4/sdk/tutorials/smart-contracts/compose.rst */}
...
<Tip>Remember that you can load all the code for this section into a folder called `intro-compose` by running `dpm new intro-compose  --template daml-intro-compose`</Tip>
...
The project both changes and adds to the `Iou` model presented in [Parties and authority](parties.mdx):
...
This project showcases how you can put the `Update` and `Script` actions you learned about in [Parties and authority](parties.mdx) to good use. For example, the `Merge` and `Split` choices each perform several actions in their consequences.
...
Blinding
: On ledgers with strong privacy, projections (see [Privacy](#privacy)) for all involved parties are created. This is also called *projecting*.
...
![The table as described above.](tutorials/smart-contracts/images/compose/divulgence.png)
...
In [Handle exceptions (Deprecated)](exceptions.mdx), we will learn about how errors in your model can be handled in Daml.
