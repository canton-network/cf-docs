...
{/* COPIED_START source="docs-website:docs/replicated/canton/3.5/participant/howtos/decommission/index.rst" hash="67c06dfe" */}
...
2.  Disconnect the participant to be decommissioned from all synchronizers as described in [enabling and disabling connections](/global-synchronizer/reference/canton-console-commands#synchronizers.disconnect_all).
3.  Use the `topology.participant_synchronizer_permissions.propose` command to fully unauthorize the participant on the synchronizer:
...
synchronizerOwners.foreach { synchronizerOwner =>
  synchronizerOwner.topology.participant_synchronizer_permissions
    ...
    .foreach(permission =>
      synchronizerOwner.topology.participant_synchronizer_permissions
        .propose(synchronizerId, participant2.id, permission = permission, change = TopologyChangeOp.Remove)
    )
}
```

Finally use the `repair.disable_member` command to disable the participant being decommissioned in all sequencers and remove any sequencer data associated with it.
