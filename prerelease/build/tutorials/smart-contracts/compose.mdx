<!-- source: daml / daml/3.5/sdk/tutorials/smart-contracts/compose.rst -->
...
The project both changes and adds to the `Iou` model presented in [Parties and authority](parties.md):
...
This project showcases how you can put the `Update` and `Script` actions you learned about in [Parties and authority](parties.md) to good use. For example, the `Merge` and `Split` choices each perform several actions in their consequences.
...
In [Handle exceptions (Deprecated)](exceptions.md), we will learn about how errors in your model can be handled in Daml.
