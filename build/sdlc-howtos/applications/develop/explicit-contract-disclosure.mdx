...
To deactivate it, configure `participants.participant.ledger-api.enable-explicit-disclosure = false`.</Note>
...
need to be properly authorized to execute the action.</Note>
...
<Note>The `created_event_blob` field in `CreatedEvent` (used to construct the [DisclosedContract](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-disclosedcontract))
is populated **only** on demand for `GetUpdates`, `GetUpdateTrees`, and `GetActiveContracts` streams.

To learn more, see [configuring event format](../../../explanations/ledger-api-services.mdx#event-format).</Note>
...
and cannot be used as disclosed contracts.</Note>
...
[Java Bindings StockExchange example project](https://github.com/digital-asset/ex-java-bindings/blob/f474ae83976b0ad197e2fabfce9842fb9b3de907/StockExchange/README.rst).</Note>
