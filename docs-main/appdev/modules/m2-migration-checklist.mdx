...
List everyone who interacts with your contracts:

| Ethereum Role | Canton Party | Notes |
|---------------|--------------|-------|
...
|----------|--------|-------|
| JSON-RPC | Ledger API (gRPC) | Different protocol |
| Web3.js / ethers.js | Ledger API client | Language-specific clients |
...
| Ethereum | Canton |
|----------|--------|
| Sign with private key | Party identity + JWT tokens |
| `msg.sender` from signature | Party from authentication |
| Anyone can call | Only authenticated parties |
...
- [ ] **Upload to validator**: Via admin API
...
| Ethereum | Canton |
|----------|--------|
| Block explorer | Validator logs + metrics |
| Transaction hash lookup | Transaction ID queries |
| Gas tracking | Traffic unit tracking |

### Key Management

| Ethereum | Canton |
|----------|--------|
| EOA private keys | Party keys (on validator or external) |
| Hardware wallets | HSMs, KMS integration |
| Multisig wallets | Multi-signatory contracts |
...
<Card title="Module 3: Daml Development" icon="code" href="/docs-main/appdev/modules/m3-dev-environment">
