{/* source: canton / canton/3.4/sdk/explanations/external-signing/external_signing_overview.rst */}
{/* Copyright (c) 2025 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="sdk-external-signing-overview"></a>
...
For an overview of external parties and how they differ from local parties, please read the corresponding overview section.
...
<Note>The PPN does not need to host the submitting party.</Note>
...
Instructions on how to compute the hash are available [here](external_signing_hashing_algorithm.mdx#external-signing-hashing-algo).

<Danger>This is `Alice`’s chance to validate the transaction and ensure it corresponds to her intent by inspecting its content.
Additional documentation on how to validate the transaction is forthcoming.</Danger>
...
<Note>The EPN *may* be the same physical node as the PPN, but does not have to be.</Note>
...
<Note>Once again, one of the CPNs may be the same physical node as either the PPN or EPN, or both, but this is not a requirement.</Note>
<Note>Currently, the completion event stream that contains events informing the client application on the outcome of a command
...
We intend to provide the completion event stream on CPNs as well in an upcoming Canton version.</Note>
<Danger>External parties entrust CPNs with validating transactions on their behalf.
See the Trust Model for the implications and methods to mitigate the risks due to this trust assumption.</Danger>

## Additional resources

### Tutorials

Refer to the [external party onboarding tutorial](../../tutorials/app-dev/external_signing_onboarding.mdx#tutorial-onboard-external-party) for a hands-on demonstration of onboarding an external party.

* [External Signing Hashing Algorithm](external_signing_hashing_algorithm.mdx)
