{/* source: splice-wallet-kernel / splice-wallet-kernel/devnet/src/exchange-integration/architecture.rst */}
...
* **Memos** are stored in the transfer metadata using the `splice.lfdecentralizedtrust.org/reason` key.
  The [Canton Network Token Standard](https://docs.dev.sync.global/app_dev/token_standard/index.html) defines this key and a way to parse these memo tags and other transfer information from transactions.
...
{/* https://lucid.app/lucidchart/2a048991-c76c-4a72-8622-66e837f6e1ec/edit?viewport_loc=-1983%2C-124%2C4710%2C2321%2C0_0&invitationId=inv_d2f23474-4e92-4b66-847a-0602e906795e */}![Integration architecture component overview](exchange-integration/images/component_diagram.png)
...
<a id="canton-integration-components"></a>

### Canton Integration Components

There are five Canton integration components:

* The **Exchange Validator Node** is a [Splice validator node](../integrating-with-canton-network/index.mdx#validator-nodes) that hosts
  your `treasuryParty`, which is the party you setup to control funds, receive deposits, and
  execute transfers for withdrawals. See [Setup Exchange Parties](node-operations.mdx#exchange-parties-setup) for details on how to setup the `treasuryParty`.
  You can [deploy and operate a validator yourself](https://docs.dev.sync.global/validator_operator/index.html#validators)
...
* The **Canton Integration DB** is used to keep track of the state of withdrawals and
  ...
  existing databases.
* The **Tx History Ingestion** service uses the [JSON Ledger API](https://docs.digitalasset.com/build/3.3/reference/json-api/json-api.html) exposed by the
...
As explained in the [High-Level Overview](#architecture-high-level-overview), you should be able to
...
in the [Integration Workflows](workflows.mdx#integration-workflows) section.
...
* The **Registry API Server** provides access to extra context to execute
  ...
  [Canton Network Token Standard definitions](https://github.com/hyperledger-labs/splice/tree/main/token-standard) in the Splice repository.

<a id="information-flows"></a>
...
{/* https://lucid.app/lucidchart/2a048991-c76c-4a72-8622-66e837f6e1ec/edit?viewport_loc=549%2C-1204%2C1947%2C960%2C6kMKYgEBcyuH&invitationId=inv_d2f23474-4e92-4b66-847a-0602e906795e */}![Information flow diagram](exchange-integration/images/information_flow_diagram.png)

There are three main information flows:

1. **Tx History Ingestion**: ingests the transactions (tx)
   ...
   Ledger API endpoint using either [plain HTTP](https://github.com/digital-asset/canton/blob/92339b6f98faaecbe3adbfb71293ed9cbfb30204/community/ledger/ledger-json-api/src/test/resources/json-api-docs/openapi.yaml#L786)
   or [websockets](https://github.com/digital-asset/canton/blob/92339b6f98faaecbe3adbfb71293ed9cbfb30204/community/ledger/ledger-json-api/src/test/resources/json-api-docs/asyncapi.yaml#L36).
...
We explain them in the [Integration Workflows](workflows.mdx#integration-workflows) section.
