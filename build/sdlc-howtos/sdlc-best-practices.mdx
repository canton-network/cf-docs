...
![Iterative feedback loop of design, development, and testing facets as described below.](sdlc-howtos/images/design-dev-test.png)
...
A Canton Network application may include [app user developed frontends and backends](system-design/daml-app-arch-design.mdx#arch-options). Lead development with early creation of Daml code as an interface description language for cross-organizational workflows.
...
![Testing pyramid as described below.](sdlc-howtos/images/testing-pyramid.png)
1. Unit Tests
...
3. End-to-End Tests
   ...
   * Use tools like [Selenium](https://www.selenium.dev/) or [Playwright](https://playwright.dev/) for browser session orchestration.
...
* The deployment topology depends on the [app architecture](system-design/daml-app-arch-design.mdx#build-sdlc-howtos-daml-app-arch-design) and requires a pre-configured Canton infrastructure, including app provider and app users on their respective administrative domains.
...
* [App provider](../overview/key_concepts.mdx#app-provider) should:
  * Deploy the backend and the frontend.
  * Configure the frontend to integrate with IAM.
* [App user](../overview/key_concepts.mdx#app-user) should:
...
* Balance deployment simplicity with [architectural trade-offs](system-design/daml-app-arch-design.mdx#properties-summary).
...
* Standard operational considerations for Canton Network applications include logging and monitoring.
  ...
  * Monitoring: Capture metrics for all components and display the golden signals – latency, traffic, errors, and saturation – on dashboards. Example dashboards for Canton components are available in [the documentation](https://docs.daml.com/canton/usermanual/monitoring.html?_gl=1*qdpp48*_gcl_au*MTQ0ODAwODc0MC4xNzM3NDQzODUw*_ga*NDg1MTgxODM0LjE3MjA2MjEzNDc.*_ga_GVK9ZHZSMR*MTczNzQ0Mzg1MC45OC4xLjE3Mzc0NDM5NjEuNjAuMC4w&_ga=2.19913016.1097062857.1737443850-485181834.1720621347#hands-on-with-the-daml-enterprise-observability-example).
