{/* source: daml / daml/3.4/overview/explanations/ledger-model/ledger-privacy.rst */}
{/* Copyright (c) 2023 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="da-model-privacy"></a>

# Privacy

<Note>This page is a work in progress. It may contain incomplete or incorrect information.</Note>

The [ledger structure section](ledger-structure.mdx#ledger-structure) answered the question “What does the Ledger looks like?” by introducing a hierarchical format to record the party interactions as changes.
...
<Note>Templates can declare `preconsuming` and `postconsuming` choices.
...
Accordingly, contract observers are only informees of the `Archive` subaction, but not of the main `Exercise` action itself.</Note>

To illustrate the concept of informees, we use the [running example of Alice and Bob swapping their assets](ledger-structure.mdx#ledger-structure-running-example).
...
{/* https://lucid.app/lucidchart/3176adad-0474-4755-bfb5-e323e1a65fab/edit */}![The informees of the nodes in the ``AcceptAndSettle`` action.](explanations/ledger-model/images/dvp-acceptandsettle-informees.svg)
...
as mentioned in the [ledger structure section](ledger-structure.mdx#actions-hierarchical-structure);
...
For example, the diagram below extends the [subaction diagram](ledger-structure.mdx#da-ledger-subaction) with the informees shown in the top right corner of the borderless box for each subaction.
...
{/* https://lucid.app/lucidchart/e5ee4a5f-3dda-4eec-a773-0ab3dde2c4ff/edit */}![The informees of the subactions of the ``Settle`` choice and the witnesses of the nodes.](explanations/ledger-model/images/dvp-settle-witness.svg)

<a id="da-model-projections"></a>
...
{/* https://lucid.app/lucidchart/9b3762db-66b4-4e72-94cb-bcefd4c1a5ea/edit */}![image](explanations/ledger-model/images/dvp-acceptandsettle-projection.svg)
...
<Note>Note the privacy implications of the banks’ projections.
...
providing privacy to Alice and Bob with respect to the banks.</Note>
...
<a id="da-model-ledger-projection"></a>
...
We defer defining the edges in the projection to the [causality section](local-ledger.mdx#local-ledger).
...
{/* https://lucid.app/lucidchart/90b7f155-aadc-4bde-9c5c-b8198b824384/edit */}![Time sequences for each party's projection, explained in-depth in the numbered list below.](explanations/ledger-model/images/dvp-ledger-projections.svg)
...
2. Bob’s projection is analogous to Alice’s:
...
3. Banks 1 and 2 only see the commits in which they create their `SimpleAsset` and the `Transfer` Exercises on them.
...
<Note>A user of a Participant Node can request the Ledger projection for the user’s parties via the
`:brokenref:`updates tree stream <com.daml.ledger.api.v2.UpdateService.GetUpdateTrees>``.</Note>
...
* **Retroactive divulgence** refers to an input contract being shown to the non-informee witnesses of a node using this contract.
  ...
  (see [Consistency](ledger-integrity.mdx#da-model-consistency) for ledger integrity).
...
Recall from the [running example](ledger-structure.mdx#ledger-structure-running-example)
