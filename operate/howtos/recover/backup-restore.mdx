{/* source: canton / canton/3.4/participant/howtos/recover/backup-restore.rst */}
...
- roll out a new synchronizer as a repair strategy in order to [recover from a lost synchronizer](repairing.mdx#recovering-from-lost-synchronizer)
...
<a id="backup-restore-private-key-rotation"></a>
...
When the key is stored in a [KMS](../secure/kms/kms.mdx#kms), the situation is simpler
...
[keys.secret.register_kms_encryption_key](../../reference/console.mdx#keys-secret-register-kms-encryption-key) or [keys.secret.register_kms_signing_key](../../reference/console.mdx#keys-secret-register-kms-signing-key).
...
* [Synchronizer connection configuration](../operate/synchronizers/connectivity.mdx#synchronizer-connections)
* [User management](../operate/users/users.mdx#user-management)
* [DAR upload](../operate/packages/packages.mdx#manage-daml-packages-and-archives)
* [Repairs](repairing.mdx#repairing-howto)
* [Party replication](../operate/parties/party_replication.mdx#party-replication)
...
<a id="database-replication-dr"></a>
...
[caveats as with backup restore](#restore-caveats) the participants should either use synchronous
...
e.g. [the high availability documentation](https://www.postgresql.org/docs/current/high-availability.html) of PostgreSQL.
