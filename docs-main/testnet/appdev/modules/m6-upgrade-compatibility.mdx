...
If you need to make a breaking change, create new templates with the desired structure and add `Upgrade` choices to the old templates that archive old contracts and create new ones. See [SCU compatibility rules](/testnet/appdev/modules/m6-writing-first-upgrade#step-3-verify-compatibility) for details on what the compiler checks.
...
- [Writing Your First Upgrade](/testnet/appdev/modules/m6-writing-first-upgrade) — Step-by-step tutorial for creating a v2 package
- [Package Selection](/testnet/appdev/modules/m6-package-selection) — How the ledger resolves package versions
