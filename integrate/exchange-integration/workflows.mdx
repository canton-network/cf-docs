...
Further extensions of these two MVPs to address Day-2 requirements are explored in [Integration Extensions](extensions.mdx#integration-extensions).

<a id="mvp-for-cc"></a>

## MVP for Canton Coin

<Note>The diagrams in the sections below adapt the diagram from the [Information Flows](architecture.mdx#information-flows)
...
* The role of the Registry API Server is taken over by the Canton Coin [Scan services](https://docs.dev.sync.global/app_dev/scan_api/toc_proxy.html)
  ...
  See [Reading from Canton Coin Scan](non-functionals.mdx#reading-from-canton-coin-scan) for more information about
  how to reliably read from multiple Canton Coin Scan instances.</Note>
...
- The Exchange has set up a CC `TransferPreapproval` for their
  `treasuryParty` as explained in [Setup the treasury party](node-operations.mdx#treasury-party-setup).
...
<Note>In most cases a `TransferInstruction` will be completed in a single extra step:
...
completed with a successful transfer, failed, or is still pending an action by one of the stakeholders.</Note>
...
1. [Upload the token admin’s .dar files](node-operations.mdx#dar-file-management) to your validator node.
