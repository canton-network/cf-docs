...
A CI/CD pipeline for Canton applications follows the same structure as any other software project: build, test, package, deploy. The Canton-specific parts are the `dpm` commands for compiling Daml, running Script tests, and managing DAR artifacts.
...
2. **Test** — Run Daml Script unit tests and backend integration tests
3. **Package** — Produce deployable artifacts (DARs, container images)
...
dpm codegen-java .daml/dist/your-project.dar -o generated-java

# Generate TypeScript bindings (if needed)
dpm codegen-js .daml/dist/your-project.dar -o generated-ts
```
...
This runs all `Script ()` declarations in your test package against the IDE Ledger. If any assertion fails, `dpm test` exits with a non-zero code and the pipeline fails.
...
For tests that need a full multi-validator setup, start [LocalNet](https://docs.sync.global/app_dev/testing/localnet.html) with Docker Compose instead of the sandbox. This is heavier but covers cross-validator scenarios.
...
Deployment to each environment involves two parts: uploading the DAR to the participant node and deploying off-ledger services (backend, frontend, PQS).

### DAR upload

Upload your DAR to the target validator's participant node through the Ledger API or Admin API:

```bash
# Via the JSON API
curl -X POST "https://${LEDGER_HOST}:${JSON_API_PORT}/v2/packages" \
...
Each promotion runs the same deployment steps with different environment configuration (see [Environment Configuration](/docs-main/appdev/modules/m5-environment-configuration)).
...
- [Testing Strategies](/docs-main/appdev/modules/m5-testing-strategies) — Testing pyramid and approach details
- [Environment Configuration](/docs-main/appdev/modules/m5-environment-configuration) — Per-environment configuration management
- [Deployment Progression](/docs-main/appdev/modules/m5-deployment-progression) — What to verify at each promotion stage
