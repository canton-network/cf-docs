...
Start LocalNet and use `curl` to verify both endpoints. First, add a comment:

```bash
curl -X POST http://localhost:8080/api/licenses/<CONTRACT_ID>/comments \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <TOKEN>" \
  -d '{"body": "Renewed for another quarter"}' \
  --url-query "commandId=$(uuidgen)"
```

Then retrieve comments for the same license:

```bash
curl http://localhost:8080/api/licenses/<CONTRACT_ID>/comments \
  -H "Authorization: Bearer <TOKEN>"
```

Replace `<CONTRACT_ID>` with an active license contract ID from `GET /api/licenses`, and `<TOKEN>` with a valid bearer token from your Keycloak login.
