...
Participant nodes are the workhorses of Canton. They:
...
> "A Canton Participant is the private, self-sovereign computational and storage unit for an entity within the Canton Network."
...
flowchart LR
    subgraph Sync[Synchronizer]
        SEQ[Sequencer<br>Orders messages] <--> MED[Mediator<br>Facilitates confirmation]
    end

    P1[Participant 1] <--> SEQ
    P2[Participant 2] <--> SEQ
    P3[Participant N] <--> SEQ
```

**Sequencer**
- Orders and distributes encrypted messages between participants
- Provides total ordering for transactions
- **Does not see** decrypted transaction content
- Ensures all participants receive messages in the same order

**Mediator**
- Facilitates the confirmation protocol
- Collects confirmations from participants
- Declares transaction outcomes (committed or rejected)
- **Does not see** decrypted transaction content
...
| **7. Confirm** | All Participants | Send confirmation/rejection to mediator |
| **8. Collect** | Mediator | Aggregates confirmations, determines outcome |
...
```mermaid
flowchart TB
    subgraph GSD[Global Synchronizer Decentralized]
...
| **Admin API** | gRPC | Node administration, party management |
