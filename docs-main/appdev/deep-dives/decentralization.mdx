...
- **Application layer** — How many validators host the parties in your application
...
**Trust model:** The party trusts all validators that host it. Transactions involving the party may be processed by any of its hosting validators. This reduces single-point-of-failure risk but requires trust in multiple operators.
...
See [Multi-Hosting](/docs-main/appdev/deep-dives/multi-hosting) for implementation details.

## Level 4: BFT Synchronizers
...
## Level 5: Multi-Synchronizer

Canton supports connecting validators to multiple synchronizers simultaneously. A party can have contracts on the Global Synchronizer and on one or more private (extension) synchronizers, with the ability to transfer contracts between them.

**Trust model:** Different workflows can have different trust properties. Sensitive bilateral transactions might use a private synchronizer operated by the two parties, while settlement against Canton Coin happens on the Global Synchronizer.

**When this is appropriate:**
- Applications with mixed privacy/performance requirements
- Workflows that need both public settlement and private processing
- Organizations with regulatory requirements about where data is processed

## Choosing Your Level

The right level of decentralization depends on your application's specific requirements:

- **Start simple** — Most applications begin at Level 2 (multi-validator) because cross-organizational workflows are Canton's primary use case
- **Add resilience** — Move to Level 3 (multi-hosting) when your application needs high availability
- **Get BFT for free** — If you're on the Global Synchronizer, you already have Level 4
- **Add privacy** — Use Level 5 (multi-synchronizer) when some workflows need private processing
...
- [Multi-Hosting](/docs-main/appdev/deep-dives/multi-hosting) — Implementation details for distributing parties across validators
