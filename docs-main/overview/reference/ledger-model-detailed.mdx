...
The section on the [ledger structure](#structure) section answered the question “What does the Ledger looks like?” by introducing a hierarchical format to record the party interactions as changes to the Ledger.
...
The last validity condition ensures that only the intended parties can request a change,
...
The running example of [Bob skipping the propose-accept workflow](#skip-the-propose-accept-workflow) will be used to show how node ③ requires more authorizers than its authorization context provides, and is thus not well authorized.
...
If for every set `P` in `F`, the projection of a Ledger `L` to `P` is weakly valid for `P` intersected with `Q`,
then the projection of `L` to `Q` is weakly valid.
Note that this projection of `L` to `Q` is equivalent to `L` for `Q` due to the [absorption property of projection](#transaction-projection).
