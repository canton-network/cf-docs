...
CC has three primary roles on the network:
...
Unlike most blockchain tokens, CC balances are private. Only you and explicitly entitled parties can see your holdings. There is no public ledger of balances or transfers.
...
- **Computational complexity** -- Transactions with more complex Daml execution consume more credits
...
- **Estimate traffic usage** -- Understand how many transactions your application generates per day and how large they are. Use DevNet or TestNet to measure actual traffic consumption.
...
1. The `License_Renew` choice creates a `LicenseRenewalRequest` contract that implements the Splice `AllocationRequest` interface
...
3. Once payment settles, the provider exercises `LicenseRenewalRequest_CompleteRenewal` to create the renewed license
...
- [Canton Coin overview](/docs-main/overview/understand/canton-coin) -- Deeper background on CC tokenomics, validator rewards, and governance
- [Backend Development](/docs-main/appdev/modules/m4-backend-dev) -- Handling transaction errors, including insufficient-traffic failures
