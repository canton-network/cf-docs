...
- **Connects to the synchronizer**: Communicates with the Global Synchronizer
...
| Role | Function | Who Operates |
|------|----------|--------------|
| **Validator** | Hosts parties, stores contracts, exposes APIs | Application operators, enterprises |
| **Super Validator** | Operates synchronizer infrastructure, governs network | Major institutions, approved operators |

**As a validator, you do not:**
- Operate synchronizer components (sequencer/mediator)
- Participate directly in network governance
- Need to run BFT consensus nodes

**As a validator, you do:**
- Run your own participant node
- Host parties for your users/applications
- Pay traffic fees in Canton Coin
- Keep your node updated with network versions

## Network Environments

Canton Network operates across four environments:

| Environment | Purpose | Access | Canton Coin |
|-------------|---------|--------|-------------|
| **LocalNet** | Local development | Anyone | Test (local) |
| **DevNet** | Integration testing | VPN + sponsorship | Test (faucet) |
| **TestNet** | Staging | Application process | Test (faucet) |
...
1. **Contact a Super Validator** sponsor ([list at sync.global](https://sync.global))
...
| Handled By GSF | Description |
|----------------|-------------|
| **Consensus** | Super Validators run BFT consensus |
| **Governance** | GSF and SVs manage network parameters |
