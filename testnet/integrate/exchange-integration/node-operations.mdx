...
As explained in [Tokenomics and Rewards](../tokenomics-and-rewards/index.md#tokenomics-and-rewards),
...
1. Create the `treasuryParty` using the wallet SDK to [Create an External Party (Wallet)](../party-management/index.md#create-an-external-party) with a key managed in a system of your choice
...
3. Wait for your Tx History Ingestion to have caught up to record time
   `acs_timestamp` or higher. Note that you must consume [offset checkpoints](txingestion.md#offset-checkpoints)
...
6. Follow the shortened version below of the
   [procedure for restoring a validator node from a backup](disaster-recovery.md#validator-backup-restore)
