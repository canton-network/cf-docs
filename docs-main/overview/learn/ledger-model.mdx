...
<Warning>
Contract keys, like contract IDs, are global within a synchronizer. Design keys carefully to avoid leaking information about contract existence.
</Warning>
...
- Chosen by the submitting participant and checked by the synchronizer against record time bounds
- Not required to increase monotonically per synchronizer (record time is monotonic)
- Used for time-based contract logic
- Required to be no earlier than the ledger times of all contracts used in the transaction
