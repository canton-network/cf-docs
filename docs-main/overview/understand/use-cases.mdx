---
title: "Use Cases"
description: "Real-world applications where Canton's privacy model enables solutions impossible on traditional blockchains"
---
...
```haskell
-- Each leg is visible only to relevant parties
template ShipmentLeg
  with
    shipper : Party
    receiver : Party
  ...
  where
    ...
    observer logisticsProvider, financier

    -- Financier sees terms; logistics provider sees goods
    -- Next leg participants don't see previous leg terms
```

The supply chain operates with full traceability for entitled parties while maintaining commercial confidentiality.
  ...
  where
    ...
    choice Transfer : ContractId Security
      with
        newHolder : Party
        transferAgent : Party
      controller holder, transferAgent
      do
        -- Compliance checks happen here
        assert (newHolder `elem` approvedHolders)
        create this with holder = newHolder
```

The regulator observes all holdings (for compliance) without that data being public. Trades remain private between counterparties while still being auditable.
...
|-------------|---------------|
| **Fully public applications** | Transparency is a feature, not a limitation |
| **EVM compatibility** | Canton uses Daml, not Solidity |
| **Anonymous participation** | Canton parties have identity |
...
<Card title="Start Building" icon="code" href="/docs-main/appdev/get-started/choose-your-path">
