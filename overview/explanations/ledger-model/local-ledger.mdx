...
Recall that the party projections are as follows.
...
: For a causality graph G,
  ...
  * act<sub>1</sub> and act<sub>2</sub> belong to different transactions in vertices tx<sub>1</sub> and tx<sub>2</sub> and there is a path in G from tx<sub>1</sub> to tx<sub>2</sub>.
    <Note>
    Checking for an *edge* instead of a *path* in G from tx<sub>1</sub> to tx<sub>2</sub> is equivalent
...
<Note>
The reduction reduce<sub>X</sub>(G) of an X-consistent causality graph G can be computed as follows:
...
3. reduce<sub>X</sub>(G) is the transitive closure of G’.
</Note>
...
<Note>
The transaction trees of the Update Service omit **Fetch** and **NoSuchKey** actions
that are part of the transactions in the local ledger.
The **Fetch** and **NoSuchKey** actions are thus removed before the Update Service outputs the transaction trees.
</Note>
