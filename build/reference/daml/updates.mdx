...
This page gives reference information on Updates. For the structure around them, see [Overview: Template Structure](structure.mdx).
...
- They are what can go in a [choice body](choices.mdx#daml-ref-choice-body).
...
- Returns unit.
- Requires authorization from the contract controllers/signatories. Without the required authorization, the transaction fails. For more detail on authorization, see [Signatory Parties](templates.mdx#daml-ref-signatories).
...
- Requires authorization from the signatories of the contract being created. This is given by being signatories of the contract from which the other contract is created, being the controller, or explicitly creating the contract itself.
  ...
  If the required authorization is not given, the transaction fails. For more detail on authorization, see [Signatory Parties](templates.mdx#daml-ref-signatories).
...
- Like `exercise`, but the contract is specified by [contract key](contract-keys.mdx#contractkeys), instead of contract ID.
- For details see [Reference: Contract Keys: exerciseByKey](contract-keys.mdx#exercisebykey)
...
- `fetchByKey` function.
- Like `fetch`, but fetches the contract with that [contract key](contract-keys.mdx#contractkeys), instead of the contract ID.
- For details see [Reference: Contract Keys: fetchByKey](contract-keys.mdx#fetchbykey).
...
- Use this to check whether a contract with the given [contract key](contract-keys.mdx#contractkeys) exists.
- For details see [Reference: Contract Keys: visibleByKey](contract-keys.mdx#visiblebykey)
...
- Use this to confirm that a contract with the given [contract key](contract-keys.mdx#contractkeys) exists.
- For details see [Reference: Contract Keys: lookupByKey](contract-keys.mdx#lookupbykey)
...
See the documentation on [Let](expressions.mdx#daml-ref-let).
