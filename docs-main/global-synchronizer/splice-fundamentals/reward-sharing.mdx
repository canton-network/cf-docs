...
When external reward sharing is enabled, the node leaves unassigned `Splice.Amulet.RewardCouponV2` untouched and the node does not automate sharing.
...
External parties participating in reward sharing must have an active [minting delegation](/global-synchronizer/splice-fundamentals/rewards-minting) so the off-node automation can mint the assigned coupons.
See [Batched Sharing](#batched-sharing) for how `batch-size` governs coupon processing.
