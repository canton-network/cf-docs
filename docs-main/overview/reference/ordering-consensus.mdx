...
Canton supports two approaches to the sequencer's ordering backend. The choice determines the trust model for the synchronizer.

**Ordering with CantonBFT.** CantonBFT, the ISS-inspired Native BFT orderer described above, is Canton's own ordering implementation and runs in-process with the sequencer. It supports three deployment modes:

- **Single-node**, for centralized ordering — one operator runs a single orderer node. Lowest latency, easiest to operate, but a single point of trust and failure.
- **Multi-node, run by a single entity**, for high availability — one operator runs multiple orderer nodes for redundancy without distributing trust across parties.
- **Cross-entity**, for full Byzantine fault tolerance — multiple independent parties each run orderer nodes, and no single party controls ordering. This is how the Global Synchronizer operates.

**BFT ordering with external CometBFT.** An external BFT ordering service using [CometBFT](https://cometbft.com/). Canton includes a driver that integrates CometBFT as the sequencer backend, running as a separate process outside the sequencer JVM. This option is used for the Global Synchronizer; it isn't recommended for dedicated synchronizers, which should use CantonBFT instead.

The **Global Synchronizer** is operated by Super Validators, each running sequencer and mediator nodes with orderer nodes communicating peer-to-peer across Super Validator infrastructure. Dedicated Synchronizers can run CantonBFT in any of the three modes above, depending on how much trust needs to be distributed among operators.
