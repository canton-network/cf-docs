...
{/* GENERATED_FROM source="digital-asset/canton" ref="v3.5.19" path="docs-open/src/sphinx/participant/reference/metrics.rst" */}
...
### daml.participant.sync.reassignments.batch-size

> - **Summary**: Number of contracts per reassignment request
> - **Description**: Records the number of contracts carried by each unassignment and assignment request. The `type` label tells the two apart.
> - **Type**: histogram
> - **Qualification**: Debug

### daml.participant.sync.reassignments.finalized

> - **Summary**: Number of unassignments and assignments finalized
> - **Description**: Records the unassignments and the assignments this reassigning participant finalizes. The `type` label tells the two apart.
> - **Type**: counter
> - **Qualification**: Debug

### daml.participant.sync.reassignments.requests

> - **Summary**: Number of unassignment and assignment requests validated
> - **Description**: Records the unassignment and assignment requests this participant validates. The `type` label tells the two apart.
> - **Type**: counter
> - **Qualification**: Debug

### daml.participant.sync.reassignments.submitted

> - **Summary**: Number of unassignments and assignments submitted
> - **Description**: Records the unassignments and the assignments this participant submits, including the assignments submitted on this participant's own initiative once the exclusivity timeout of an unassignment has elapsed. The `type` label tells unassignments and assignments apart. A submission that is retried is recorded once per attempt.
> - **Type**: counter
> - **Qualification**: Debug

### daml.participant.sync.reassignments.unassignment.local-target-timestamp-lag

> - **Summary**: Lag of the target topology behind the requested target timestamp
> - **Description**: Milliseconds by which this participant's view of the target synchronizer's topology lags behind the target timestamp of an unassignment request, measured during validation. Zero means there is no lag.
> - **Type**: histogram
> - **Qualification**: Latency
...
### daml.sequencer.bftordering.p2p.send.grpc-flow-control-not-ready
...
> - **Description**: Records the rate and latency when sending messages to the P2P stream via gRPC's `onNext`.
> - **Type**: timer
> - **Qualification**: Latency

### daml.sequencer.bftordering.p2p.send.sends-dropped
