...
{/* https://lucid.app/lucidchart/cd96a3a6-e10b-4edc-bfb3-a70e484d7c06 */}![image](explanations/images/ha/participant-ha-system-topology.svg)
...
<Danger>The health endpoint polling frequency can affect the failover duration.</Danger>
...
<Note>Alternative approaches for leader election, such as Raft, are unsuitable because the leader status can be lost between the leader check and the use of the shared resource, i.e. writing to the database. Therefore, we cannot guarantee a single writer.</Note>
...
<Note>Using the same connection for writes ensures that the lock is active while writes are performed.</Note>
...
<Note>On Oracle, the lock scope is the schema name, i.e. user name. On PostgreSQL, it is the name of the database.</Note>
...
<Danger>A passive node replica does not hold the exclusive lock and cannot write to the shared database.</Danger>
