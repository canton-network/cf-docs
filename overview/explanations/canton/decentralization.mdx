{/* source: canton / canton/3.4/overview/explanations/canton/decentralization.rst */}
...
<a id="decentralized-namespace"></a>

## Decentralized namespaces

As described in the section [Namespaces](topology.mdx#topology-namespaces), a namespace is
...
Whenever a topology transaction expects a [unique identifier](topology.mdx#topology-unique-identifiers), the namespace for that unique identifier can be
...
* **Decentralization of ownership**: the namespace for the `SynchronizerId` is
  created as a [decentralized namespace](#decentralized-namespace) with
...
* **Decentralization of Mediator groups**: the Synchronizer owners can set up
  ...
  reach the same verdict for a confirmation request in the [two-phase
  commit protocol](protocol.mdx#protocol-two-phase-commit), before the Sequencers deliver
...
See [Local and external parties](external-party.mdx#overview-canton-external-parties) for more details.
