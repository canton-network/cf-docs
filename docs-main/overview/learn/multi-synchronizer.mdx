...
## Multiple Synchronizers — formal specification
...
For each input contract `c`<sub>i</sub>, denote by `S`<sub>i</sub> its assignation (at the submission time). A Synchronizer `S` is admissible for the transaction if:

- The reassignment of `c`<sub>i</sub> from `S`<sub>i</sub> to `S` is valid.
- The submitting participant is a reassigning participant for the submitting party for each `c`<sub>i</sub>.
...
![Reassignment protocol overview](/images/overview/reassignment-high-level-2.svg)
...
![Multi-synchronizer running example](/images/overview/reassignment-running-example.svg)
...
<Warning title="Pre-reviewed Content - Do Not Modify">
This section was copied from existing reviewed documentation.
**Source:** `docs-website:docs/replicated/canton/3.4/overview/explanations/canton/traffic-management.rst`
Reviewers: Skip this section. Remove markers after final approval.
</Warning>
...
<img src="./images/traffic_control_enforcement.svg" class="align-center" style="width:100.0%" alt="image" />
