...
For tests that need a full multi-validator setup, start [LocalNet](/mainnet/appdev/modules/m5-localnet-development) with Docker Compose instead of the sandbox. This is heavier but covers cross-validator scenarios.
...
Each promotion runs the same deployment steps with different environment configuration (see [Environment Configuration](/mainnet/appdev/modules/m5-environment-configuration)).
...
- [Testing Strategies](/mainnet/appdev/modules/m5-testing-strategies) — Testing pyramid and approach details
- [Environment Configuration](/mainnet/appdev/modules/m5-environment-configuration) — Per-environment configuration management
- [Deployment Progression](/mainnet/appdev/modules/m5-deployment-progression) — What to verify at each promotion stage
