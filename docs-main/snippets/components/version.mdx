...
export const networkOrder = ['mainnet', 'testnet', 'devnet'];
export const deploymentNetworkOrder = ['devnet', 'testnet', 'mainnet'];

export const tokenPattern = /\|([A-Za-z0-9_]+)\|/g;

export const tokenTextValue = (value) => {
...
export const replacementForToken = (token, substitutions) => tokenTextValue(substitutions?.[token]);

export const replaceStringProp = (value, substitutions) => {
...
export const replaceTextNode = (value, substitutions, keyPrefix = 'token') => {
  if (typeof value !== 'string') {
    return value;
  }
...
export const replaceNode = (node, substitutions, keyPrefix = 'node') => {
