{/* source: daml / daml/3.4/sdk/sdlc-howtos/applications/develop/explicit-contract-disclosure.rst */}
{/* Copyright (c) 2023 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

<a id="explicit-contract-disclosure"></a>

# Explicit Contract Disclosure

In Daml, you must specify who can view data using [stakeholder](../../../reference/glossary.mdx#stakeholder) annotations in your template definitions.
...
<Note>
Explicit disclosure is activated by default.
...
The following snippet of [Daml Script](../../../reference/glossary.mdx#daml-script) models the setup of the trade between the parties.
...
decide to [disclose](#stakeholder-contract-share) their contracts to any party desiring to execute such a trade.
...
<Note>
The Ledger API uses the disclosed contracts attached to command submissions
...
<a id="stakeholder-contract-share"></a>
...
associated [CreatedEvent](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-createdevent),
...
by conventional means, such as HTTPS, SFTP, or e-mail. A [DisclosedContract](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-disclosedcontract) can
be constructed from the fields of the same name from the original contract’s `CreatedEvent`.

<Note>
The `created_event_blob` field in `CreatedEvent` (used to construct the [DisclosedContract](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-disclosedcontract))
is populated **only** on demand for `GetUpdates`, `GetUpdateTrees`, and `GetActiveContracts` streams.

To learn more, see [configuring event format](../../../explanations/ledger-api-services.mdx#event-format).
</Note>

<a id="submitter-disclosed-contract"></a>

## Attaching a disclosed contract to a command submission

A disclosed contract can be attached as part of the `Command`’s [disclosed_contracts](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-commands-disclosed-contracts)
and requires the following fields (see [DisclosedContract](../../../reference/lapi-proto-docs.mdx#com-daml-ledger-api-v2-disclosedcontract) for content details) to be populated from
...
<Note>
Only contracts created starting with Canton 2.8 can be shared as disclosed contracts.
...
<Note>
For an example using Java bindings for client applications, see the
[Java Bindings StockExchange example project](https://github.com/digital-asset/ex-java-bindings/blob/f474ae83976b0ad197e2fabfce9842fb9b3de907/StockExchange/README.rst).
</Note>
