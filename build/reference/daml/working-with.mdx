{/* source: daml / daml/3.4/sdk/reference/daml/working-with.rst */}
...
- `pass`: (in [Daml Script tests](../../tutorials/smart-contracts/daml-scripts.mdx#test-using-scripts) only) use `pass : RelTime -> Script Time` to advance the ledger time by the argument amount. Returns the new time.
...
<Note>We’d usually recommend using `foldl`, as `foldr` is usually slower. This is because it needs to traverse the whole list before starting to discharge its elements.</Note>
