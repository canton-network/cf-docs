...
If you need to make a breaking change, create new templates with the desired structure and add `Upgrade` choices to the old templates that archive old contracts and create new ones. See [SCU compatibility rules](/docs-main/appdev/modules/m6-writing-first-upgrade#step-3-verify-compatibility) for details on what the compiler checks.
