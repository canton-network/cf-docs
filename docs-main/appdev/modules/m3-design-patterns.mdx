...
Beyond the composition patterns above, this section covers common multi-party workflow patterns used in Daml.

### Propose-Accept

The most common way to get multiple parties to agree on a shared contract. One party creates a proposal contract that the other party can accept, reject, or let expire. The `IouProposal` [in the authorization module](/docs-main/appdev/modules/m3-authorization#use-propose-accept-workflow-for-one-off-authorization) is a classic example. See the [Propose-Accept pattern documentation](https://docs.digitalasset.com/build/3.4/sdk/howtos/smart-contracts/develop/patterns/propose-accept) for a full walkthrough.

### Delegation

Gives one party the right to exercise a choice on behalf of another. A principal creates a delegation contract that authorizes an agent to act for them. The agent can then control contracts on the ledger without the principal committing each action. See the [Delegation pattern documentation](https://docs.digitalasset.com/build/3.4/sdk/howtos/smart-contracts/develop/patterns/delegation) for a full walkthrough.

### Authorization

Verifies that a controlling party has the right permissions before they take certain actions. An authorization contract serves as proof. Before performing a restricted action (like transferring an asset), the choice body checks for the existence and validity of the authorization contract. See the [Authorization pattern documentation](https://docs.digitalasset.com/build/3.4/sdk/howtos/smart-contracts/develop/patterns/authorization) for a full walkthrough.

### Locking

Prevents choices from being exercised on a contract while it is in a locked state. Useful for scenarios like securities settlement where assets must be frozen during clearing. Three approaches exist: locking by archiving (recreating the contract with a lock flag), locking by state (adding a lock field to the template), and locking by safekeeping (transferring the contract to a trusted third party). See the [Locking pattern documentation](https://docs.digitalasset.com/build/3.4/sdk/howtos/smart-contracts/develop/patterns/locking) for a full walkthrough.

### Multiple party agreement

Collects signatures from more than two parties. A pending contract is created by one party and passed around for others to sign. Once all parties have signed, any signatory can finalize the agreement and create the actual contract. This avoids the limitations of chaining bilateral Propose-Accept workflows when three or more parties need to reach consensus. See the [Multiple Party Agreement pattern documentation](https://docs.digitalasset.com/build/3.4/sdk/howtos/smart-contracts/develop/patterns/multiple-party-agreement) for a full walkthrough.