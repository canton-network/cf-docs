...
that validator node's administrator ([docs](https://docs.test.sync.global/validator_operator/validator_compose.html#deployment)).
The validator node automatically mints rewards for that party.
It can further be configured to
[automatically purchase traffic](https://docs.test.sync.global/validator_operator/validator_helm.html#configuring-automatic-traffic-purchases)
...
3. Setup [automatic traffic purchases in the validator app](https://docs.test.sync.global/validator_operator/validator_helm.html#configuring-automatic-traffic-purchases).
4. Optional: setup [auto-sweep](https://docs.test.sync.global/validator_operator/validator_helm.html#configuring-sweeps-and-auto-accepts-of-transfer-offers) from the `exchangParty` to your `treasuryParty` to limit the funds managed directly by the validator node.
...
See this [FAQ to measure the traffic spent on an individual transaction](https://docs.test.sync.global/faq.html#term-How-do-I-determine-the-traffic-used-for-a-specific-transaction).
...
[deploy your validator node](https://docs.test.sync.global/validator_operator/validator_compose.html#deployment).
...
1. [Log into the wallet UI for the validator user](https://docs.test.sync.global/validator_operator/validator_helm.html#logging-into-the-wallet-ui), which presents itself as in this screenshot:
...
1. [Log into the wallet UI for the validator user](https://docs.test.sync.global/validator_operator/validator_helm.html#logging-into-the-wallet-ui) on your MainNet validator node.
...
[configured when setting up authentication for your validator node](https://docs.test.sync.global/validator_operator/validator_helm.html#oidc-provider-requirements).
Another option is to
[log-in to your Splice Wallet UI for the validator operatory party](https://docs.test.sync.global/validator_operator/validator_helm.html#logging-into-the-wallet-ui)
...
[how to monitor your validator node](https://docs.test.sync.global/deployment/observability/index.html).
Note in particular that it includes
[Grafana dashboards](https://docs.test.sync.global/deployment/observability/metrics.html#grafana-dashboards)
for monitoring the traffic usage, balances of local parties (e.g., the `exchangeParty`),
and [many other metrics](https://docs.test.sync.global/deployment/observability/metrics_reference.html).

## Rolling out Major Splice Upgrades

For major protocol changes, the global sychronizer undergoes a [Major
Upgrade Procedure](https://docs.test.sync.global/validator_operator/validator_major_upgrades.html).
...
[Splice docs](https://docs.test.sync.global/validator_operator/validator_major_upgrades.html).
...
1. Wait for the synchronizer to be paused and your node to have
   ...
   docs](https://docs.test.sync.global/validator_operator/validator_major_upgrades.html#catching-up-before-the-migration).
...
5. Upgrade your validator and connect it to the new synchronizer following the
   [Splice docs](https://docs.test.sync.global/validator_operator/validator_major_upgrades.html#deploying-the-validator-app-and-participant-docker-compose).
6. Follow the shortened version below of the
   [procedure for restoring a validator node from a backup](/testnet/global-synchronizer/production-operations/backup-and-recovery)
   to determine the offset from which to restart your Tx History Ingestion:
    ...
    2. Log into the [Canton Console of your validator node](https://docs.test.sync.global/deployment/console_access.html) and query the offset `offRecovery` assigned to the ACS import transactions at time `0001-01-01T00:00:00.000000Z` using
