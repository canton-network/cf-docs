...
The Splice wallet uses a UTXO model for Canton Coin holdings. Each active `Holding` contract on the ledger represents a single UTXO with a specific CC amount. UTXO management matters for wallet providers because active Holding contracts consume storage and compute on the hosting validators, and each UTXO input to a transfer incurs extra traffic cost.
