...
1.  Extract the latest version of the `splice-util-token-standard-wallet.dar` file from the release bundle (<a href="https://github.com/digital-asset/decentralized-canton-sync/releases/download/v0.8.1/0.8.1_splice-node.tar.gz">Download Bundle (DevNet 0.8.1)</a>).
...
1.  Extract the latest version of the `splice-util-token-standard-wallet.dar` file from the release bundle (<a href="https://github.com/digital-asset/decentralized-canton-sync/releases/download/v0.8.0/0.8.0_splice-node.tar.gz">Download Bundle (TestNet 0.8.0)</a>).
2.  Upload the extracted `.dar` file to your validator node.
3.  Adjust your user onboarding procedure such that the users signs the creation of a `MergeDelegationProposal` contract (see docs).
4.  Accept the `MergeDelegationProposal` contracts by exercising their `Accept` choice using your wallet provider's party.
...
1.  Extract the latest version of the `splice-util-token-standard-wallet.dar` file from the release bundle (<a href="https://github.com/digital-asset/decentralized-canton-sync/releases/download/v0.7.5/0.7.5_splice-node.tar.gz">Download Bundle (MainNet 0.7.5)</a>).
2.  Upload the extracted `.dar` file to your validator node.
3.  Adjust your user onboarding procedure such that the users signs the creation of a `MergeDelegationProposal` contract (see docs).
4.  Accept the `MergeDelegationProposal` contracts by exercising their `Accept` choice using your wallet provider's party.
...
## Common pitfall: unresolved registry token packages

Your participant must have a registry token's DAR packages installed before it can interpret contracts and transactions involving that token — this is separate from having the Wallet SDK or CLI code that talks to the registry's off-ledger API. If a transfer involving a registry-issued token fails to resolve with an error like `UNRESOLVED_PACKAGE_NAME`, check that the relevant DARs are uploaded and vetted on your participant.
