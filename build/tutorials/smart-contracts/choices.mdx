{/* source: daml / daml/3.4/sdk/tutorials/smart-contracts/choices.rst */}
{/* Copyright (c) 2023 Digital Asset (Switzerland) GmbH and/or its affiliates. All rights reserved. */}
{/* SPDX-License-Identifier: Apache-2.0 */}

# Transform contracts using choices

In the example in [Manipulate contracts](data.mdx#manipulate-contracts) the accountant party wanted to change some data on a contract. They did so by archiving the contract and re-creating it with the updated data. That works because the accountant is the sole signatory on the `Account` contract defined there.
...
<Tip>Remember that you can load all the code for this section into a folder called `intro-choices` by running `dpm new intro-choices  --template daml-intro-choices`</Tip>

## Choices as methods

If you think of templates as classes and contracts as objects, where are the methods?

Take as an example a `Contact` contract on which the contact owner wants to be able to change the telephone number, just like on the `Account` in [Manipulate contracts](data.mdx#manipulate-contracts). Rather than requiring them to manually look up the contract, archive the old one, and create a new one, you can provide them a convenience method on `Contact`:
...
As mentioned in [Data types](data.mdx), within a choice we use `create`
...
In [Basic contracts](contracts.mdx) you learned about the high-level structure of a Daml ledger. With choices and the exercise function, you have the next important ingredient to understand the structure of the ledger and transactions.
...
With the power of choices, you can build your first interesting model: issuance of cash IOUs (I owe you). The model presented here is simpler than the one in [Data types](data.mdx) as it’s not concerned with the location of the physical cash, but merely with liabilities:
...
In [Add constraints to a contract](constraints.mdx), you will learn how to restrict data and transformations further. In that context, you will also learn about time on Daml ledgers, `do` blocks, and `<-` notation within those.
