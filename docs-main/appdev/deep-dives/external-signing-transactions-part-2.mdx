---
title: "External Signing: Submitting Transactions, Part 2"
description: "Exercise a choice on an externally signed contract using explicit disclosure"
---

# Submit Externally Signed Transactions - Part 2

{/* COPIED_START source="docs-website:docs/replicated/canton/3.5/sdk/tutorials/app-dev/external_signing_submission_part_2.rst" hash="a408bb90" */}

Complete [Part 1](/appdev/deep-dives/external-signing-transactions-part-1) before proceeding.
...
- `ping_created_event`: Event retrieved in the last [step](/appdev/deep-dives/external-signing-transactions-part-1#6-observe-the-transaction-outcome) of Part 1.
...
The only significant difference with Part 1 in the metadata is: `input_contracts`.
This field now contains the input `Ping` contract. It is also included in the hash of the transaction.

<Warning>
Like in Part 1, the transaction must be [validated](/appdev/deep-dives/external-signing-transactions-part-1#2-validate-the-transaction), [hashed](/appdev/deep-dives/external-signing-transactions-part-1#3-compute-the-transaction-hash) and [signed](/appdev/deep-dives/external-signing-transactions-part-1#4-sign-the-transaction-hash).
The hash computation and signature is performed by the `execute_and_get_contract_id` function provided [at the end of Part 1](/appdev/deep-dives/external-signing-transactions-part-1#6-observe-the-transaction-outcome), as shown in the next section.
</Warning>
