{/* source: canton / canton/3.4/overview/explanations/canton/security.rst */}
...
As a preliminary, it outlines general options for [storing secrets in Canton](#storage-secrets).
Then, it gives an [overview of the various kinds of cryptographic keys](#overview-crypto-keys) and how Canton uses and stores them.
...
<a id="session-encryption-keys"></a>
...
Every [topology namespace](topology.mdx#topology-namespaces) has a public signing key, called the namespace root key.
...
A namespace can have [further signing keys](topology.mdx#topology-delegation) and operators can use them as well to authorize topology transactions for the namespace.

Examples of topology transactions that a namespace key (root or not) can authorize include:

- [NamespaceDelegation](topology.mdx#topology-delegation): authorize further keys to authorize topology transactions related to the namespace,
- [OwnerToKeyMapping](topology.mdx#topology-cryptographic-keys): associate a key with a Canton node,
- [PartyToParticipant](topology.mdx#topology-parties): associate a party with a participant.

Canton supports the following [options](#storage-secrets) for storing private namespace keys:
...
Canton supports the following [options](#storage-secrets) for storing node signing keys:
...
Alternatively, the submitting party (referenced as [external party](external-party.mdx#overview-canton-external-parties)) may
...
The recommended option for storing signing keys of external parties is [“offline”](#storage-option-offline).
Other options (for example database) may be technically possible, but they are not recommended.

### Encryption keys

When a Participant Node proposes a transaction, it decomposes the transaction into so called transaction views.
...
but instead they apply [session encryption keys](#session-encryption-keys) for better performance.
...
- Operators can configure Participant Nodes to enable authorization at the Ledger API.
  If enabled, clients of a Ledger API need to include a [JWT](https://jwt.io) token into every request.
