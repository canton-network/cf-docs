...
[distributed synchronizer bootstrapping process](bootstrap.mdx#synchronizer-bootstrap), or dynamically add a new Sequencer
at a later point as described in this section.

The reverse procedure is documented in the [Sequencer decommissioning section](../decommission/index.mdx#decommissioning-sequencers).
...
1. Assuming that at least one existing Sequencer is accessible, [prepare a new Sequencer](bootstrap.mdx#synchronizer-bootstrap) and make sure it’s running.
...
3. Set up new connections in either or both directions for all Sequencers using the following commands:
   ...
   > For the newly-onboarded Sequencer, the endpoints can be [configured as part of the initial network](../configure/sequencer_backend.mdx#sequencer-backend-bft-initial-peers).
...
If you encounter issues, refer to the [troubleshooting guide](../troubleshoot/index.mdx#synchronizer-troubleshoot-new-sequencer-onboarding).

For details on the necessary admin commands, check the reference documentation.

### Use separate consoles

Similarly to [initializing a distributed synchronizer with separate consoles](bootstrap.mdx#synchronizer-bootstrapping-separate-consoles),
...
[distributed synchronizer bootstrapping process](bootstrap.mdx#synchronizer-bootstrap), or dynamically add a new Mediator
at a later point as described in this section.

1. [Prepare a new Mediator](bootstrap.mdx#synchronizer-bootstrap) node and make sure it’s running.
...
The reverse procedure is documented in the [Mediator decommissioning section](../decommission/index.mdx#decommissioning-mediators).
