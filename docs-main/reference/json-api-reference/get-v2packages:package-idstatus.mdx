...
sidebarTitle: "/v2/packages/:package-id/status"
---
...
<div class="x2mdx-ref-hero">
  ...
  <p class="x2mdx-ref-summary">Returns the status of a single package.</p>
...
const response = await fetch('http://localhost:7575/v2/packages/{package-id}/status', {
  method: 'GET',
  headers: {
...
curl_setopt_array($curl, [
    CURLOPT_URL => 'http://localhost:7575/v2/packages/{package-id}/status',
...
func main() {
  req, _ := http.NewRequest("GET", "http://localhost:7575/v2/packages/{package-id}/status", nil)
...
var request = HttpRequest.newBuilder()
    .uri(URI.create("http://localhost:7575/v2/packages/{package-id}/status"))
    .header("Authorization", "Bearer <token>")
...
uri = URI('http://localhost:7575/v2/packages/{package-id}/status')
...
Allowed values: <code>PACKAGE_STATUS_UNSPECIFIED</code>, <code>PACKAGE_STATUS_REGISTERED</code>.
