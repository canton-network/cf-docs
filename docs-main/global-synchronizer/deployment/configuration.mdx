...
| Deployment | Primary config | Override mechanism |
|---|---|---|
| Docker Compose | Environment variables in `.env` and `compose.yaml` | `JAVA_TOOL_OPTIONS` for JVM/HOCON overrides |
| Kubernetes | Helm `values.yaml` files | ConfigMaps, Secrets, and Helm value overrides |
