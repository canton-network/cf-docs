...
<Note>Make sure to complete the [Prerequisites](prerequisites.mdx#utility-prerequisites) before proceeding.</Note>
...
- `AUTH_CLIENT_ID`: the one you [created for the Utility UI](prerequisites.mdx#oidc-setup)
- `UTILITY_APP_UTILITY_BACKEND_URL`: [depends on which network you’re connecting to](../reference/operator-backend-api/index.mdx#operator-backend-url)
