...
The participant log shows:

```
Authorization error: Audience doesn't match the target value
```

The `aud` claim in the JWT must match the target audience configured on the validator's participant (`LEDGER_API_AUTH_AUDIENCE` in Docker Compose, `auth.targetAudience` in Helm). Check your OIDC provider's application settings and align the audience with your validator config.
